...
- **Docked styling toolbars:** Font and axis controls now stay anchored to the workspace header instead of floating, preventing sticky header jumps when typography or axis settings open.【F:css/style.css†L1068-L1070】【F:js/shared/fontControls.js†L222-L230】【F:js/shared/axisControls.js†L229-L246】
...
- **Snapshot-aware paste undo:** Full-table paste operations store lightweight snapshots when the sheet stays below the configurable `Shared.tableImport.snapshotCellThreshold` (default 12,000 cells). Larger grids automatically downgrade to diff-only undo and log a debug warning so memory stays bounded during massive imports.【F:js/shared/tableImport.js†L55-L188】【F:js/shared/tableImport.js†L404-L543】
...
- **Manual species detection:** Trigger batched lookups through the **Detect Species** button; cached guesses avoid repeated network work and respect cancellation when you edit lists rapidly.【F:index.html†L216-L242】【F:js/components/venn.js†L1007-L1110】
...
- **Sweep-line Cox risk sets:** Risk-set construction now uses a single-pass sweep-line window that keeps only aggregate counts and weighted sums in memory, so Cox fitting scales smoothly to thousands of rows without duplicating subject arrays.【F:js/components/survival.js†L1336-L1458】【F:js/components/survival.js†L1476-L1625】
- **Large dataset guardrails:** Survival analysis caps Cox preparation at 20,000 observations with a debug warning so legacy hardware remains responsive even when users paste massive spreadsheets.【F:js/components/survival.js†L1346-L1363】【F:js/components/survival.js†L1626-L1644】
...
6. Workspace components are lazily ensured based on the active tab. The loader now uses dynamic `import()` in the browser with a Jest-friendly `require` fallback, and each `config.ensure()` returns a promise so downstream logic can await bundle hydration before touching the DOM. When writing tests or debugging in JSDOM, call `Main.tabs.handleGraphSelection(type)` (or otherwise activate the tab) before asserting that Handsontable grids or stats panels exist, or preload everything up front with `Main.components.preloadAllBundlesSync()` to mirror the legacy eager loading path.【F:js/main/components.js†L7-L142】【F:js/main/bootstrap.js†L44-L104】【F:js/main/domControls.js†L178-L266】
...
- **Verbose logging:** Enable aggregated debug output during troubleshooting by running `Shared.enableDebugLogging()` (or `Shared.setDebugLogging(true)`) in the browser console. Hot-path helpers such as `Shared.debounceFrame` and the scatter renderer now emit batched summaries—call `Shared.disableDebugLogging()` when finished to avoid unnecessary console noise.
