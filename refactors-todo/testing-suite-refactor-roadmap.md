...
- Bounded full-Jest wave: `scripts/run-full-jest.cjs` now runs all six configured projects sequentially in fresh processes, defaults to four files per process, and forces one file per process for integration and statistical-oracle projects. It emits one aggregate machine-readable report while preserving each project/group status; the full run was intentionally stopped during the slow integration project, so no full-suite green result is claimed.
- Runtime/evidence governance wave: `quality:static` now checks the supported Node runtime; the generated inventory reports 96 explicit and 615 inferred requirement mappings, with all nine critical control-plane scenarios explicitly evidenced and linked to discovered tests.
- Scatter ownership wave: style controls now write through the active Scatter owner session before redraw. The Match Styles contract passed 3/3 repeated; same-type isolation passed 11/11; the persistence matrix remains 10/11 with the known Heatmap failure.
...
- Governance closure wave: the duplicate Jest project assignment was removed, per-project discovery now rejects overlapping ownership, the Chromium package shortcut now selects only `full-chromium`, and manifest metadata now records requirements, fixture provenance, readiness, owner expectations, artifacts, mutation policy, and reviewed skip expiry. The focused governance suites passed 20/20; static quality passed.
- Workspace control-plane coverage wave: focused Chromium contracts now cover Match Styles, unsaved Cancel/Discard decisions, and tab drag/reorder. Unsaved decisions and tab reorder passed with one worker; Match Styles reproduces a Scatter owner-payload write-through defect and remains open in `issues.txt`. The inventory now reports 539 manifest rows, 294 Jest files, and 826 Chromium tests in 245 files.
- Production-boundary correction wave: Line's asynchronous draw now stops when its owner session is no longer active, and Scatter worker results retain the producing control signature and project restored owner statistics. Targeted verification passed Line 14/14 and Scatter 1/1. The corresponding resolved issue entries were removed; unrelated product issues remain open.
- Coverage certification wave: the canonical monolithic coverage command reached the Node heap limit, and an 8 GB retry exceeded 30 minutes without completing or producing fresh output. Source coverage was subsequently split by Jest project/process and merged; the full-Jest lane now uses the same bounded process boundary, while the retained coverage artifact is not presented as fresh.
- Coverage process-boundary wave: a V8 runner now executes the explicit source-coverage projects (architecture, statistical-oracle, unit-node, DOM-unit, and workers) separately, shards architecture and statistical-oracle by file, merges maps, and adds zero-count entries only for source files never loaded. Full integration remains a separate release gate because broad index.html fixtures reach the approximately 4 GB Node ceiling under coverage instrumentation. The fresh source aggregate is 63.28% statements, 95.65% branches, 64.61% functions, and 63.28% lines; the baseline was migrated explicitly to this scope/provider after the fresh run, and the trend gate passes. No threshold was raised.

- Coverage certification recheck: file sharding removed the earlier architecture stall, and the source-project aggregate now completes. Coverage of broad integration fixtures still reproduces the process-memory boundary and Line timing overhead. The baseline migration is recorded in `test-support/coverage-baseline.json`; per-project reporting and higher floors remain follow-up work.
...
- Production-derived loader/owner fixture contracts: focused loader and persistence suites passed; twenty-three Jest files use the production-derived application loader, and the manifest-derived component bootstrap now covers the isolated Heatmap and figure-summary component suites without copied shared-module lists. Venn's additional-tab suite remains on a legacy partial bootstrap until its owner/state blocker is fixed. PCA view is split into a 29-test behavior suite and a one-test cache/rotation suite; both pass under selective production preload. Line view has a reusable event/async teardown boundary, but its latest serial full-suite result is 13/14 because an asynchronous draw reaches a cleared plot reference; the production execution-context defect is recorded in `issues.txt`. The all-component DOM-binding switch case and config-isolation case also pass under explicit eleven-component preload.
...
- Final static lane: passed after the PCA split, graph-export consolidation, Line teardown migration, welcome/logistic extraction, Box swarm extraction, all-component DOM-binding/config-isolation loader migrations, lifecycle model extraction, AG Grid source-contract extraction, `ui.events` ownership sharding, AG Grid clipboard/selection capability sharding, lifecycle core ownership sharding, AG Grid binding ownership sharding, session payload ownership sharding, E2E helper/driver extraction, archive/recovery driver adoption, strict locator migration, reviewed scenario-ID additions, all critical timing waves, the top-20 recovery/archive closure wave, the strict/parameter same-type contract split, and the component-bootstrap/evidence slice. The current static inventory is 540 rows, 713 scenario mappings over 538 files, two intentionally unmapped Firefox-only files, 111 E2E waits, 398 timers, and zero synchronous direct-DOM click shortcuts. Lint, inventory, bootstrap, vendor, asset, and discovery gates passed.
