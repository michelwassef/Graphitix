describe('chartStyle proportional font resize tab-scoped isolation', () => {
  ...
  beforeEach(() => {
    ...
    document.body.innerHTML = `
      ...
      <div id="fontResizeSvg" class="svgbox"></div>
      <input id="fontResizeFont" type="range" value="12" min="6" max="36" />
    `;
    require('../js/vendor.js');
    require('../js/shared/chartStyle.js');
  });

  test('same scope id does not share proportional font resize state across tabs', () => {
    const chartStyle = window.Shared?.chartStyle;
    const svgBox = document.getElementById('fontResizeSvg');
    const scopeId = 'boxGraphPanel';

    setActiveTab('tab-a');
    chartStyle.setProportionalFontResize(true, { scopeId, svgBox, force: true });
    expect(chartStyle.isProportionalFontResizeEnabled({ scopeId, svgBox })).toBe(true);

    setActiveTab('tab-b');
    expect(chartStyle.isProportionalFontResizeEnabled({ scopeId, svgBox })).toBe(false);
    chartStyle.setProportionalFontResize(true, { scopeId, svgBox, force: true });
    expect(chartStyle.isProportionalFontResizeEnabled({ scopeId, svgBox })).toBe(true);
