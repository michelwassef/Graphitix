...
function runPythonOracle(cases, options = {}) {
  ...
  for (const attempt of attempts) {
    const result = runPythonCommand(attempt.command, attempt.args, scriptPath, requestPayload);
    if (result.error) {
    ...
    return parseOracleOutput(result.stdout, result.stderr, attempt.label);
  }
