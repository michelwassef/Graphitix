...
describe('workspace toolbar numeric wheel editing', () => {
  ...
  test('supports shared section replacement, placement, and canonical range attributes', () => {
    const canonicalInput = document.createElement('input');
    canonicalInput.type = 'number';
    canonicalInput.min = '0';
    canonicalInput.step = '0.5';
    canonicalInput.value = '2.7903';
    const overlay = document.createElement('div');
    const shapes = document.createElement('section');
    shapes.className = 'shared-color-picker__section--shapes';
    const oldNumericSection = document.createElement('section');
    oldNumericSection.className = 'shared-color-picker__section shared-color-picker__section--scatter-style';
    const unrelatedSection = document.createElement('section');
    unrelatedSection.className = 'shared-color-picker__section--unrelated';
    overlay.append(shapes, oldNumericSection, unrelatedSection);
    document.body.appendChild(overlay);
    ...
    const section = overlay.querySelector('.shared-color-picker__section--symbol-size');
    const input = section.querySelector('input');

    expect(oldNumericSection.isConnected).toBe(false);
    expect(unrelatedSection.isConnected).toBe(true);
    expect(section.previousElementSibling).toBe(shapes);
    expect(input.value).toBe('2.79');
