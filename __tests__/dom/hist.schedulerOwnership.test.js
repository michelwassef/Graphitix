describe('hist owner draw scheduler ownership', () => {
  ...
  test('owner draw preserves an explicit presentation impact through sanitation', () => {
    const liveScheduler = jest.fn();
    state.scheduleDraw = liveScheduler;

    const session = {
      componentKey: 'hist',
      tabId: 'hist-paint-owner',
      timers: {
        scheduleDraw: null,
        pendingDrawOptions: null
      }
    ...
    expect(hooks.scheduleOwnerDraw({ session }, {
      reason: 'hist-bar-fill-change',
      viewOnly: true,
      renderImpact: 'paint'
    })).toBe(true);

    expect(liveScheduler).toHaveBeenCalledWith(expect.objectContaining({
      reason: 'hist-bar-fill-change',
      viewOnly: true,
      renderImpact: 'paint',
      tabId: session.tabId
    }));
    expect(session.timers.pendingDrawOptions).toMatchObject({ renderImpact: 'paint' });
  });
});
