...
describe('statistical figure-summary reporting matrix', () => {
  ...
  test('Histogram reuses diagnostics only within the owning session cache and data signature', () => {
    const hooks = loadComponent('hist');
    const session = { tabId: 'hist-cache-test', cache: {} };
    const values = [1, 2, 3, 5, 8, 13];
    const goodnessOfFit = jest.spyOn(window.Shared.stats, 'goodnessOfFit').mockReturnValue({
      available: true,
      n: values.length,
      calibration: 'test',
      iterations: 200,
      ks: { statistic: 0.1, pValue: 0.4 },
      ad: { statistic: 0.2, pValue: 0.3 }
    });

    const first = hooks.computeNormalFitDiagnostic(values, { alpha: 0.05, cacheSession: session });
    const second = hooks.computeNormalFitDiagnostic(values, { alpha: 0.05, cacheSession: session });
    hooks.computeNormalFitDiagnostic([...values, 21], { alpha: 0.05, cacheSession: session });

    expect(first.gof).toEqual(second.gof);
    expect(goodnessOfFit).toHaveBeenCalledTimes(2);
    expect(JSON.stringify(session.cache)).not.toMatch(/function/);
    goodnessOfFit.mockRestore();
  });
