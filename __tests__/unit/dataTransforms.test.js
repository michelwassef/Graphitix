describe('Shared.dataTransforms', () => {
  ...
  test('parses dot and comma decimal notation without changing the value', () => {
    const api = window.Shared.dataTransforms;
    expect(api.toFiniteNumber('1.5')).toBe(1.5);
    expect(api.toFiniteNumber('1,5')).toBe(1.5);
    expect(api.toFiniteNumber('1.234,5')).toBe(1234.5);
    expect(api.toFiniteNumber('1,234.5')).toBe(1234.5);
    expect(api.toFiniteNumber('')).toBeNull();
    expect(api.toFiniteNumber('   ')).toBeNull();
  });
