...
describe('component table import bindings', () => {
  ...
  test.each(components)('%s table import controls use deduplicating handlers', (component, binder) => {
    ...
    expect(source).toContain('Shared.componentLifecycle.createOwnerControlBinder');
    expect(source).not.toContain(`function ${binder}`);
    expect(source).toMatch(new RegExp(`${binder}\\([^\\n]+,\\s*['"]click['"],\\s*['"]import-table['"]`));
    expect(source).toMatch(new RegExp(`${binder}\\([^\\n]+,\\s*['"]change['"],\\s*['"]import-file['"]`));
  });

  test('Venn keeps flexible target resolution while sharing owner binding lifecycle', () => {
    const source = fs.readFileSync(path.join(__dirname, '..', '..', 'js', 'components', 'venn.js'), 'utf8');
    expect(source).toContain('Shared.componentLifecycle.bindOwnerControlHandler');
    expect(source).not.toContain('__vennEventHandlers');
  });
});
