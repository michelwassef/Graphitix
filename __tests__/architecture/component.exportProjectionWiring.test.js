...
describe('component export projection wiring', () => {
  test.each(COMPONENTS)('%s declares its component identity on shared SVG export controls', component => {
    ...
    if (['box', 'scatter', 'heatmap'].includes(component)) {
      const mountIndex = source.indexOf('mountSvgControls');
      const snippet = source.slice(mountIndex, mountIndex + 1200);
      expectSource(snippet, `${component}.js export source`).toContain('getSvg:');
      expectSource(snippet, `${component}.js export source`).toContain('getHybridSvg:');
      expectSource(snippet, `${component}.js export source`).toContain('hybridOptions:');
    }
  });
