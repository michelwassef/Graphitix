...
describe('PCA lifecycle ownership contract', () => {
  ...
  test('exposes the owner-scoped primary graph publication probe', () => {
    ...
    expect(source).toContain("'[data-plot-point=\"1\"]'");
    expect(source).toContain("'canvas.pca-fast-points-layer'");
    expect(source).toContain("img[data-graphitix-render-cache-canvas-bitmap=\"true\"]");
  });
