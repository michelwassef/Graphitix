...
describe('semantic SVG composition contracts', () => {
  ...
  test('axis controls prefer semantic metadata while retaining the legacy line fallback', () => {
    const axisControls = source('js/shared/axisControls.js');
    expectSource(axisControls, 'axisControls.js').toContain('*[data-axis-control="1"][data-axis-key="${axisKey}"]');
    expectSource(axisControls, 'axisControls.js').toContain('axisSegment');
    expectSource(axisControls, 'axisControls.js').toContain('getBBox');
    expectSource(axisControls, 'axisControls.js').toContain('line[data-axis-control="1"]');
  });
});
