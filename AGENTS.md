...
- `colorPicker.js` builds a floating palette overlay with Office-style swatches, recent colors, and a shared `<input type="color">` control, exposing `Shared.attachColorPickerNear(el)` so inputs share a consistent picker even in browsers with limited native UI.
