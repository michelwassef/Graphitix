...
const LANE_SPECS = Object.freeze({
  ...
  'full-jest': Object.freeze([nodeCommand([JEST_CLI, '--runInBand'], 'jest-full')]),
  coverage: Object.freeze([nodeCommand([
    JEST_CLI, '--runInBand', '--coverage', '--coverageReporters=json-summary', '--coverageReporters=text-summary'
  ], 'jest-coverage'), nodeCommand([
  ...
  full: Object.freeze([
    nodeCommand([JEST_CLI, '--runInBand'], 'jest-full'),
...
function runLane(lane, options = {}) {
  ...
  for (const command of getLaneSpec(lane)) {
    ...
    const result = execute(command, {
      ...
      requirePythonOracle: lane === 'stats' || options.requirePythonOracle === true
    });
