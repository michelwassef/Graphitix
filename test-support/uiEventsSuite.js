...
const uiEventShard = globalThis.__GRAPHITIX_UI_EVENTS_SHARD__;
const UI_EVENT_SHARD_MATCHERS = Object.freeze({
  'box-line': title => title.startsWith('Box Plot:')
    || title === 'component import opens the import wizard with a fixed graph type'
    || title.startsWith('Line Graph:')
    || title === 'Panel resizer drag triggers Shared.syncPanelWidths',
  scatter: title => title.startsWith('Scatter Plot:'),
  'histogram-and-statistics': title => title.startsWith('Histogram:')
    || title.startsWith('Proportion Graph:')
    || title.startsWith('Correlation Heatmap:')
    || title.startsWith('Surface Plot:')
    || title.startsWith('ROC:')
    || title.startsWith('Survival:'),
  venn: title => title === 'Color picker overlay opens on color input click'
    || title.startsWith('Venn ')
});
if (typeof UI_EVENT_SHARD_MATCHERS[uiEventShard] !== 'function') {
  throw new Error('uiEventsSuite requires a declared UI event shard.');
}
const originalJestTest = global.test;
global.test = (title, ...args) => {
  if (UI_EVENT_SHARD_MATCHERS[uiEventShard](String(title))) {
    return originalJestTest(title, ...args);
  }
  return undefined;
};
