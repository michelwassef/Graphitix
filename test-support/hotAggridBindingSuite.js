const hotBindingShard = globalThis.__GRAPHITIX_HOT_AGGRID_BINDING_SHARD__;
const HOT_BINDING_SHARD_RANGES = Object.freeze({
  'selection-and-payload': Object.freeze({
    first: 'row checkbox selection is opt-in',
    last: 'a user cell edit lifts the owner tab restore suppression so the graph redraws after reopen',
    expected: 7
  }),
  'editor-behavior': Object.freeze({
    first: 'title edits in data-empty columns persist without redraw or render-cache invalidation',
    last: 'loadData with recordUndo can be undone and redone',
    expected: 5
  }),
  'filters-and-analysis': Object.freeze({
    first: 'applyFilters keeps header rows visible and narrows analysis data',
    last: 'user cell edits schedule a userInitiated draw while programmatic loads stay non-user',
    expected: 7
  }),
  'structural-and-scroll': Object.freeze({
    first: 'a user table edit lifts the post-restore draw suppression for the owning tab',
    last: 'horizontal scroll auto-growth uses the real horizontal viewport',
    expected: 6
  })
});
const hotBindingShardSpec = HOT_BINDING_SHARD_RANGES[hotBindingShard];
if (!hotBindingShardSpec) {
  throw new Error('hotAggridBindingSuite requires a declared shard.');
}
...
global.test = (title, ...args) => {
  ...
  observedTestCount += 1;
  if (titleText === hotBindingShardSpec.first) {
    shardStarted = true;
  }
  ...
  if (include) {
    ...
    if (titleText === hotBindingShardSpec.last) {
      shardEnded = true;
    }
    return result;
  }
...
if (!shardStarted || !shardEnded || observedTestCount !== 25 || registeredTestCount !== hotBindingShardSpec.expected) {
  throw new Error(`Invalid AG Grid binding shard ${hotBindingShard}: observed ${observedTestCount}, registered ${registeredTestCount}.`);
}
