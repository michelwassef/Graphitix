...
const sessionPayloadShard = globalThis.__GRAPHITIX_SESSION_ASSIGN_PAYLOAD_SHARD__;
const SESSION_PAYLOAD_SHARD_RANGES = Object.freeze({
  'assignment-guards': Object.freeze({
    first: 'refuses to overwrite a populated payload with null when reason is recovery-interval',
    last: 'every component clears a stale preview when its assigned payload is not renderable',
    expected: 7
  }),
  ...
  'dirty-state': Object.freeze({
    ...
    last: 'clean canonical payload is preserved when adding a tab',
    expected: 9
  }),
  'canonical-ui-events': Object.freeze({
    first: 'global user-input listener promotes trusted change events on workspace controls into markActiveTabUserModified',
    last: 'shared color picker overlay marks the source workspace target dirty even with synthetic events',
    expected: 12
  }),
  'signature-and-cache-persistence': Object.freeze({
    first: 'serializePayloadSignature compacts large untagged data matrices to a short signature',
    last: 'persistUserModifiedTabState marks user dirty and flushes mounted payload state',
    expected: 7
  })
});
const sessionPayloadShardSpec = SESSION_PAYLOAD_SHARD_RANGES[sessionPayloadShard];
if (!sessionPayloadShardSpec) {
  throw new Error('sessionAssignTabPayloadSuite requires a declared shard.');
}
...
global.test = (title, ...args) => {
  ...
  observedTestCount += 1;
  if (titleText === sessionPayloadShardSpec.first) {
    shardStarted = true;
  }
  ...
  if (include) {
    ...
    if (titleText === sessionPayloadShardSpec.last) {
      shardEnded = true;
    }
    return result;
  }
...
global.test.each = (...tableArgs) => {
  const register = originalJestTest.each(...tableArgs);
  const table = tableArgs[0];
  const rowCount = Array.isArray(table) ? table.length : 0;
  return (title, ...args) => {
    observedTestCount += rowCount;
    if (!shardStarted || shardEnded) {
      return undefined;
    }
    registeredTestCount += rowCount;
    return register(title, ...args);
  };
};
