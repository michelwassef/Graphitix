...
const lifecycleShard = globalThis.__GRAPHITIX_COMPONENT_LIFECYCLE_CORE_SHARD__;
const LIFECYCLE_SHARD_RANGES = Object.freeze({
  'authority-and-readiness': Object.freeze({
    firstDescribe: 'componentLifecycle — draw option sanitization',
    lastDescribe: 'componentLifecycle — payload capture ownership',
    first: 'preserves serializable cross-realm records while removing live objects',
    last: 'rejects live capture when a mounted root belongs to another tab',
    expected: 16
  }),
  'cache-and-editing': Object.freeze({
    firstDescribe: 'componentLifecycle — isGraphFrameLayoutAuthorityWrite',
  ...
  'payload-and-async': Object.freeze({
    ...
    lastDescribe: 'componentLifecycle — createAsyncScope',
    first: 'diffPayload: identical payloads → ok:true, no changedPaths',
    last: 'tab-scoped frame debouncer is cancelled with the owning async scope',
    expected: 24
  }),
  'runtime-ownership': Object.freeze({
    firstDescribe: 'componentLifecycle — createRuntimeOwner',
    lastDescribe: 'workspaceTabs teardown contract',
    first: 'capture stores an owned snapshot in workspace and session runtime',
    last: 'disposeTab cancels component async scope for the disposed tab',
    expected: 29
  }),
  'restore-and-scheduling': Object.freeze({
    firstDescribe: 'session teardown contract',
    lastDescribe: 'componentLifecycle — draw scheduling helpers',
    first: 'applySessionData disposes existing tab runtime before replacing the tab list',
    last: 'runDrawWithOverlayPaintGate exposes stale owner-frame recovery',
    expected: 24
  }),
  'cleanup-and-publication': Object.freeze({
    firstDescribe: 'component runtime ownership adapter cleanup',
    lastDescribe: 'componentLifecycle — primary graph publication detection',
    first: 'scatter selection state is owned runtime, not module-level Maps',
    last: 'a standardized terminal plot notice counts as a published result',
    expected: 12
  })
});
const lifecycleShardSpec = LIFECYCLE_SHARD_RANGES[lifecycleShard];
if (!lifecycleShardSpec) {
  throw new Error('componentLifecycleCoreSuite requires a declared shard.');
}
const originalJestTest = global.test;
const originalJestDescribe = global.describe;
let observedTestCount = 0;
let registeredTestCount = 0;
let observedDescribeCount = 0;
let shardStarted = false;
let shardEnded = false;
let describeShardStarted = false;
let describeShardEnded = false;
global.describe = (title, ...args) => {
  const titleText = String(title);
  observedDescribeCount += 1;
  if (titleText === lifecycleShardSpec.firstDescribe) {
    describeShardStarted = true;
  }
  const include = describeShardStarted && !describeShardEnded;
  if (!include) {
    return undefined;
  }
  const result = originalJestDescribe(title, ...args);
  if (titleText === lifecycleShardSpec.lastDescribe) {
    describeShardEnded = true;
  }
  return result;
};
global.test = (title, ...args) => {
  const titleText = String(title);
  observedTestCount += 1;
  if (titleText === lifecycleShardSpec.first) {
    shardStarted = true;
  }
  const include = shardStarted && !shardEnded;
  if (include) {
    registeredTestCount += 1;
    const result = originalJestTest(title, ...args);
    if (titleText === lifecycleShardSpec.last) {
      shardEnded = true;
    }
    return result;
  }
...
global.describe = originalJestDescribe;
if (!describeShardStarted || !describeShardEnded || !shardStarted || !shardEnded || registeredTestCount !== lifecycleShardSpec.expected) {
  throw new Error(`Invalid componentLifecycle core shard ${lifecycleShard}: observed describes ${observedDescribeCount}, observed tests ${observedTestCount}, registered ${registeredTestCount}.`);
}
