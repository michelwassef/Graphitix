...
async function drawPca(){
  ...
  async function drawRoc(){
    ...
    if(series.length>=2 && rocCompareSel && rocCompareSel.value){
      ...
      if(graphType==='roc' && rocDiffMethod==='delong'){
        ...
        rocCompareResult.textContent=`╬öAUC = ${diffRes.diff.toFixed(3)}, p = ${formatP(diffRes.p)}, CI = [${diffRes.ci[0].toFixed(3)}, ${diffRes.ci[1].toFixed(3)}]`;
      }else if(rocDiffMethod==='bootstrap'){
        diffRes=bootstrapCurveDiff(allPairs[i],allPairs[j],graphType);
        const metric=graphType==='roc'? '╬öAUC':'╬öAP';
  ...
  function updateCountLabels(labels){
    ...
    $('#labelABName').textContent=labels.A+'Ôê®'+labels.B;
    $('#labelACName').textContent=labels.A+'Ôê®'+labels.C;
    $('#labelBCName').textContent=labels.B+'Ôê®'+labels.C;
    $('#labelABCName').textContent=labels.A+'Ôê®'+labels.B+'Ôê®'+labels.C;
  }
  function updateRegionSelect(labels){
    const map={
      ...
      B:labels.B+' only',
      C:labels.C+' only',
      AB:labels.A+'Ôê®'+labels.B+' only',
      AC:labels.A+'Ôê®'+labels.C+' only',
      BC:labels.B+'Ôê®'+labels.C+' only',
      ABC:labels.A+'Ôê®'+labels.B+'Ôê®'+labels.C
    };
