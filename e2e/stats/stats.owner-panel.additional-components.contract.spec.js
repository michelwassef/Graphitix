...
for (const componentCase of CASES) {
  test(`${componentCase.key} durable stats surfaces are owner-bound across switching, inactive restore, and archive reopen`, async ({ page }, testInfo) => {
    ...
    await activateTab(page, componentCase.key, tabA);
    const archivePath = await captureArchive(page, componentCase.key, testInfo.outputPath(`${componentCase.key}-owner-panel-additional.graph`));
