...
async function captureWorkspaceArchive(page, componentKey, archivePath) {
...
for (const componentCase of STATS_COMPONENT_CASES) {
  test(`archive reopen keeps ${componentCase.key} statistics after tab switch`, async ({ page }, testInfo) => {
    test.setTimeout(240_000);
    const issues = registerIssueCollectors(page);
    await installLocalCdnOverrides(page);

    await openComponentAndPrepareStats(page, componentCase);
    const archivePath = testInfo.outputPath(`${componentCase.key}-statistics.graph`);
    const { size } = await captureWorkspaceArchive(page, componentCase.key, archivePath);
