...
for (const componentCase of CASES) {
  test(`${componentCase.key} stats stay isolated across same-component tabs and archive restore`, async ({ page }, testInfo) => {
    ...
    const archivePath = await captureArchive(page, componentCase.key, testInfo.outputPath(`${componentCase.key}-same-component-stats.graph`));
