...
async function snapshotPanelLayout(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      yTickLabels: Array.from(svg?.querySelectorAll?.('text[data-font-role="yTick"]') || [])
        .map(node => String(node.textContent || '').trim()),
      densityAreaBottoms: Array.from(svg?.querySelectorAll?.('[data-series-role="density-area"]') || [])
        .map(node => {
          const box = node.getBBox();
          return box.y + box.height;
        }),
      overlayPlotBottom: (() => {
        const axis = Array.from(svg?.querySelectorAll?.('line') || []).find(node => {
          ...
          return Number.isFinite(x1) && Number.isFinite(x2) && Number.isFinite(y1) && Number.isFinite(y2)
            && x2 > x1 && Math.abs(y1 - y2) < 0.001 && (x2 - x1) > 100;
        });
        return axis ? Number(axis.getAttribute('y1')) : null;
      })(),
      panelPlotBottoms: panels.map(panel => (
        Number(panel.getAttribute('data-hist-panel-plot-y'))
        + Number(panel.getAttribute('data-hist-panel-plot-height'))
      )),
...
test('Automatic log-scaled density axes use consistent power-of-ten labels', async ({ page }) => {
  test.setTimeout(90_000);
  await installLocalCdnOverrides(page);
  await page.goto('/index.html', { waitUntil: 'domcontentloaded' });
  await openComponentFromWelcome(page, HISTOGRAM, { first: true });

  for (const display of ['overlay', 'panels']) {
    await loadPanelHistogram(page, {
      display,
      arrangement: 'grid',
      sharedY: true
    }, 'density');
    await page.locator('#histPage:not([hidden]) #histLogY').check();
    await expect.poll(async () => {
      const labels = (await snapshotPanelLayout(page)).yTickLabels;
      return labels.length > 0 && labels.every(label => /^10/.test(label));
    }, { timeout: 30_000 }).toBe(true);
    const geometry = await snapshotPanelLayout(page);
    const plotBottoms = display === 'panels'
      ? geometry.panelPlotBottoms
      : [geometry.overlayPlotBottom];
    expect(geometry.densityAreaBottoms.length).toBeGreaterThan(0);
    expect(geometry.densityAreaBottoms.every(bottom => plotBottoms.some(plotBottom => (
      Number.isFinite(plotBottom) && Math.abs(bottom - plotBottom) < 0.5
    )))).toBe(true);
  }
});
