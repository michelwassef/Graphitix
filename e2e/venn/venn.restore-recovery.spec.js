...
test('venn GO chart renders on tab switch and survives reopen', async ({ page }, testInfo) => {
  ...
  const archivePath = await captureArchive(page, 'venn-go-reopen', testInfo.outputPath('venn-go-reopen.graph'));
...
test('venn button-run GO and STRING survive archive reopen and result-tab clicks', async ({ page }, testInfo) => {
  test.setTimeout(180_000);
  const issues = registerIssueCollectors(page); await installLocalCdnOverrides(page);
  await buildVenn(page);
  await runGoStringFromButtons(page);
  const archivePath = await captureArchive(page, 'venn-button-run-go-string-reopen', testInfo.outputPath('venn-button-run-go-string-reopen.graph'));
  await reopen(page, archivePath);
...
test('venn result-tab clicks heal stale analysis payload from session after restore', async ({ page }, testInfo) => {
  test.setTimeout(180_000);
  const issues = registerIssueCollectors(page); await installLocalCdnOverrides(page);
  await buildVenn(page);
  await runGoStringFromButtons(page, 'DRIFT');
  const archivePath = await captureArchive(page, 'venn-stale-analysis-payload-reopen', testInfo.outputPath('venn-stale-analysis-payload-reopen.graph'));
  await reopen(page, archivePath);
...
test('venn restored GO and STRING survive first graph redraw before result-tab clicks', async ({ page }, testInfo) => {
  test.setTimeout(180_000);
  const issues = registerIssueCollectors(page); await installLocalCdnOverrides(page);
  await buildVenn(page);
  await runGoStringFromButtons(page, 'REDRAW');
  const archivePath = await captureArchive(page, 'venn-go-string-redraw-before-tabs', testInfo.outputPath('venn-go-string-redraw-before-tabs.graph'));
  await reopen(page, archivePath);
...
test('two button-run venn tabs keep GO and STRING after archive reopen and result-tab clicks', async ({ page }, testInfo) => {
  ...
  const archivePath = await captureArchive(page, 'venn-two-button-run-go-string-reopen', testInfo.outputPath('venn-two-button-run-go-string-reopen.graph'));
  await reopen(page, archivePath);
...
test('venn restored GO and STRING survive recovery lifecycle capture', async ({ page }, testInfo) => {
  test.setTimeout(180_000);
  const issues = registerIssueCollectors(page); await installLocalCdnOverrides(page);
  await buildVenn(page);
  await injectAnalysis(page);
  const archivePath = await captureArchive(page, 'venn-go-string-lifecycle-reopen', testInfo.outputPath('venn-go-string-lifecycle-reopen.graph'));
  await reopen(page, archivePath);
...
test('venn overlap-group switching refreshes the gene list after reopen', async ({ page }, testInfo) => {
  test.setTimeout(180_000);
  const issues = registerIssueCollectors(page); await installLocalCdnOverrides(page);
  await buildVenn(page);
  const archivePath = await captureArchive(page, 'venn-region-reopen', testInfo.outputPath('venn-region-reopen.graph'));
  await reopen(page, archivePath);
  const result = await page.evaluate(async () => {
