...
const { waitForComponentOwnerReady, waitForComponentSnapshotReady, waitForObservationWindow } = require('../helpers/contractWaits');
...
test('restored Venn analysis stays authoritative across passive redraw and snapshot settling', async ({ page }) => {
  ...
  await waitForObservationWindow(page, 1_800, {
    label: 'Venn restored-analysis delayed refresh window'
  });
...
test('Venn automatic species detection is cancelled owner-safely on A→B before execution', async ({ page }) => {
  ...
  await waitForObservationWindow(page, 1_800, {
    label: 'Venn species-cancellation window'
  });
