...
test.describe('Heatmap graph-scope font sizing', () => {
  ...
  test('changing Scale font size produces one settled redraw', async ({ page }) => {
    ...
    const observation = await observeStableValue(page,
      async () => (await getHeatmapDrawPerf(page))?.sequence,
      {
        durationMs: 1_000,
        label: 'Heatmap scale-font draw sequence',
        isStable: (initialSequence, currentSequence) => Number(currentSequence) - Number(initialSequence) <= 1
      }
    );
    const later = { sequence: observation.latest };
