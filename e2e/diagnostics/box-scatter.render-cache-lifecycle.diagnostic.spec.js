...
async function saveArchiveToPath(archive, fileStem, testInfo) {
  if (!testInfo || typeof testInfo.outputPath !== 'function') {
    throw new Error('Playwright test output path is required for diagnostic archives');
  }
  const archivePath = testInfo.outputPath(`${fileStem}.graph`);
...
async function runLifecycleScenario(page, testInfo, scenario, issues) {
  ...
  if (scenario.mode === 'reopen') {
    const archivePath = await saveArchiveToPath(archive, label, testInfo);
    await page.reload({ waitUntil: 'domcontentloaded' });
    await expect(page.locator('#welcomeScreen')).toBeVisible({ timeout: 20_000 });
