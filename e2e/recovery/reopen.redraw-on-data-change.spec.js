...
test.describe('Reopen → data edit redraws immediately (all components)', () => {
  ...
  for (const component of COMPONENT_MATRIX) {
    test(`reopen + table edit redraws without resize: ${component.type}`, async ({ page }, testInfo) => {
      ...
      const archivePath = await captureWorkspaceArchive(page, testInfo.outputPath(`reopen-redraw-${component.type}.graph`));
