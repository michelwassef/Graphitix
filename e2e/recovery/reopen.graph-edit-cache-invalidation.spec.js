...
async function captureWorkspaceArchive(page, outputPath, fileStem) {
...
test.describe('Reopened graph edits invalidate restored render caches', () => {
  ...
  test('box font editing uses restored interactions without redrawing', async ({ page }, testInfo) => {
    ...
    const archivePath = await captureWorkspaceArchive(page, testInfo.outputPath('reopen-graph-edit-box.graph'), 'reopen-graph-edit-box');
  ...
  test('heatmap palette editing repaints after archive restore', async ({ page }, testInfo) => {
    test.setTimeout(180_000);
    const issues = registerIssueCollectors(page);
    await installLocalCdnOverrides(page);
    ...
    const archivePath = await captureWorkspaceArchive(page, testInfo.outputPath('reopen-graph-edit-heatmap.graph'), 'reopen-graph-edit-heatmap');
    const tabId = await reopenArchiveAndActivate(page, archivePath, component);
  ...
  test('surface rotation uses restored handlers on the first archive-reopen drag', async ({ page }, testInfo) => {
    test.setTimeout(180_000);
    const issues = registerIssueCollectors(page);
    await installLocalCdnOverrides(page);
    ...
    const archivePath = await captureWorkspaceArchive(page, testInfo.outputPath('reopen-graph-edit-surface.graph'), 'reopen-graph-edit-surface');
    const tabId = await reopenArchiveAndActivate(page, archivePath, component);
  ...
  for(const component of [
    ...
    test(`${component.type} graph editing is rebound before the first restored click`, async ({ page }, testInfo) => {
      test.setTimeout(180_000);
      const issues = registerIssueCollectors(page);
      await installLocalCdnOverrides(page);
      ...
      const archivePath = await captureWorkspaceArchive(page, testInfo.outputPath(`reopen-graph-edit-${component.type}.graph`), `reopen-graph-edit-${component.type}`);
      const tabId = await reopenArchiveAndActivate(page, archivePath, component);
  ...
  for(const component of restoredAxisCases){
    test(`${component.type}${component.chartType ? ` ${component.chartType}` : ''} axes open their toolbar on the first click after archive reopen`, async ({ page }, testInfo) => {
      test.setTimeout(180_000);
      const issues = registerIssueCollectors(page);
      await installLocalCdnOverrides(page);
      await page.goto('/index.html', { waitUntil: 'domcontentloaded' });
      ...
      const archivePath = await captureWorkspaceArchive(page, testInfo.outputPath(`reopen-axis-interaction-${component.type}${component.chartType ? `-${component.chartType}` : ''}.graph`), `reopen-axis-interaction-${component.type}${component.chartType ? `-${component.chartType}` : ''}`);
      const tabId = await reopenArchiveAndActivate(page, archivePath, component);
      await waitForOwnerProjection(page, component, axisSelector, {
        timeout: 30_000,
        requireMountedRoot: true
  ...
  for(const component of restoredInlineEditCases){
    test(`${component.type} graph text enters inline editing immediately after archive reopen`, async ({ page }, testInfo) => {
      test.setTimeout(180_000);
      const issues = registerIssueCollectors(page);
      await installLocalCdnOverrides(page);
      await page.goto('/index.html', { waitUntil: 'domcontentloaded' });
      ...
      const archivePath = await captureWorkspaceArchive(page, testInfo.outputPath(`reopen-inline-edit-${component.type}.graph`), `reopen-inline-edit-${component.type}`);
      const tabId = await reopenArchiveAndActivate(page, archivePath, component);
