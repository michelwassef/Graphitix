...
async function captureWorkspaceArchive(page, fileStem, testInfo) {
  ...
  if (!testInfo || typeof testInfo.outputPath !== 'function') {
    throw new Error('Playwright test output path is required for recovery archives');
  }
  const archivePath = testInfo.outputPath(archive.fileName);
...
test('mixed heavy scatter tabs + heavy box tab survive archive reopen with tab isolation and previews', async ({ page }, testInfo) => {
  ...
  const { archivePath, byteLength, diagnostics: archiveDiagnostics } = await captureWorkspaceArchive(page, 'heavy-mixed-reopen', testInfo);
