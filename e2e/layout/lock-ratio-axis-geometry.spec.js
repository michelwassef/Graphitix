...
const { observeStableValue, waitForAnimationFrame } = require('../helpers/contractWaits');
...
async function dragHandle(page, pageId, selector, dx, dy) {
  ...
  const observation = await observeStableValue(page,
    () => page.evaluate(() => window.__lockRatioReleaseDraws || []),
    { durationMs: 150, label: 'lock-ratio post-release redraws' }
  );
  const releaseDraws = observation.latest;
...
test('Lock ratio is toggle-neutral and preserves primary-axis proportions across default components', async ({ page }, testInfo) => {
  ...
  await openWorkspaceArchive(page, archive, {
    reload: false,
    fileName: 'lock-ratio-axis-geometry.graph',
    filePath: testInfo.outputPath('lock-ratio-axis-geometry.graph')
  });
