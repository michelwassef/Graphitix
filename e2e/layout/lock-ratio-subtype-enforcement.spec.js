...
test.describe('Lock ratio subtype enforcement', () => {
  ...
  for (const componentCase of FORCED_LOCK_CASES) {
    test(`${componentCase.type} forced lock ratio stays isolated across same-component tabs and archive restore`, async ({ page }, testInfo) => {
      ...
      await waitForComponentOwnerReady(page, componentCase.type, {
        expectedTabId: forcedTab,
        requireMountedRoot: true,
        requirePublished: true,
        requireIdle: true,
        timeout: 90_000
      });
      const archivePath = await captureArchive(page, testInfo.outputPath(`lock-ratio-${componentCase.type}.graph`));
