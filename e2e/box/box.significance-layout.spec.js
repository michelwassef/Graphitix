...
async function expectBoxDrawsToSettle(page, maxDelta = 2) {
  ...
  const observation = await observeStableValue(page, () => readBoxDrawToken(page), {
    durationMs: 1_400,
    label: 'Box post-significance draw count',
    isStable: (initial, current) => current - initial <= maxDelta
  });
  const after = observation.latest;
