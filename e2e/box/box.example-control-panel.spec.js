...
test('Box empty graph resize moves the controls with the drawing zone', async ({ page }) => {
  test.setTimeout(60_000);
  const issues = registerIssueCollectors(page);
  await installLocalCdnOverrides(page);
  await page.goto('/index.html', { waitUntil: 'domcontentloaded' });
  ...
  await expect.poll(async () => {
    ...
    return metrics.stackWidth > before.stackWidth + 40
      && metrics.stackWidth >= metrics.svgBoxWidth - 1
      && metrics.controlsLeft >= metrics.stackLeft + metrics.stackWidth - 1;
  }).toBe(true);
