...
const { waitForObservationWindow } = require('../helpers/contractWaits');
...
test('Box live styles avoid exposed full-frame redraws', async ({ page }) => {
  ...
  await page.waitForFunction(() => {
    ...
    return Number(state?.drawToken || 0) === Number(window.__boxTokenBeforeLiveStyle || 0) + 1;
...
test('Box Density samples updates only the violin layer', async ({ page }) => {
  ...
  await expect.poll(async () => page.evaluate(() => {
    ...
    return !!path && path.getAttribute('d') !== window.__boxViolinPathBefore;
...
test('Box worker statistics publishes default pairwise annotations immediately', async ({ page }) => {
  ...
  await waitForObservationWindow(page, 2_000, {
    label: 'Box worker statistics delayed-render window'
  });
