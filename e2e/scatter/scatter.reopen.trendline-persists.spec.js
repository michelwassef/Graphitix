...
async function captureWorkspaceArchive(page, outputPath) {
  return saveWorkspaceArchive(page, outputPath, {
    scope: 'workspace',
...
test('scatter trend line persists after reopening a saved file', async ({ page }, testInfo) => {
  ...
  const archivePath = await captureWorkspaceArchive(page, testInfo.outputPath('scatter-reopen-trend.graph'));
