...
test('scatter live-update works after stats computation: trendline toggle and resize both trigger a redraw', async ({ page }) => {
  ...
  const statsObservation = await observeStableValue(page, async () => ({
    draws: await getDrawCount(page),
    columns: await getGridColumnCount(page)
  }), { durationMs: 800, label: 'Scatter statistics no-redraw contract' });

  const drawsAfterStats = statsObservation.latest.draws;
  expect(
    drawsAfterStats - drawsBeforeStats,
    'Computing statistics with the trend line OFF must not redraw the scatter graph'
  ).toBe(0);
  const colsAfterStats = statsObservation.latest.columns;
  expect(
    colsAfterStats,
  ...
  const trendlineObservation = await observeStableValue(page,
    () => getDrawCount(page),
    { durationMs: 800, label: 'Scatter trendline redraw count' }
  );

  const drawsAfterTrendline = trendlineObservation.latest;
  expect(
