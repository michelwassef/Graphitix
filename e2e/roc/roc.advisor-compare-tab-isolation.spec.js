...
async function captureArchive(page, stem, outputPath) {
  ...
  if (!outputPath) {
    throw new Error('ROC advisor archive capture requires a per-test output path.');
  }
...
test('ROC advisor and exact comparison controls stay isolated across same-type tabs and reopen', async ({ page }, testInfo) => {
  ...
  const archivePath = await captureArchive(
    page,
    'roc-advisor-compare-tabs',
    testInfo.outputPath('roc-advisor-compare-tabs.graph')
  );
