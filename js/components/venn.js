(function(global){
  ...
  function ensureInputs(){
    if(!state.inputs){
      throw new Error('Venn inputs not initialised');
    }
    return state.inputs;
  }
  ...
  function renderGOResults(limit=5){
    ...
    const link=`<div><a href="${fullUrl}" target="_blank" rel="noopener">View full GO analysis</a>${state.lastGOResult.length>5?` | <button class="btn" id="toggleGoResults" data-state="${limit===5?'top5':'all'}">${limit===5?'Show all results':'Show top 5'}</button>`:''}</div>`;
  ...
  function drawFromNumeric(){
    ...
    state.lastRegions={A:new Set(),B:new Set(),C:new Set(),Aonly:new Set(),Bonly:new Set(),Conly:new Set(),AB:new Set(),AC:new Set(),BC:new Set(),ABC:new Set()};
