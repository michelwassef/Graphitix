(function(global){
  ...
  if(typeof svgGeometry.buildCompoundLinePath !== 'function' && typeof require === 'function'){
    try{
      ...
      console.debug('Debug: roc component svgGeometry helper require failed', { message: err?.message || String(err) });
    }
  }
  ...
  const normalizeRocQueuedDrawOptions = (options, owner = {}) => (
    Shared.componentLifecycle?.sanitizeOptionalComponentDrawOptions?.('roc', options, owner) || null
  );

  function mergeRocDrawOptions(previousOptions, nextOptions){
    const previous = normalizeRocQueuedDrawOptions(previousOptions);
    const next = normalizeRocQueuedDrawOptions(nextOptions);
    ...
    }else if(Object.prototype.hasOwnProperty.call(next, 'viewOnly')){
      merged.viewOnly = next.viewOnly === true && previous.viewOnly === false ? false : next.viewOnly === true;
    }else{
      merged.viewOnly = false;
    }
  ...
  function isRocDrawRuntimeSnapshotIdle(runtime){
    const source = runtime && typeof runtime === 'object' ? runtime : {};
    // Deferred work belongs to an inactive owner and is not executing. It is safe to
    // persist canonical state while that replay remains queued; cache currentness is
    // deliberately stricter and rejects a frame until the deferred redraw publishes.
    return source.scheduled !== true && source.inProgress !== true;
  }

  function isRocDrawRuntimeCacheCurrent(runtime){
    const source = runtime && typeof runtime === 'object' ? runtime : {};
    return isRocDrawRuntimeSnapshotIdle(source) && !source.deferredOptions;
  }
  ...
  function invalidateRocRenderCacheForTab(tabLike, reason){
    try{
      ...
      if(!tab || tab.type !== 'roc'){ return false; }
      let cleared = false;
      if(typeof sessionApi?.clearTabRenderCache === 'function'){
        cleared = sessionApi.clearTabRenderCache(tab, { tabId, reason: reason || 'roc-payload-change' }) || cleared;
      }
      if(typeof sessionApi?.clearTabArchiveRenderCache === 'function'){
        cleared = sessionApi.clearTabArchiveRenderCache(tab, { tabId, reason: reason || 'roc-payload-change' }) || cleared;
      }
      return cleared;
    }catch(err){
      console.debug('Debug: roc render cache invalidation failed', { reason: reason || null, message: err?.message || String(err) });
      return false;
    }
  }
  ...
  function createDefaultRocDurableState(source = {}){
    ...
    return {
      ...
      labelPositions: normalizeRocLabelPositions(src.labelPositions),
  ...
  function formatRocCompareResultText(graphType, diffMethod, diffResult){
    ...
    if(graphType === 'roc' && diffMethod === 'delong'){
      return `${metric} = ${diff.toFixed(3)}; 95% CI [${Number(ci?.[0]).toFixed(3)}, ${Number(ci?.[1]).toFixed(3)}]; ${formatRocPExpression(p)}`;
    }
    if(diffMethod === 'bootstrap'){
      return `${metric} = ${diff.toFixed(3)}; 95% CI [${Number(ci?.[0]).toFixed(3)}, ${Number(ci?.[1]).toFixed(3)}]; ${formatRocPExpression(p)}`;
    }
    if(diffMethod === 'permutation'){
      return `${metric} = ${diff.toFixed(3)}; ${formatRocPExpression(p)}`;
    }
    return '';
  }
  ...
  function createDefaultRocRefs(root = null){
    return {
      ...
      fontSizeVal: null,
      showLegend: null,
      showComparisonOnPlot: null,
      showComparisonOnPlotLabel: null,
      graphType: null,
      positiveClass: null,
      negativeClass: null,
      scoreDirection: null,
  ...
  function createDefaultRocDrawRuntime(source = {}){
    ...
    return {
      ...
      requestOptions: normalizeRocQueuedDrawOptions(src.requestOptions || src.scheduledOptions),
      deferredOptions: normalizeRocQueuedDrawOptions(src.deferredOptions),
      updatedAt: Number.isFinite(Number(src.updatedAt)) ? Number(src.updatedAt) : Date.now()
    };
  }

  function getRocDrawRuntime(session = null){
    const shaped = ensureRocSessionOwnershipShape(session || getActiveRocSessionForState());
    return shaped?.timers?.drawRuntime || createDefaultRocDrawRuntime();
  }

  function updateRocDrawRuntime(session = null, mutator = null){
    const shaped = ensureRocSessionOwnershipShape(session || getActiveRocSessionForState());
    if(!shaped){
      return createDefaultRocDrawRuntime();
    }
    const runtime = createDefaultRocDrawRuntime(shaped.timers?.drawRuntime || {});
  ...
  function cleanupRocFramePublication(session = null){
    const shaped = ensureRocSessionOwnershipShape(session);
    const publication = shaped?.timers?.framePublication || null;
    if(!publication){ return false; }
    shaped.timers.framePublication = null;
    publication.cleanup?.();
    return true;
  }

  function deactivateRocDrawOwner(tabLike = null, reason = 'roc-tab-deactivated'){
    const tabId = normalizeRocSessionTabId(tabLike, { tabId: typeof tabLike === 'string' ? tabLike : tabLike?.id || null });
    const session = tabId ? getRocSession(tabId, { tabId, reason }, { create: false }) : null;
    if(!session){ return false; }
    const current = getRocDrawRuntime(session);
    const deferredOptions = mergeRocDrawOptions(current.deferredOptions, current.requestOptions);
    cleanupRocFramePublication(session);
    updateRocDrawRuntime(session, runtime => {
      runtime.generation = Number(runtime.generation || 0) + 1;
      runtime.scheduled = false;
      runtime.inProgress = false;
      runtime.requestOptions = null;
      runtime.deferredOptions = deferredOptions;
      runtime.lastStatus = deferredOptions ? 'deferred' : 'idle';
  ...
  function createRocSession({ tabId, root = null, initialState = null } = {}){
    ...
    return {
      ...
      timers: {
        scheduleDraw: null,
        drawRuntime: createDefaultRocDrawRuntime(),
        framePublication: null
      },
      workers: new Map(),
      managers: {
        hot: null,
  ...
  function getRocSession(tabLike = null, meta = {}, options = {}){
    ...
    if(!session && options.create !== false){
      const projectedTabId = String(getRocProjectionTabId() || '').trim();
      session = createRocSession({
        tabId,
        root: projectedTabId === tabId ? (normalizeRocRefValue(refs.root) || resolveRocRoot(tabId) || null) : null
      });
  ...
  function runRocControlOwner(event, reason, callback){
    ...
    if(session?.tabId && !isRocSessionActive(session)){
  ...
  function syncRocSessionRefsFromActive(session = null){
    ...
    const snapshot = createRocRefsSnapshot({
      ...shaped.refs,
      ...refs,
      root: normalizeRocRefValue(refs.root) || shaped.root || state.root || null,
      notesControl: canUseRocNotesControl(notesState.control) ? notesState.control : null,
      legendControl: rocLegendControl || shaped.refs?.legendControl || null
    });
    setRocSessionRefs(shaped, snapshot, { applyActive: false });
    return shaped;
  }

  function refreshRocActiveDomRefsForSession(session = null, meta = {}) {
    const shaped = ensureRocSessionOwnershipShape(session || getActiveRocSessionForState());
    if(!shaped || (shaped.tabId && !isRocSessionActive(shaped))){
      return shaped;
    }
    ...
    const snapshot = createRocRefsSnapshot({
      ...shaped.refs,
      ...refs,
      root: normalizeRocRefValue(refs.root) || state.root || shaped.root || null
    });
    setRocSessionRefs(shaped, snapshot, { applyActive: false });
  ...
  function isRocSessionActive(session = null){
    ...
    return Shared.componentLifecycle?.canOwnerUseLiveProjection?.('roc', session, {
      component: roc,
      projectedSession: projectedRocSession,
      session,
  ...
  function isRocSessionActivationTarget(session = null){
    return Shared.componentLifecycle?.isOwnerActivationTarget?.('roc', session, { component: roc }) === true;
  }

  function settleRocScheduledDrawOwner(tabLike = null, drawGeneration = null, reason = 'roc-scheduled-draw-settled'){
    const tabId = normalizeRocSessionTabId(tabLike, { tabId: typeof tabLike === 'string' ? tabLike : tabLike?.id || null });
    const session = tabId
      ? getRocSession(tabId, { tabId, reason }, { create: false })
      : null;
    if(!session){
      return false;
    }
    const expectedGeneration = Number(drawGeneration);
    const runtime = getRocDrawRuntime(session);
    const currentGeneration = Number(runtime.generation || 0);
    if(Number.isFinite(expectedGeneration) && expectedGeneration > 0 && currentGeneration > 0 && expectedGeneration !== currentGeneration){
      return false;
    }
    updateRocDrawRuntime(session, nextRuntime => {
      nextRuntime.scheduled = false;
      nextRuntime.inProgress = false;
      nextRuntime.requestOptions = null;
      nextRuntime.lastStatus = nextRuntime.deferredOptions ? 'deferred' : 'idle';
      nextRuntime.lastReason = reason || nextRuntime.lastReason || null;
    });
    console.debug('Debug: roc scheduled draw owner settled', {
      tabId: session.tabId || null,
      drawGeneration: currentGeneration || null,
      reason
    });
    return true;
  }

  function deferRocScheduledDrawOwner(tabLike = null, options = {}, reason = 'roc-scheduled-draw-deferred'){
    const tabId = normalizeRocSessionTabId(tabLike, { tabId: typeof tabLike === 'string' ? tabLike : tabLike?.id || null });
    const session = tabId
      ? getRocSession(tabId, { tabId, reason }, { create: false })
      : null;
    if(!session){ return false; }
    const currentRuntime = getRocDrawRuntime(session);
    const requested = normalizeRocQueuedDrawOptions(options) || currentRuntime?.requestOptions || null;
    const requestedGeneration = Number(requested?.drawGeneration);
    const currentGeneration = Number(currentRuntime?.generation || 0);
    if(Number.isFinite(requestedGeneration) && requestedGeneration > 0
      && currentGeneration > 0 && requestedGeneration !== currentGeneration){
      return false;
    }
    updateRocDrawRuntime(session, runtime => {
      runtime.deferredOptions = mergeRocDrawOptions(runtime.deferredOptions, requested);
      runtime.scheduled = false;
      runtime.inProgress = false;
      runtime.requestOptions = null;
      runtime.lastStatus = runtime.deferredOptions ? 'deferred' : 'idle';
      runtime.lastReason = reason || runtime.lastReason || null;
    });
    return true;
  }

  function consumeRocDeferredDraw(session = null){
    const shaped = ensureRocSessionOwnershipShape(session || getActiveRocSessionForState());
    if(!shaped){ return null; }
    let deferred = null;
    updateRocDrawRuntime(shaped, runtime => {
      deferred = normalizeRocQueuedDrawOptions(runtime.deferredOptions);
      runtime.deferredOptions = null;
      if(!runtime.scheduled && !runtime.inProgress){
  ...
  function settleRocScheduledDrawFromOptions(options = {}, reason = 'roc-scheduled-draw-settled', meta = {}){
    const source = options && typeof options === 'object' ? options : {};
    return settleRocScheduledDrawOwner(
      source.tabId || source.workspaceTabId || source.tab?.id || source.__workspaceSessionMeta?.tabId || meta?.tabId || null,
      source.drawGeneration,
      reason
    );
  }
  ...
  function scheduleRocDrawForSession(session = null, options = {}){
    ...
    const {
      __workspaceSessionMeta: _staleWorkspaceSessionMeta,
      sessionGeneration: _staleSessionGeneration,
      drawGeneration: _staleDrawGeneration,
      ...semanticOptions
    } = sourceOptions;
    const scheduleOptions = {
      ...semanticOptions,
      tabId: shaped.tabId || undefined,
      reason: semanticOptions.reason || 'roc-session-draw'
    };
    const lifecycleMeta = {
      ...scheduleOptions,
    ...
    if(!isRocSessionActive(shaped)){
      updateRocDrawRuntime(shaped, runtime => {
        runtime.deferredOptions = mergeRocDrawOptions(runtime.deferredOptions, scheduleOptions);
        runtime.lastStatus = 'deferred';
        runtime.lastReason = scheduleOptions.reason || runtime.lastReason || 'inactive-owner';
      });
      console.debug('Debug: roc draw schedule deferred for inactive owner', {
        tabId: shaped.tabId || null,
    ...
    shaped.updatedAt = Date.now();
    const scheduled = scheduler(sanitizeRocDrawOptions(scheduleOptions));
  ...
  function patchRocLabelPosition(session = null, key, value, meta = {}){
    const owner = ensureRocSessionOwnershipShape(session || getActiveRocSessionForState());
    const currentPositions = normalizeRocLabelPositions(owner?.state?.labelPositions || state.labelPositions);
    const nextPositions = normalizeRocLabelPositions({ ...currentPositions, [key]: value || null });
    return patchRocVisualState(owner, { labelPositions: nextPositions }, meta);
  }
  ...
  function syncRocSessionManagersFromActive(session = null){
    const shaped = ensureRocSessionOwnershipShape(session || projectedRocSession || getActiveRocSessionForState());
    if(!shaped){ return null; }
    const sessionIsActive = !shaped.tabId || isRocSessionActive(shaped);
  ...
  function captureRocSessionStateFromActive(session = null, meta = {}){
    ...
    const storedStatsPanelModel = selectRocStatsPanelModel(
      shaped.results?.statsPanelModel,
      shaped.state?.statsPanelModel,
      state.statsPanelModel
    );
    const statsPanelModel = meta.captureStatsPanel === true
      ? captureRocStatsPanelModel(storedStatsPanelModel, { session: shaped })
      : createDefaultRocStatsPanelModel(storedStatsPanelModel);
  ...
  function applyRocSessionStateToActive(session = null, options = {}){
    ...
    state.labelPositions = normalizeRocLabelPositions(shaped.state.labelPositions);
    ...
    if(rocAutoDrawManager && options.syncUi !== false){
      ...
      // Session application is a projection step. Persisted AutoDraw state is
      // authoritative during activation/reopen and must not be recomputed from
      // whichever table DOM happens to be mounted at that moment.
  ...
  function resolveRocOwnedRoot(tabLike = null, options = {}){
    const tabId = normalizeRocSessionTabId(tabLike || options.tabId || null, { tabId: options.tabId || null });
    if(tabId){
      const mountedRoot = normalizeRocRefValue(Shared.workspaceTabs?.getMountedRoot?.(tabId, 'roc') || null);
      if(mountedRoot){
        return mountedRoot;
      }
      const session = rocSessionsByTabId.get(tabId) || null;
      const sessionRoot = normalizeRocRefValue(session?.refs?.root || session?.root || null);
      if(sessionRoot){
        const rootTabId = String(getRocTabIdFromTarget(sessionRoot) || '').trim();
        if(rootTabId === tabId || (!rootTabId && String(getRocProjectionTabId() || '') === tabId)){
          return sessionRoot;
        }
      }
      return null;
    }
    if(options.allowProjected === false){
      return null;
    }
    return normalizeRocRefValue(refs.root || state.root || null) || resolveRocRoot(null) || null;
  }

  function getRocOwnedNodeById(id, tabLike = null, options = {}){
    if(!id){
      return null;
    }
    const root = resolveRocOwnedRoot(tabLike, options);
  ...
  function resolveRocStatsRefsForSession(session = null, options = {}){
    const shaped = ensureRocSessionOwnershipShape(session || getActiveRocSessionForState());
    const baseRefs = resolveRocRefsContext(shaped, { ...options, allowFallback: !shaped });
    if(!shaped){
      return baseRefs;
    }
    const ownerRoot = normalizeRocRefValue(options.root)
      || resolveRocOwnedRoot(shaped.tabId || null, { allowProjected: isRocSessionActive(shaped) })
      || null;
    const statsResults = ownerRoot?.querySelector?.('#rocStatsResults') || null;
    const statsControls = ownerRoot?.querySelector?.('#rocStatsControls') || null;
    const statsAdvisor = ownerRoot?.querySelector?.('#rocStatsAdvisor') || null;
    const snapshot = createRocRefsSnapshot({
      ...baseRefs,
      root: ownerRoot,
      statsResults,
    ...
    // Resolving an owner's stats surface must never mutate the projected refs.
    // Keep the owner session's snapshot current, but leave active projection
    // replacement to the normal activation/DOM-binding path.
    shaped.refs = snapshot;
    shaped.root = ownerRoot || shaped.root || null;
    shaped.updatedAt = Date.now();
    return snapshot;
  }
  ...
  function persistRocTabState(reason, session = null){
    const ownerSession = ensureRocSessionOwnershipShape(session || getActiveRocSessionForState());
    const tabId = String(ownerSession?.tabId || getRocProjectionTabId() || '').trim();
    if(!tabId){
      return false;
    }
    return Shared.componentLifecycle?.persistOwnedUserState?.(
      ...
      { reason: reason || 'roc-user-state-change' }
    ) === true;
  }
  ...
  function normalizeRocRefValue(value){
    if(!value){
      return null;
    }
  ...
  function createRocRefsSnapshot(source = {}){
    const defaults = createDefaultRocRefs();
    const src = source && typeof source === 'object' ? source : {};
    Object.keys(defaults).forEach(key => {
      defaults[key] = normalizeRocRefValue(src[key]) || null;
    });
    return defaults;
  }

  function replaceActiveRocRefs(source = {}){
    const snapshot = createRocRefsSnapshot(source);
  ...
  function getRocSessionRefs(session = null, options = {}){
    const shaped = ensureRocSessionOwnershipShape(session);
    if(shaped){
      return createRocRefsSnapshot(shaped.refs);
    }
    return options.allowFallback === false ? null : createRocRefsSnapshot(refs);
  }

  function resolveRocRefsContext(session = null, options = {}){
    if(options.refs && typeof options.refs === 'object'){
      return createRocRefsSnapshot(options.refs);
    }
    return getRocSessionRefs(session || getActiveRocSessionForState(), options) || createRocRefsSnapshot({});
  }

  function setRocSessionRefs(session = null, source = {}, options = {}){
    const shaped = ensureRocSessionOwnershipShape(session);
    const snapshot = createRocRefsSnapshot(source);
    if(shaped){
      shaped.refs = snapshot;
    ...
    if(!shaped || isRocSessionActive(shaped) || options.applyActive === true){
      replaceActiveRocRefs(snapshot);
    }
    return snapshot;
  }

  function createDefaultRocRuntimeControls(){
    return { graphType: 'roc', showGrid: false, showFrame: true, showLegend: true, showComparisonOnPlot: false, fontSize: '12' };
  }
  ...
  function normalizeRocRuntimeControls(source = {}){
    ...
    return {
      ...
      showLegend: Object.prototype.hasOwnProperty.call(src, 'showLegend') ? src.showLegend !== false : defaults.showLegend !== false,
      showComparisonOnPlot: Object.prototype.hasOwnProperty.call(src, 'showComparisonOnPlot') ? !!src.showComparisonOnPlot : !!defaults.showComparisonOnPlot,
  ...
  function buildRocSeriesAnalysis(serie, graphType, pMethod = state.singleRocPMethod){
    const pairs = Array.isArray(serie?.pairs) ? serie.pairs : [];
    const rankedCurve = buildRankedCurve(pairs, graphType);
    return {
      serie,
      pairs,
      rankedCurve,
      aucUncertainty: String(graphType || 'roc').toLowerCase() === 'roc'
  ...
  function formatRocLegendMetricLabel(analysis, graphType){
    const name = String(analysis?.serie?.name || 'Series');
    const rankedCurve = analysis?.rankedCurve || { metric: NaN };
    if(String(graphType || 'roc').toLowerCase() === 'pr'){
      return `${name} — AP ${formatRocDecimal(rankedCurve.metric, 3)}`;
    }
    return `${name} (AUC=${formatRocDecimal(rankedCurve.metric, 2)})`;
  }

  function readRocGraphGeometryValue(value){
    const numeric = Number.parseFloat(value);
    return Number.isFinite(numeric) && numeric > 0 ? numeric : null;
  }

  function applyRocDefaultXAxisGutter(margin, naturalPlotWidth, naturalPlotHeight, svgBox = null){
    const resolved = {
    ...
    const defaultWidth = readRocGraphGeometryValue(dataset.resizerDefaultWidth);
    const defaultHeight = readRocGraphGeometryValue(dataset.resizerDefaultHeight);
    const currentWidth = readRocGraphGeometryValue(dataset.resizerBaseWidth)
      ?? readRocGraphGeometryValue(dataset.graphWidthPx)
      ?? readRocGraphGeometryValue(dataset.svgWidth);
    const currentHeight = readRocGraphGeometryValue(dataset.resizerBaseHeight)
      ?? readRocGraphGeometryValue(dataset.graphHeightPx)
      ?? readRocGraphGeometryValue(dataset.svgHeight);
    const width = Number(naturalPlotWidth);
    const height = Number(naturalPlotHeight);
    ...
    // Make only the canonical ROC plot square. Express that default as a fixed
    // right-side layout gutter so the ordinary renderer/resizer remains the
    // sole geometry path afterwards: every horizontal resize changes the X span
    // by the same delta, with no first-drag switch between layout modes.
    const defaultNaturalWidth = width - (currentWidth - defaultWidth);
    const defaultNaturalHeight = height - (currentHeight - defaultHeight);
    const gutter = defaultNaturalWidth - defaultNaturalHeight;
    if(!Number.isFinite(gutter) || gutter <= 0){
      return resolved;
    }

    resolved.right += gutter;
    return resolved;
  }

  function buildRocLegendMetricLabel(serie, graphType, pMethod = state.singleRocPMethod){
    return formatRocLegendMetricLabel(buildRocSeriesAnalysis(serie, graphType, pMethod), graphType);
  }
  ...
  function applyRocClassificationSetting(key, value, session, reason){
    ...
    persistRocTabState(reason, session);
    scheduleRocControlDraw(reason, { session, force: true });
    return setup;
  }

  function ensureRocStatsReportHost(options = {}){
    const reporting = Shared.statsReporting;
    const rocRefs = resolveRocStatsRefsForSession(options.session || null, options);
  ...
  function clearRocStatsReportHost(options = {}){
    const reporting = Shared.statsReporting;
    const rocRefs = resolveRocStatsRefsForSession(options.session || null, options);
    if(reporting && typeof reporting.clearReportHost === 'function'){
      reporting.clearReportHost(rocRefs.statsResults || null);
    }
  }

  function pinRocStatsReportAfterMetrics(target = null, options = {}){
    const rocRefs = target ? null : resolveRocStatsRefsForSession(options.session || null, options);
    const root = target || rocRefs?.statsResults || null;
  ...
  function scheduleRocStatsReportOrderPin(options = {}){
    ...
    const ownerTabId = session?.tabId || normalizeRocSessionTabId(options.tabId || null, { tabId: options.tabId || null }) || null;
    const run = () => pinRocStatsReportAfterMetrics(statsResults, { ...options, session, refs: rocRefs });
    run();
  ...
  function captureRocStatsPanelModel(fallback = null, options = {}){
    const session = ensureRocSessionOwnershipShape(options.session || getActiveRocSessionForState());
    const rocRefs = resolveRocStatsRefsForSession(session, options);
    const previous = selectRocStatsPanelModel(
      fallback,
      session?.results?.statsPanelModel,
      session?.state?.statsPanelModel,
      session && isRocSessionActive(session) ? state.statsPanelModel : null
    );
    let captured = null;
    if(rocRefs.statsResults && Shared.statsReporting && typeof Shared.statsReporting.capturePanelModel === 'function'){
      try{
        captured = Shared.statsReporting.capturePanelModel(rocRefs.statsResults);
      }catch(err){
        console.debug('Debug: roc stats panel capture failed', {
          tabId: session?.tabId || null,
          err: err?.message || String(err)
        });
      }
    }
    const normalizedCaptured = normalizeRocStatsPanelModel(captured);
    const normalized = rocStatsPanelModelHasContent(normalizedCaptured)
      ? normalizedCaptured
      : previous;
    if(session){
      ...
      session.results = createDefaultRocResultsState({
        statsPanelModel: session.state.statsPanelModel,
        compareSelection: session.results?.compareSelection || session.state.compareSelection || state.compareSelection || state.compareSel?.value || null,
        diffMethod: session.results?.diffMethod || session.state.diffMethod || state.diffMethod,
        compareResult: session.results?.compareResult || session.state.compareResult || state.compareResultModel
      });
      session.updatedAt = Date.now();
      if(isRocSessionActive(session)){
  ...
  function rocStatsPanelModelHasContent(model){
    ...
    // Shared capturePanelModel() always returns an object, including for an
    // empty/transient panel. The durable ROC contract only needs to reject
    // that empty { resultsModel:null, reportModel:null } capture. Do not
    // reinterpret the nested stats model here: ROC may persist valid shared
    // or fallback node shapes that are not limited to Line's table/card forms.
    return !!(normalized.resultsModel || normalized.reportModel);
  }

  function selectRocStatsPanelModel(...sources){
    let fallback = null;
    for(const source of sources){
      if(!source || typeof source !== 'object'){
        continue;
      }
      const normalized = normalizeRocStatsPanelModel(source);
      fallback = fallback || normalized;
      if(rocStatsPanelModelHasContent(normalized)){
        return normalized;
      }
    }
    return fallback || createDefaultRocStatsPanelModel();
  }

  function rocStatsPanelHasRenderedResults(options = {}){
    const session = ensureRocSessionOwnershipShape(options.session || getActiveRocSessionForState());
    const rocRefs = resolveRocStatsRefsForSession(session, options);
    const statsResults = rocRefs.statsResults || null;
  ...
  function commitRocStatsPValueFormat(scientific, tabId = null){
    ...
    const rocRefs = resolveRocStatsRefsForSession(session, { tabId: normalizedTabId || session.tabId || null });
    let captured = null;
    if(rocRefs.statsResults && Shared.statsReporting?.capturePanelModel){
      try{
        captured = Shared.statsReporting.capturePanelModel(rocRefs.statsResults);
      }catch(err){
        console.debug('Debug: roc stats p-value format capture failed', {
          tabId: session.tabId || null,
          err: err?.message || String(err)
        });
      }
    }
    const sourceModel = selectRocStatsPanelModel(
      captured,
      session.results?.statsPanelModel,
      session.state?.statsPanelModel,
      isRocSessionActive(session) ? state.statsPanelModel : null
    );
  ...
  function restoreRocStatsPanelModel(model, options = {}){
    const normalized = normalizeRocStatsPanelModel(model);
    const session = ensureRocSessionOwnershipShape(options.session || getActiveRocSessionForState());
    const rocRefs = resolveRocStatsRefsForSession(session, options);
    const statsResults = rocRefs.statsResults || null;
    if(!statsResults || !rocStatsPanelModelHasContent(normalized) || !Shared.statsReporting || typeof Shared.statsReporting.restorePanelModel !== 'function'){
      return false;
    }
    let restoreOutcome = null;
    try{
      restoreOutcome = Shared.statsReporting.restorePanelModel(statsResults, normalized, {
        ensureReportHost: () => ensureRocStatsReportHost({ ...options, session, refs: rocRefs }),
        clearMainWhenMissing: false
      });
      ensureRocStatsReportHost({ ...options, session, refs: rocRefs });
      if(typeof Shared.statsReporting.pinReportHostLast === 'function'){
        Shared.statsReporting.pinReportHostLast(statsResults);
      }
      scheduleRocStatsReportOrderPin({ ...options, session, refs: rocRefs });
    }catch(err){
      console.debug('Debug: roc stats panel restore failed', {
        tabId: session?.tabId || null,
        err: err?.message || String(err)
      });
      return false;
    ...
    if(session){
      ...
      session.results = createDefaultRocResultsState({
        ...
        compareSelection: session.results?.compareSelection || session.state.compareSelection || null,
        diffMethod: session.results?.diffMethod || session.state.diffMethod || 'delong',
        compareResult: session.results?.compareResult || session.state.compareResult || null
      });
      session.updatedAt = Date.now();
      if(isRocSessionActive(session)){
        state.statsPanelModel = createDefaultRocStatsPanelModel(normalized);
    ...
    const restoredBySharedModel = restoreOutcome?.restoredMain === true || restoreOutcome?.restoredReport === true;
    return restoredBySharedModel || rocStatsPanelHasRenderedResults({ ...options, session, refs: rocRefs });
  }
  ...
  function persistRocDerivedResultsToOwnerPayload(session = null, reason = 'roc-draw-results-published'){
    const shaped = ensureRocSessionOwnershipShape(session);
    const tabId = String(shaped?.tabId || '').trim();
    const sessionApi = global.Main?.session || null;
    const tabs = Array.isArray(sessionApi?.workspaceState?.tabs) ? sessionApi.workspaceState.tabs : [];
    const tab = tabId ? (tabs.find(item => item && String(item.id || '').trim() === tabId) || null) : null;
    if(!shaped || !tab || tab.type !== 'roc' || typeof sessionApi?.updateTabPayload !== 'function'){
      return false;
    }
    const panelModel = selectRocStatsPanelModel(
      shaped.results?.statsPanelModel,
      shaped.state?.statsPanelModel
    );
    const statsPatch = {
      diffMethod: shaped.results?.diffMethod || shaped.state?.diffMethod || 'delong',
      singleRocPMethod: normalizeSingleRocPMethod(shaped.state?.singleRocPMethod),
      resamplingSeed: normalizeRocResamplingSeed(shaped.state?.resamplingSeed, ROC_RESAMPLING_DEFAULT_SEED),
      resamplingIterations: normalizeRocResamplingIterations(shaped.state?.resamplingIterations, ROC_RESAMPLING_DEFAULT_ITERATIONS),
      compareSelection: shaped.results?.compareSelection || shaped.state?.compareSelection || null,
      compareResult: normalizeRocCompareResultModel(shaped.results?.compareResult || shaped.state?.compareResult || null),
      analysisSignature: shaped.state?.analysisSignature || '',
      statsPanelSignature: shaped.state?.statsPanelSignature || '',
      advisor: createDefaultRocAdvisorState(getRocAdvisorState(shaped)),
      resultsModel: panelModel.resultsModel || null,
      reportModel: panelModel.reportModel || null
    };
    return sessionApi.updateTabPayload(tab, draft => {
      const next = draft && typeof draft === 'object' ? draft : { type: 'roc' };
      next.type = 'roc';
      next.stats = {
        ...(next.stats && typeof next.stats === 'object' ? next.stats : {}),
        ...cloneSimple(statsPatch)
      };
      return next;
    }, {
      reason: reason || 'roc-draw-results-published',
      origin: 'system',
  ...
  function ensureAxisSettings(session = null){
    ...
    if(!target.x || typeof target.x !== 'object'){
      target.x = { tickInterval: null, majorTickLength: null, labelAngle: null, minorTickSubdivisions: DEFAULT_MINOR_TICK_SUBDIVISIONS };
    }
    ...
    target.x.labelAngle = chartStyle.normalizeOptionalXAxisLabelAngle(target.x.labelAngle);
  ...
  function getXAxisTickLabelAngle(ownerSession = null){
    const owner = ownerSession || getActiveRocSessionForState();
    if(owner && !isRocSessionActive(owner)){
  ...
  function updateXAxisTickLabelAngle(value, ownerSession = null){
    const owner = ownerSession || getActiveRocSessionForState();
    if(owner && !isRocSessionActive(owner)){
      console.debug('Debug: roc x tick label angle ignored for inactive owner', { tabId: owner.tabId || null });
      return;
    }
    const settings = ensureAxisSettings();
    ...
    console.debug('Debug: roc x tick label angle updated',{ angle: nextValue, tabId: owner?.tabId || null });
    scheduleRocViewRefresh('roc-axis-x-label-angle', { tabId: owner?.tabId || null, userInitiated: true });
  }
  ...
  function buildRocAxisControlConfig(axis, ownerSession = null, axisMeta = {}){
    const owner = ensureRocSessionOwnershipShape(ownerSession || getActiveRocSessionForState());
    return {
      axis,
      scopeId: 'roc',
  ...
  function ensureElements(){
    ...
    refs.statsAdvisor = getRocNodeById('rocStatsAdvisor');
    ...
    refs.showComparisonOnPlot = getRocNodeById('rocShowComparisonOnPlot');
    refs.showComparisonOnPlotLabel = getRocNodeById('rocShowComparisonOnPlotLabel');
    if(refs.showLegend){
  ...
  function ensureRocDataViewsForHot(hotInstance, options = {}){
    ...
    const manager = Shared.componentLifecycle?.ensureOwnedDataViewsManager?.({
      ...
      createOptions: {
        ...
        onInteraction(){
          if(isRocSessionActive(getRocSessionForHot(hotInstance, { reason: 'roc-dataview-interaction' }, { create: false }))){
  ...
  function syncRocActiveDataViewFromHot(hotInstance, reason){
    ...
    if(ownerSession && !isRocSessionActive(ownerSession)){
  ...
  function clearPlotArea(reason, options = {}){
    const session = ensureRocSessionOwnershipShape(options.session || getActiveRocSessionForState());
    const rocRefs = resolveRocStatsRefsForSession(session, options);
    const plotDiv = rocRefs.plotDiv || null;
    const statsResults = rocRefs.statsResults || null;
    ...
    if(plotDiv){
      if(typeof Shared.renderPlotNotice === 'function'){
        Shared.renderPlotNotice(plotDiv, noticeMessage, { resetAspect: true, show: true });
      }else{
        ...
        plotDiv.appendChild(notice);
      }
    }
    if(statsResults){
      clearRocStatsReportHost({ ...options, session, refs: rocRefs });
  ...
  // PART: ADVISOR
  function buildRocAdvisorContext(raw, options = {}){
    const session = ensureRocSessionOwnershipShape(options.session || getActiveRocSessionForState());
    const rocRefs = resolveRocRefsContext(session, options);
    const graphType=(rocRefs.graphType?.value || raw?.graphType || session?.state?.controls?.graphType || 'roc').toLowerCase();
  ...
  function computeRocAdvisorRecommendation(answers, context){
    const recommendation={
      ...
      rationale:[],
      warnings:[],
      diffMethod:context?.diffMethod || state.diffMethod || 'delong'
    };
    if(!answers.methodChoice){
  ...
  function renderRocStatsAdvisor(rawContext, options = {}){
    const session = ensureRocSessionOwnershipShape(options.session || getActiveRocSessionForState());
    const rocRefs = resolveRocStatsRefsForSession(session, options);
    const container = rocRefs.statsAdvisor || null;
    if(!container){
      return;
    }
    const advisorState = getRocAdvisorState(session);
    const context=buildRocAdvisorContext(rawContext || advisorState.context || {}, { ...options, session, refs: rocRefs });
    ...
    if(sharedAdvisorUi && typeof sharedAdvisorUi.renderAdvisorPanel==='function'){
      sharedAdvisorUi.renderAdvisorPanel({
        ...
        onToggle: (nextOpen)=>{
          ...
          setRocAdvisorState(advisorState, session);
          persistRocTabState('roc-stats-advisor-toggle', session);
          renderRocStatsAdvisor(advisorState.context, { ...options, session, refs: rocRefs });
        },
        onAnswerChange: (question, value)=>{
          ...
          setRocAdvisorState(advisorState, session);
          persistRocTabState('roc-stats-advisor-answer', session);
          renderRocStatsAdvisor(advisorState.context, { ...options, session, refs: rocRefs });
        },
        onApply: ()=>{
          ...
          setRocAdvisorState(advisorState, session);
          persistRocTabState('roc-stats-advisor-apply', session);
          renderStatsControls({ ...options, session });
        ...
        onReset: ()=>{
          ...
          setRocAdvisorState(advisorState, session);
          persistRocTabState('roc-stats-advisor-reset', session);
          renderRocStatsAdvisor(advisorState.context, { ...options, session, refs: rocRefs });
        }
      });
      return;
    ...
    toggle.addEventListener('click', event => {
      runRocControlOwner(event, 'roc-stats-advisor-toggle', ownerSession => {
        ...
        persistRocTabState('roc-stats-advisor-toggle', ownerSession || session);
        renderRocStatsAdvisor(advisorState.context, { ...options, session: ownerSession || session, refs: rocRefs });
      });
    });
    ...
    if(advisorState.open){
        ...
        (question.options||[]).forEach(option=>{
          ...
          input.addEventListener('change', event => {
            runRocControlOwner(event, 'roc-stats-advisor-answer', ownerSession => {
              ...
              setRocAdvisorState(advisorState, ownerSession || session);
              persistRocTabState('roc-stats-advisor-answer', ownerSession || session);
              renderRocStatsAdvisor(advisorState.context, { ...options, session: ownerSession || session, refs: rocRefs });
            });
          });
      ...
      applyBtn.addEventListener('click', event => {
        runRocControlOwner(event, 'roc-stats-advisor-apply', ownerSession => {
          ...
          console.debug('Debug: roc statsAdvisor applied',{ diffMethod:recommendation.diffMethod, answers:{ ...answers } });
          setRocAdvisorState(advisorState, ownerSession || session);
          persistRocTabState('roc-stats-advisor-apply', ownerSession || session);
          renderStatsControls({ ...options, session: ownerSession || session });
      ...
      resetBtn.addEventListener('click', event => {
        runRocControlOwner(event, 'roc-stats-advisor-reset', ownerSession => {
          ...
          console.debug('Debug: roc statsAdvisor reset');
          setRocAdvisorState(advisorState, ownerSession || session);
          persistRocTabState('roc-stats-advisor-reset', ownerSession || session);
          renderRocStatsAdvisor(advisorState.context, { ...options, session: ownerSession || session, refs: rocRefs });
        });
      });
  ...
  function renderStatsControls(options = {}){
    const session = ensureRocSessionOwnershipShape(options.session || getActiveRocSessionForState());
    if(!refs.statsControls){
      return;
    }
    renderRocStatsAdvisor(null, { ...options, session });
    ...
    if(!methodOptions.some(opt => opt[0] === state.diffMethod)){
      state.diffMethod = methodOptions[0][0];
    }
  ...
  function initExampleAndImport(){
    ...
    bindRocControlHandler(refs.fileInput, 'change', 'import-file', async (_event, ownerSession) => {
      ...
      if(hasFile && isRocSessionActive(importSession)){
      ...
      try{
        const result = await tableImport.openFile(refs.fileInput, {
          ...
          scheduleDraw: () => {
            if(importSession && !isRocSessionActive(importSession)){
        ...
        if(!result && forcedOverlay && isRocSessionActive(importSession)){
        ...
        if(forcedOverlay && isRocSessionActive(importSession)){
  ...
  function getRocStatsInferenceTabId(options = {}){
    const session = ensureRocSessionOwnershipShape(options.session || null);
    return session?.tabId
      || normalizeRocSessionTabId(options.tabId || null, { tabId: options.tabId || null })
      || getRocProjectionTabId()
      || getActiveRocSessionForState()?.tabId
      || null;
  }

  function getRocStatsAlpha(options = {}){
    const alpha = Number(Shared.statsInference?.getAlpha?.({ tabId:getRocStatsInferenceTabId(options) }));
    return Number.isFinite(alpha) && alpha > 0 && alpha < 1
      ? alpha
      : (Shared.statsInference?.DEFAULT_ALPHA || 0.05);
  }

  function createRocInferenceSpec(options = {}){
    if(typeof Shared.statsInference?.createDecisionSpec === 'function'){
      return Shared.statsInference.createDecisionSpec({
        tabId:getRocStatsInferenceTabId(options), criterion:'alpha', method:'none', valueKind:'raw-p'
      });
    }
    return { criterion:'alpha', level:getRocStatsAlpha(options), method:'none', valueKind:'raw-p' };
  }
  ...
  function formatRocPExpression(value, options = {}){
    const reporting = Shared.statsReporting;
    const session = ensureRocSessionOwnershipShape(options.session || getActiveRocSessionForState());
    const rocRefs = resolveRocStatsRefsForSession(session, options);
  ...
  function buildRocOnPlotPresentation({ seriesCount = 0, graphType = 'roc', stats = [], compareResultModel = null, session = null, refs: ownerRefs = null } = {}){
    const count = Math.max(0, Math.trunc(Number(seriesCount) || 0));
    ...
    if(singleCurve){
      ...
      if(normalizedGraphType === 'roc'){
        ...
        const averagePrecision = stat.avgPrecision ?? stat.auc;
        if(averagePrecision !== null && averagePrecision !== undefined && Number.isFinite(Number(averagePrecision))){
          presentation.lines.push(`AP = ${formatRocDecimal(Number(averagePrecision), 3)}`);
        }
      }
      return presentation;
    }
    if(count >= 2 && compareResultModel?.displayText){
      presentation.lines.push(String(compareResultModel.displayText));
    }
    return presentation;
  }

  function updateRocOnPlotControlPresentation(seriesCount, graphType = refs.graphType?.value || state.controls?.graphType || 'roc'){
    const presentation = buildRocOnPlotPresentation({ seriesCount, graphType });
    if(refs.showComparisonOnPlotLabel){
      refs.showComparisonOnPlotLabel.textContent = presentation.label;
    }
    return presentation;
  }
  ...
  function renderRocAucDirectionWarning(stats, graphType, options = {}){
    const session = ensureRocSessionOwnershipShape(options.session || getActiveRocSessionForState());
    const rocRefs = resolveRocStatsRefsForSession(session, options);
    const statsResults = rocRefs.statsResults || null;
    const warning = getRocAucDirectionWarning(stats, graphType);
    if(!warning || !statsResults){ return null; }
    ...
    statsResults.insertBefore(banner, statsResults.firstChild || null);
    return banner;
  }
  ...
  function renderRocStatsSummary(stats, graphType, options = {}){
    ...
    if(!Array.isArray(stats) || !stats.length){
      ...
      statsResults.appendChild(message);
      const emptyModel = createDefaultRocStatsPanelModel({ resultsModel: null, reportModel: null });
      if(session){
        session.state.statsPanelModel = emptyModel;
        session.results = createDefaultRocResultsState({
          statsPanelModel: emptyModel,
          compareSelection: session.results?.compareSelection || session.state.compareSelection || null,
          diffMethod: session.results?.diffMethod || session.state.diffMethod || 'delong',
          compareResult: session.results?.compareResult || session.state.compareResult || null
        });
        session.updatedAt = Date.now();
        if(isRocSessionActive(session)){
          state.statsPanelModel = emptyModel;
        }
      }else{
        state.statsPanelModel = emptyModel;
      }
    ...
    const rows=stats.map(stat=>({
      ...
      aucCi:graphType==='roc'
        ...
        : undefined,
      p:graphType==='roc' ? formatPValue(stat.pVal, { ...options, session, refs: rocRefs }) : undefined,
      pCell:graphType==='roc' ? rocInferencePValue(stat.pVal, { ...options, session, refs: rocRefs }) : undefined,
    ...
    const appendThresholdTables = useSharedTable => {
      ...
      stats.forEach(stat => {
        ...
        if(useSharedTable){
          Shared.statsTable.render({ target: statsResults, ...thresholdModel, append: true });
          return;
        }
        const caption=document.createElement('div');
        caption.className='stats-table-lead';
        caption.textContent=thresholdModel.caption;
        statsResults.appendChild(caption);
    ...
    if(hasStatsTable){
      Shared.statsTable.render({ target:statsResults, ...model });
      appendThresholdTables(true);
      renderRocAucDirectionWarning(stats, graphType, { ...options, session, refs: rocRefs });
    ...
    rows.forEach(row=>{
      ...
      statsResults.appendChild(paragraph);
    });
    ...
    statsResults.appendChild(footnoteBlock);
  ...
  function appendRocReportPanel(stats, graphType, diffResult, options = {}){
    ...
    const rocRefs = resolveRocStatsRefsForSession(session, options);
    const statsResults = rocRefs.statsResults || null;
    if(!statsResults || !Array.isArray(stats) || !stats.length || !(Shared.statsReporting && typeof Shared.statsReporting.appendReportPanel==='function')){
      return false;
    }
    ...
    const compareSelection = session?.results?.compareSelection || session?.state?.compareSelection || state.compareSelection || (isRocSessionActive(session) ? state.compareSel?.value : null) || null;
    const formatOptions = { ...options, session, refs: rocRefs };
    const primaryCiText = graphType === 'roc' && Number.isFinite(primary?.aucCiLow) && Number.isFinite(primary?.aucCiHigh)
      ? `; 95% CI [${formatRocDecimal(primary.aucCiLow,3)}, ${formatRocDecimal(primary.aucCiHigh,3)}]`
      : '';
    ...
    Shared.statsReporting.appendReportPanel(statsResults, {
      ...
      resultsParts: [
        `${stats.length} series were analysed.`,
        primary ? (graphType === 'roc' ? [' ', primaryTextPrefix, { type:'pValue', value:primary.pVal, fallback:String(formatPValue(primary.pVal, formatOptions)), __statsInference:createRocInferenceSpec(formatOptions) }, '.'] : [' ', primaryTextPrefix]) : null,
        graphType === 'roc' && primary && Array.isArray(primary.thresholdRows) ? ` ${primary.thresholdRows.length} cutoff row(s) were tabulated for ${primary.name}.` : null,
      ...
      analysisSpec: {
        ...
        inference: typeof Shared.statsInference?.createSnapshot === 'function'
          ? Shared.statsInference.createSnapshot({ tabId:getRocStatsInferenceTabId(formatOptions), includeOverall:true, includeComparisons:false })
    ...
    pinRocStatsReportAfterMetrics(statsResults, formatOptions);
    return true;
  }
  ...
  async function runRocDrawCycle(meta = {}){
    ...
    if(scheduleMeta && !Shared.workspaceTabs?.isSessionMetaCurrent?.('roc', scheduleMeta)){
      deferRocScheduledDrawOwner(meta?.tabId || scheduleMeta?.tabId || null, meta, 'roc-draw-cycle-stale-session-deferred');
      console.debug('Debug: roc scheduled draw skipped for stale session', {
    ...
    if(drawSession && !isRocSessionActive(drawSession)){
      deferRocScheduledDrawOwner(
        drawSession.tabId || meta?.tabId || null,
        meta,
        'roc-draw-cycle-inactive-owner-deferred'
      );
      return;
    }
    const requestedGeneration = Number(meta?.drawGeneration);
    let runtime = getRocDrawRuntime(drawSession);
    if(Number.isFinite(requestedGeneration) && requestedGeneration > 0){
      if(requestedGeneration !== Number(runtime.generation || 0)){
        settleRocScheduledDrawOwner(drawSession?.tabId || meta?.tabId || null, requestedGeneration, 'roc-draw-cycle-stale-generation');
        return;
      }
    }else if(drawSession){
      runtime = updateRocDrawRuntime(drawSession, nextRuntime => {
        nextRuntime.generation = Number(nextRuntime.generation || 0) + 1;
      });
      meta = { ...(meta || {}), drawGeneration: runtime.generation };
    }
    const drawGeneration = Number(meta?.drawGeneration || runtime.generation || 0);
    updateRocDrawRuntime(drawSession, nextRuntime => {
      nextRuntime.scheduled = false;
    ...
    try{
      ...
      const latestRuntime = getRocDrawRuntime(drawSession);
      if(drawSession && drawGeneration === Number(latestRuntime.generation || 0)){
        updateRocDrawRuntime(drawSession, nextRuntime => {
          nextRuntime.scheduled = false;
          nextRuntime.inProgress = false;
          nextRuntime.requestOptions = null;
          nextRuntime.lastStatus = nextRuntime.deferredOptions ? 'deferred' : status;
          nextRuntime.lastReason = meta?.reason || nextRuntime.lastReason || 'roc-draw';
        });
      }
  ...
  async function drawRoc(meta = {}, session = null){
    ...
    if(drawSession && !isRocSessionActive(drawSession)){
      deferRocScheduledDrawOwner(drawSession.tabId || meta?.tabId || null, meta, 'roc-draw-inactive-owner-deferred');
      return false;
    }
    bindRocSessionForTab(drawSession?.tabId || meta?.tab || meta?.tabId || getRocProjectionTabId() || null, {
      ...(meta || {}),
    ...
    try{
    refreshRocActiveDomRefsForSession(drawSession, { reason: meta?.reason || 'roc-draw-owner-refs' });
    const drawRefs = resolveRocRefsContext(drawSession, { allowFallback: false });
    const drawRoot = resolveRocOwnedRoot(drawTabId, { allowProjected: isRocSessionActive(drawSession) }) || drawRefs.root || null;
    drawRefs.root = drawRoot;
    drawRefs.plotDiv = getRocOwnedNodeById('rocPlot', drawTabId, { allowProjected: false }) || drawRefs.plotDiv || null;
    drawRefs.statsResults = getRocOwnedNodeById('rocStatsResults', drawTabId, { allowProjected: false }) || drawRefs.statsResults || null;
    drawRefs.svgBox = drawRefs.plotDiv?.closest?.('.svgbox') || drawRoot?.querySelector?.('#rocGraphPanel .svgbox') || drawRefs.svgBox || null;
    drawRefs.fontSize = getRocOwnedNodeById('rocFontSize', drawTabId, { allowProjected: false }) || drawRefs.fontSize || null;
    drawRefs.fontSizeVal = getRocOwnedNodeById('rocFontSizeVal', drawTabId, { allowProjected: false }) || drawRefs.fontSizeVal || null;
    setRocSessionRefs(drawSession, drawRefs, { applyActive: false });
    if(!state.hot || !drawRefs.plotDiv){
      return false;
    }
    const debugStamp = Date.now();
    const controls = syncRocRuntimeControlsFromDom(drawSession);
    ...
    if(!data || !data.length){
      clearPlotArea('no-table', { session: drawSession, refs: drawRefs });
      return;
    }
    ...
    if(!hasRowContent){
      clearPlotArea('empty-rows', { session: drawSession, refs: drawRefs });
      return;
    }
    const header = data[0] || [];
    let labelIndex = header.findIndex(h => String(h).trim().toLowerCase() === 'label');
    ...
    if(!classification.valid){
      clearPlotArea('invalid-outcome-classes', { session: drawSession, refs: drawRefs, message: 'Exactly two valid Label classes are required.' });
      return;
    }
    ...
    if(!series.length){
      clearPlotArea('no-series', { session: drawSession, refs: drawRefs });
      return;
    }
    const hasValidScores = series.some(serie => serie.scores.some(score => !Number.isNaN(score)));
    if(!hasValidScores){
      clearPlotArea('no-scores', { session: drawSession, refs: drawRefs });
      return;
    }
    ...
    framePublication = Shared.framePublication.stage({
      ...
      canCommit: () => execution?.isCurrent?.() !== false
        && Number(getRocDrawRuntime(drawSession).generation || 0) === Number(meta?.drawGeneration || 0)
        && (!drawSession || isRocSessionActive(drawSession))
    });
    ...
    const legendEntries = showLegend ? legendLabels.map((label, index) => ({
      label: legendMetricLabels[index] || label,
    ...
    if(showGrid){
      ...
      xTicks.forEach(tick => {
        const x = xToPx(tick);
        gridSegments.push({ x1: x, y1: margin.top, x2: x, y2: margin.top + plotHeight });
      });
      yTicks.forEach(tick => {
        const y = yToPx(tick);
        gridSegments.push({ x1: margin.left, y1: y, x2: margin.left + plotWidth, y2: y });
      });
    ...
    const axisControlConfig = axis => buildRocAxisControlConfig(axis, drawSession, {
    ...
    bindRocTitleInlineInteraction(titleNode, drawSession);
    ...
    const onPlotPresentation = buildRocOnPlotPresentation({
      seriesCount: series.length,
      graphType,
      stats,
    ...
    if(refs.showComparisonOnPlotLabel){
      refs.showComparisonOnPlotLabel.textContent = onPlotPresentation.label;
    }
    if(controls.showComparisonOnPlot && onPlotPresentation.lines.length){
      const statsFontSize = chartStyle.resolveStatsAnnotationFontMetrics(fontSize, { styles: rocFontStyles }).fontSizePx;
      const statsFrame = { originX: margin.left, originY: margin.top, width: plotWidth, height: plotHeight };
      const statsPosition = chartStyle.resolveStatsAnnotationPosition(
        normalizeRocLabelPositions(drawSession?.state?.labelPositions || state.labelPositions).stats,
        {
          x: margin.left + plotWidth - Math.max(6, fontSize * 0.6),
          y: margin.top + Math.max(fontSize * 1.2, 14)
        },
        statsFrame
      );
      chartStyle.renderStatsAnnotation(svg, {
        lines: onPlotPresentation.lines,
        x: statsPosition.x,
        y: statsPosition.y,
        textAnchor: 'end',
        ...
        className: 'roc-plot-stats',
        onDragEnd: pos => patchRocLabelPosition(
          drawSession,
          'stats',
          chartStyle.captureStatsAnnotationPosition(pos, statsFrame),
          { reason: 'roc-stats-position' }
        )
      });
    }
    ...
    return true;
      ...
      if(drawSession?.timers?.framePublication === framePublication){
        drawSession.timers.framePublication = null;
      }
    }
  }
  ...
  function getPayload(context = {}){
    ...
    captureRocSessionStateFromActive(payloadSession, {
      reason: 'roc-get-payload-canonicalize',
      captureStatsPanel: true,
      syncControls: false
    });
    ...
    const payload = {
      ...
      config: {
        ...
        showComparisonOnPlot: !!controls.showComparisonOnPlot,
        fontSize: controls.fontSize,
        fontStyles: exportFontStyles('roc', { tabId: payloadSession?.tabId || requestedTabId || projectionTabId || null }) || undefined,
        labelColors: state.labelColors,
        labelStrokeWidth: state.labelStrokeWidth,
        labelOpacity: state.labelOpacity,
        labelLinePattern: state.labelLinePattern,
    ...
    const statsPanelModel = selectRocStatsPanelModel(
  ...
  function syncRocRuntimeControlsFromState(controlSnapshot = {}){
    ...
    if(refs.showComparisonOnPlot && hasControl('showComparisonOnPlot')){
      refs.showComparisonOnPlot.checked = !!controls.showComparisonOnPlot;
    }
  ...
  roc.applyRuntimeState = function applyRocRuntimeState(snapshot, meta = {}){
    ...
    const isActiveOwner = isRocSessionActive(runtimeSession);
    if(isActiveOwner){
  ...
  roc.deactivateTab = Shared.componentLifecycle?.createDeactivateHandler?.({
    ...
    componentKey: 'roc',
    cancel: (tab, meta = {}) => {
      const tabId = (tab && typeof tab === 'object' ? tab.id : tab) || meta?.tabId || getRocProjectionTabId() || null;
      captureRocSessionForDeactivation(tab, meta);
      deactivateRocDrawOwner(tabId, meta?.reason || 'roc-tab-deactivated');
      state.drawPending = false;
    }
  ...
  roc.createEmptyPayload = function createEmptyRocPayload(){
    ...
    payload.config.graphType = payload.config.graphType || 'roc';
    payload.config.showGrid = payload.config.showGrid !== undefined ? !!payload.config.showGrid : false;
    payload.config.showFrame = payload.config.showFrame !== undefined ? !!payload.config.showFrame : true;
  ...
  function applyRocPayload(payload, meta){
    ...
    const passivePayloadHydration = skipDraw
      || meta?.passive === true
      || meta?.passiveControls === true
      || meta?.restoreRenderCache === true
      || meta?.restoreTransaction === true;
    if(!passivePayloadHydration && rocAutoDrawManager && isRocSessionActive(getActiveRocSessionForState())){
    ...
    importFontStyles('roc', config.fontStyles || null, { tabId: scheduleTargetSession?.tabId || scheduleTargetTab || null });
    ...
    if(refs.showComparisonOnPlot){
      refs.showComparisonOnPlot.checked = !!config.showComparisonOnPlot;
    }
    ...
    state.statsPanelSignature = statsConfig.statsPanelSignature == null ? '' : String(statsConfig.statsPanelSignature);
      ...
      state.analysisSignature = '';
      state.statsPanelSignature = '';
      setRocAdvisorState({}, payloadSession || scheduleTargetSession || getRocProjectionSession({ reason: 'roc-projection-mutation' }));
    ...
    const statsOwnerSession = payloadSession || scheduleTargetSession || getActiveRocSessionForState();
    const statsOwnerRefs = resolveRocStatsRefsForSession(statsOwnerSession, { tabId: statsOwnerSession?.tabId || scheduleTargetTab || null });
    if(statsConfig && statsSignatureMatches && rocStatsPanelModelHasContent(state.statsPanelModel)){
      restoreRocStatsPanelModel(state.statsPanelModel, { session: statsOwnerSession, refs: statsOwnerRefs });
    }else if(statsOwnerRefs.statsResults){
      if(!statsSignatureMatches){
        state.statsPanelModel = { resultsModel: null, reportModel: null };
        state.statsPanelSignature = '';
      }
      renderRocStatsSummary([], refs.graphType?.value || 'roc', { session: statsOwnerSession, refs: statsOwnerRefs });
    }
    if(!skipDraw){
  ...
  async function saveFile(){
    ...
    const result = await fileIO.saveGraphFile({
      context: 'roc',
      owner: { component: 'roc', tabId: operationSession?.tabId || getRocProjectionTabId() || null },
  ...
  function loadFromFile(file, options = {}){
    const ownerTabId = String(options?.tabId || options?.operation?.tabId || getRocProjectionTabId() || '').trim() || null;
    const operation = fileIO?.createGraphOpenOperation?.({
      context: 'roc',
      operation: options?.operation,
      owner: { component: 'roc', tabId: ownerTabId }
    ...
    const apply = payload => {
      const routed = fileIO?.routeGraphOpenPayload?.({
        ...
        operation,
        payload,
        reason: 'roc-graph-file-open',
        apply: (nextPayload, owner) => applyRocPayload(nextPayload, {
          source: 'file',
          tabId: owner?.tabId || ownerTabId || undefined
        })
      });
      const fallbackOwnerIsCurrent = !ownerTabId || String(getRocProjectionTabId() || '') === ownerTabId;
      return routed ? routed.value !== false : (fallbackOwnerIsCurrent && applyRocPayload(payload, {
        source: 'file',
  ...
  async function openFile(){
    const operationSession = getActiveRocSessionForState();
    const operationTabId = operationSession?.tabId || getRocProjectionTabId() || null;
    ...
    const result = await fileIO.openGraphFile({
      context: 'roc',
      owner: { component: 'roc', tabId: operationTabId },
  ...
  function initExportsAndFiles(){
    if (Shared.exporter && typeof Shared.exporter.mountSvgControls === 'function') {
      Shared.exporter.mountSvgControls({
        container: getRocNodeById('rocExportControls'),
        getSvg: () => getRocNodeById('rocSvg'),
        fileName: 'roc',
    ...
    refs.graphFileInput?.addEventListener('change', event => {
      ...
      if(file){
        ...
        const operationTabId = session?.tabId || getRocProjectionTabId() || null;
        setRocFileNameForSession(file.name, session);
        setRocFileHandleForSession(null, session);
        loadFromFile(file, { tabId: operationTabId });
      }
    });
  }
  ...
  function initControls(){
    ...
    refs.showComparisonOnPlot?.addEventListener('change', event => {
      const session = getRocSessionForEvent(event, { reason: 'roc-comparison-overlay-control' }, { create: true });
      syncRocRuntimeControlsFromEvent({ event, session, reason: 'roc-comparison-overlay-control' });
      scheduleRocControlDraw('roc-comparison-overlay-control', { event, session });
    });
  ...
  function init(options = {}){
    ...
    const scheduleRocDrawBase = Shared.componentLifecycle?.createTabScopedFrameDebouncer
      ? Shared.componentLifecycle.createTabScopedFrameDebouncer(roc, 'roc', runRocDrawCycle, {
          reason: 'roc-draw-frame',
          onStaleDiscard: ({ args, meta }) => {
            const discardedOptions = args?.[0] && typeof args[0] === 'object' ? args[0] : {};
            const tabId = discardedOptions.tabId || meta?.tabId || null;
            deferRocScheduledDrawOwner(tabId, discardedOptions, 'roc-stale-frame-deferred');
          }
        })
      : runRocDrawCycle;
    const scheduleRocDrawInstrumented = (opts) => {
      ...
      if(ownerSession){
        updateRocDrawRuntime(ownerSession, runtime => {
          runtime.scheduled = true;
          runtime.requestOptions = normalizeRocQueuedDrawOptions(nextOpts);
      ...
      const runSchedule = () => {
        const handle = scheduleRocDrawBase(nextOpts);
        if(handle == null || handle === false){
          settleRocScheduledDrawFromOptions(nextOpts, 'roc-frame-schedule-rejected');
        }
        return handle;
      };
      if(Shared.componentLifecycle?.runDrawWithOverlayPaintGate?.({
        ...
        onFrameStale: scoped => {
          deferRocScheduledDrawOwner(nextOpts.tabId || scoped?.tabId || null, nextOpts, 'roc-overlay-frame-deferred');
        },
        run: runSchedule
      })){
        return;
    ...
    if(rocAutoDrawManager){
      ...
      rocAutoDrawManager.updateUi();
      const passiveInit = options?.suppressDraw === true
        || options?.suppressAutoDraw === true
        || options?.liveDomFastPath === true
        || options?.liveDomReuse === true
        || options?.passiveControls === true
        || options?.restoreRenderCache === true
        || options?.restoreTransaction === true;
      // Passive activation/reopen projects the owner-canonical AutoDraw state. Re-evaluating
      // thresholds from whichever DOM table happens to be mounted can mutate another owner's
      // scheduling state before its payload/cache is bound.
      if(!passiveInit){
        rocAutoDrawManager.evaluateThresholds();
      }
  ...
  roc.activateTab = Shared.componentLifecycle?.bindTabActivation?.({
    ...
    afterReady: (tabLike, meta = {}) => {
      ...
      syncRocClassificationControls();
      syncRocSessionManagersFromActive();
      const ownerSession = getActiveRocSessionForState();
      rocAutoDrawManager?.evaluateThresholds?.();
      captureRocSessionStateFromActive(ownerSession, {
      ...
      if(deferredDraw || !frameMatchesOwner){
        scheduleRocDrawForSession(ownerSession, {
          ...(deferredDraw || {}),
          tabId: ownerSession?.tabId || deferredDraw?.tabId || null,
          force: !frameMatchesOwner || deferredDraw?.force === true,
          viewOnly: !frameMatchesOwner || deferredDraw?.viewOnly === true,
          silentOverlay: !frameMatchesOwner || deferredDraw?.silentOverlay === true,
          reason: !frameMatchesOwner ? 'roc-activation-frame-sync' : (deferredDraw?.reason || 'roc-activation-deferred-draw')
        });
      }
    },
  ...
  roc.cancelCurrentDraw = function cancelCurrentDraw(meta = {}){
    ...
    settleRocScheduledDrawOwner(tabId, null, meta?.reason || 'roc-draw-cancel');
    const owner = tabId ? getRocSession(tabId, { tabId, reason: 'roc-draw-cancel-runtime' }, { create: false }) : null;
    if(owner){
      cleanupRocFramePublication(owner);
      updateRocDrawRuntime(owner, runtime => {
        runtime.generation = Number(runtime.generation || 0) + 1;
        runtime.scheduled = false;
        runtime.inProgress = false;
        runtime.deferredOptions = null;
  ...
  roc.loadFromPayload = function loadFromPayload(payload, options = {}){
    if(options.restoreRenderCache !== true && options.restoreTransaction !== true){
      invalidateRocRenderCacheForTab(
        options.tabId || options.workspaceTabId || getRocProjectionTabId() || null,
        options.source || options.reason || 'roc-payload-load'
      );
    }
  ...
  function resolveRocRenderCacheRoot(tabLike = null){
    const targetTabId = normalizeRocSessionTabId(tabLike || getRocProjectionTabId() || null, {});
    if(targetTabId){
      return resolveRocOwnedRoot(targetTabId, { allowProjected: false })
        || (String(getRocProjectionTabId() || '') === String(targetTabId) ? (normalizeRocRefValue(refs.root) || normalizeRocRefValue(state.root) || null) : null);
    }
    return normalizeRocRefValue(refs.root) || normalizeRocRefValue(state.root) || null;
  }
  ...
  function hasRocPublishedGraphType(root = null, graphType = 'roc'){
    const ownerRoot = root || resolveRocRenderCacheRoot();
    const svg = ownerRoot?.querySelector?.('#rocSvg') || null;
    if(!svg){ return false; }
    const expected = String(graphType || 'roc').toLowerCase() === 'pr' ? 'pr' : 'roc';
    const stamped = String(svg.dataset?.rocGraphType || '').toLowerCase();
    if(stamped){ return stamped === expected; }
    const text = String(svg.textContent || '');
    return expected === 'pr'
      ? /Precision-Recall|Average Precision|\bAP\b/i.test(text)
      : /ROC curve|False Positive Rate|\bAUC\b/i.test(text);
  }
  ...
  function restoreRocStatsSurfaceFromOwner(owner = null){
    ...
    const model = selectRocStatsPanelModel(
      shaped.results?.statsPanelModel,
      shaped.state?.statsPanelModel,
      isRocSessionActive(shaped) ? state.statsPanelModel : null
    );
    if(!rocStatsPanelModelHasContent(model)){
      return false;
    }
    const ownerRoot = resolveRocOwnedRoot(shaped.tabId || null, { allowProjected: isRocSessionActive(shaped) });
    const ownerRefs = resolveRocStatsRefsForSession(shaped, { root: ownerRoot, tabId: shaped.tabId || null });
  ...
  roc.isIdleForSnapshot = function isIdleForSnapshot(meta = {}){
    ...
    // Auto-draw's drawPending flag means "edits are waiting for a manual Update Plot";
    // it is not an in-flight publication. Snapshot readiness follows the same contract as
    // Box/Scatter and blocks only on accepted/running draw work. Render-cache provenance
    // separately prevents an older published frame from being certified for newer payload.
    return isRocDrawRuntimeSnapshotIdle(runtime);
  };

  roc.isRenderCacheCurrent = function isRenderCacheCurrent(meta = {}){
    const owner = getRocSession(meta?.session || meta?.tab || meta?.tabId || null, {
      ...(meta || {}),
      reason: meta?.reason || 'roc-render-cache-currentness'
    }, { create: false }) || getActiveRocSessionForState();
    if(!owner || !isRocSessionActive(owner)){
      return false;
    }
    const runtime = getRocDrawRuntime(owner);
    if(!isRocDrawRuntimeCacheCurrent(runtime)){
      return false;
    }
    // Shared AutoDraw uses drawPending to mean that canonical data/controls are newer than
    // the published graph because the user is in manual-render mode. That state is safe to
    // save, but the current DOM must not be stamped as a cache for the newer payload.
    ...
    return state.drawPending !== true && hasRocPublishedGraphType(ownerRoot, expectedGraphType);
  };

  roc.awaitReadyForSnapshot = async function awaitReadyForSnapshot(meta = {}){
    const owner = getRocSession(meta?.session || meta?.tab || meta?.tabId || null, {
      ...(meta || {}),
      reason: meta?.reason || 'roc-snapshot-ready-owner'
    }, { create: false }) || getActiveRocSessionForState();
    if(owner && isRocSessionActive(owner)){
      ...
      if(!hasRocPublishedGraphType(ownerRoot, expectedGraphType) && !drawInFlight){
        scheduleRocDrawForSession(owner, {
          ...
          viewOnly: true,
          silentOverlay: true,
          reason: 'roc-snapshot-frame-sync'
        });
      }
    }
  ...
  roc.rehydrateGraphInteractions = function rehydrateGraphInteractions(meta = {}){
    const owner = getRocRenderCacheOwner(meta, 'roc-render-cache-interaction-rehydrate');
    const root = meta.root || resolveRocRenderCacheRoot(owner?.tabId || meta.tabId || null);
    const svg = root?.querySelector?.('#rocSvg') || meta.svgs?.find?.(node => node?.id === 'rocSvg') || null;
    if(!owner || !svg){ return false; }
    const axesReady = axisControls?.rehydrateAxisElements?.(svg, (axis, _element, metadata) =>
      buildRocAxisControlConfig(axis, owner, metadata)
    ) !== false;
    const textReady = rehydrateRocInlineTextInteractions(svg, owner);
    rehydrateRocCurveInteractions(root);
    bindRocLegendInteractions(
  ...
  roc.restoreRenderCache = function restoreRenderCache(cache, meta = {}){
    ...
    const requiresStatsRestore = rocStatsPanelModelHasContent(owner.results?.statsPanelModel || owner.state?.statsPanelModel || null);
    const statsRestored = requiresStatsRestore ? restoreRocStatsSurfaceFromOwner(owner) : true;
    const visuallyReady = hasRocPublishedGraph(ownerRoot);
    if(visuallyReady){
      chartStyle.rehydrateLegendViewports?.(plot);
      const svg = plot?.querySelector?.('#rocSvg') || null;
      bindRocLegendInteractions(
        svg?.querySelector?.('[data-legend-viewport-content="true"]') || null,
        svg,
        owner
    ...
    if(typeof Shared.isDebugEnabled === 'function' && Shared.isDebugEnabled()){
      console.debug('Debug: roc render cache restored', {
        ...
        plot: restoredPlot,
        statsRestored,
        requiresStatsRestore,
        curveBindings,
        tabId: targetTabId
      });
    }
    return visuallyReady && (!requiresStatsRestore || statsRestored);
  };
  ...
  roc.__testHooks = Object.assign({}, roc.__testHooks, {
    ...
    restoreStatsSurfaceForOwner: tabId => {
      const session = getRocSession(tabId, { tabId, reason: 'roc-test-stats-surface-restore' }, { create: false });
      return restoreRocStatsSurfaceFromOwner(session);
    },
    ...
    mergeDrawOptions: (previous, next) => mergeRocDrawOptions(previous, next),
    isDrawRuntimeSnapshotIdle: runtime => isRocDrawRuntimeSnapshotIdle(runtime),
    isDrawRuntimeCacheCurrent: runtime => isRocDrawRuntimeCacheCurrent(runtime),
    getDrawRuntime: tabId => {
      const session = getRocSession(tabId, { tabId, reason: 'roc-test-draw-runtime' }, { create: false });
      return cloneSimple(getRocDrawRuntime(session)) || null;
    },
    settleScheduledDraw: (tabId, generation = null, reason = 'roc-test-settle') => settleRocScheduledDrawOwner(tabId, generation, reason),
  ...
  Shared.componentLifecycle?.installInternalStateBridge?.(roc, {
    componentKey: 'roc',
    targets: [
      { key: 'state', get: () => state, excludeKeys: ['hot', 'root', 'svg', 'svgBox', 'drawPending', 'statsPanelModel'] },
