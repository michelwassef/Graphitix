...
(function(global){
  ...
  function resolvePieRenderImpact(options = {}, fallback = 'analysis'){
  ...
  function isPiePresentationDraw(options = {}){
    return Shared.componentLifecycle?.isPresentationOnlyDraw?.({
      renderImpact: resolvePieRenderImpact(options)
    }) === true;
  }

  function sanitizePieDrawOptions(options = {}, owner = {}){
    const source = options && typeof options === 'object' ? options : {};
    const renderImpact = resolvePieRenderImpact(source);
    const sanitized = Shared.componentLifecycle?.sanitizeComponentDrawOptions?.('pie', {
      ...source,
      renderImpact
    }, owner) || {
      ...source,
      ...
      reason: source.reason || owner?.reason || 'pie-draw'
    };
    sanitized.renderImpact = resolvePieRenderImpact(sanitized, renderImpact);
    if(Object.prototype.hasOwnProperty.call(source, 'viewOnly') || isPiePresentationDraw({ renderImpact })){
      sanitized.viewOnly = isPiePresentationDraw(sanitized);
    }
    return sanitized;
  }
  ...
  const PIE_STATS_CORRECTION_KEYS = Object.freeze([
    'none', 'bonferroni', 'holm', 'holm-sidak', 'sidak', 'hochberg', 'bh', 'by'
  ]);
  const PIE_STATS_CORRECTION_KEY_SET = new Set(PIE_STATS_CORRECTION_KEYS);
  ...
  function mutateAxisSettingsForOwner(session, reason, mutation){
    const owner = ensurePieSessionOwnershipShape(session || getActivePieSessionForState());
    const ownerIsActive = !owner || isPieSessionActive(owner);
  ...
  function scheduleAxisSettingsDraw(result, reason, renderImpact = 'layout'){
    return schedulePieDrawForSession(result?.owner || getActivePieSessionForState(), { reason, renderImpact });
  }
  ...
  function updateXAxisTickLabelAngle(value, session = null){
    ...
    schedulePieViewRefresh('pie-axis-x-label-angle', { tabId: result.owner?.tabId || null, userInitiated: true, renderImpact: 'layout' });
  }
  ...
  function updateAxisStrokeWidth(value, session = null){
    ...
    const tabId = result.owner?.tabId || getPieProjectionTabId() || null;
    const svg = getPieNodeById('pieSvg', tabId) || getPieNodeById('pieSvg');
    const projected = Shared.visualProjection?.apply?.(svg, {
      component: 'pie',
      channel: 'axis',
      tabId,
      attributes: { strokeWidth: result.settings.strokeWidth }
    });
    if(!projected){
      schedulePieViewRefresh('pie-axis-stroke-width-change', { tabId, renderImpact: 'layout' });
    }
  }
  ...
  function updateAxisColor(value, session = null){
    ...
    const projected = Shared.visualProjection?.apply?.(svg, {
      ...
      channel: 'axis',
      tabId,
      attributes: { stroke: result.settings.color }
    });
    if(!projected){
      schedulePieViewRefresh('pie-axis-color-change', { tabId, renderImpact: 'paint' });
    }
  }

  function buildPieAxisControlConfig(axis, ownerSession = null, axisMeta = {}){
    const owner = ensurePieSessionOwnershipShape(ownerSession || getActivePieSessionForState());
    return {
      axis,
      scopeId: 'pie',
      tabId: owner?.tabId || null,
      getTickInterval: () => getAxisTickInterval(axis, owner),
      ...
      getColor: () => getAxisColor(owner),
      isTickIntervalEnabled: () => axis === 'y',
      getTickIntervalDisabledMessage: () => 'Tick interval is managed automatically for categorical axes.',
      tickPlaceholder: 'Auto',
      onTickIntervalChange: value => updateAxisTickInterval(axis, value, owner),
      getMinorTicksEnabled: () => getAxisMinorTicksEnabled(axis, owner),
      onMinorTicksChange: value => updateAxisMinorTicks(axis, value, owner),
      isMinorTicksSupported: () => axis === 'y',
      getMinorTickSubdivisions: () => getAxisMinorTickSubdivisions(axis, owner),
      onMinorTickSubdivisionsChange: value => updateAxisMinorTickSubdivisions(axis, value, owner),
      onThicknessChange: value => updateAxisStrokeWidth(value, owner),
      onColorChange: value => updateAxisColor(value, owner)
    };
  }
...
let state = {
    ...
    labelPositions: { title: null, legend: null, stats: null },
  ...
  function normalizePieAdvisorState(value = {}){
  ...
  function normalizePieStatsSettings(value = {}){
    const source = value && typeof value === 'object' ? value : {};
    const selectedSource = source.selectedCols instanceof Set
      ? Array.from(source.selectedCols)
      : (Array.isArray(source.selectedColumns) ? source.selectedColumns : (Array.isArray(source.selectedCols) ? source.selectedCols : []));
    const customSource = source.customPairs instanceof Set
      ? Array.from(source.customPairs)
      : (Array.isArray(source.customPairs) ? source.customPairs : []);
    return {
      scope: sanitizePieStatsScope(source.scope),
      test: sanitizePieStatsTest(source.test),
      correction: sanitizePieStatsCorrection(source.correction),
      sparseThreshold: sanitizePieStatsSparseThreshold(source.sparseThreshold),
      yatesCorrection: source.yatesCorrection !== false,
      referenceColumn: parsePieColumnIndex(source.referenceColumn),
      valueColumn: parsePieColumnIndex(source.valueColumn),
      expectedColumn: parsePieColumnIndex(source.expectedColumn),
      selectedCols: Array.from(selectedSource || []).map(Number).filter(value => Number.isInteger(value) && value >= 1).sort((a, b) => a - b),
      customPairs: Array.from(customSource || []).map(String).filter(Boolean).sort(),
      advancedOpen: source.advancedOpen === true,
      resultsTab: sanitizePieStatsResultsTab(source.resultsTab),
      contextSignature: typeof source.contextSignature === 'string' ? source.contextSignature : null,
      lastRunSignature: typeof source.lastRunSignature === 'string' ? source.lastRunSignature : null
    };
  }

  function normalizePieStatsPanelModel(value = {}){
    const source = value && typeof value === 'object' ? value : {};
    return {
      resultsModel: source.resultsModel || null,
      reportModel: source.reportModel || null
    };
  }

  function pieStatsPanelNodeText(node){
    if(!node || typeof node !== 'object'){ return ''; }
    const own = node.type === 'text' ? String(node.text || '') : '';
    const children = Array.isArray(node.children) ? node.children.map(pieStatsPanelNodeText).join(' ') : '';
    return `${own} ${children}`.trim();
  }

  function pieStatsPanelNodeHasStatContent(node){
    ...
    return Array.isArray(node.children) && node.children.some(pieStatsPanelNodeHasStatContent);
  }

  function pieStatsPanelModelHasContent(value = null){
    const model = normalizePieStatsPanelModel(value || {});
    if(pieStatsPanelNodeHasStatContent(model.reportModel)){ return true; }
    if(!pieStatsPanelNodeHasStatContent(model.resultsModel)){ return false; }
    const text = pieStatsPanelNodeText(model.resultsModel);
    return !/^(?:no data|expected values required|unable to compute)/i.test(text);
  }

  function composePieStatsConfig(settings = {}, advisor = {}, panelModel = {}){
    const normalized = normalizePieStatsSettings(settings);
    const panel = normalizePieStatsPanelModel(panelModel);
    return {
      scope: normalized.scope,
      test: normalized.test,
      correction: normalized.correction,
      sparseThreshold: normalized.sparseThreshold,
      yatesCorrection: normalized.yatesCorrection,
      referenceColumn: normalized.referenceColumn,
      valueColumn: normalized.valueColumn,
      expectedColumn: normalized.expectedColumn,
      selectedColumns: normalized.selectedCols.slice(),
      customPairs: normalized.customPairs.slice(),
      advancedOpen: normalized.advancedOpen,
      resultsTab: normalized.resultsTab,
      advisor: normalizePieAdvisorState(advisor),
      resultsModel: panel.resultsModel,
      reportModel: panel.reportModel,
      contextSignature: normalized.contextSignature,
      lastRunSignature: normalized.lastRunSignature
    };
  }
  ...
  function createDefaultPieDurableState(source = {}){
    ...
    return {
      ...
      labelPositions: normalizePieLabelPositions(src.labelPositions),
  ...
  function createDefaultPieResultsState(source = {}){
    ...
    return {
      ...
      statsPanelModel: normalizePieStatsPanelModel(src.statsPanelModel || {}),
  ...
  function createPieSession({ tabId, root = null, initialState = null } = {}){
    ...
    return {
      ...
      results: createDefaultPieResultsState({
        statsDataModel: source.results?.statsDataModel || source.statsDataModel,
        statsPanelModel: source.results?.statsPanelModel || source.statsPanelModel || {},
        statsSummaryTabIdCounter: source.results?.statsSummaryTabIdCounter || source.statsSummaryTabIdCounter
      }),
  ...
  const pieSessionShapeGuard = Shared.componentLifecycle.createSessionShapeGuard({
    getOwnerKey: session => session.tabId,
    fields: [
      {
        key: 'state',
        normalize: value => createDefaultPieDurableState(value || {})
      },
      {
        key: 'advisor',
        normalize: value => normalizePieAdvisorState(value || {})
      },
      {
        key: 'results',
        normalize: value => createDefaultPieResultsState(value || {})
      },
      {
        key: 'notes',
        normalize: value => createDefaultPieNotesState(value || {})
      }
    ]
  });
  ...
  function runPieControlOwner(event, reason, callback){
    ...
    if(session?.tabId && !isPieSessionActive(session)){
  ...
  function isPieSessionActive(session = null){
    ...
    return Shared.componentLifecycle?.canOwnerUseLiveProjection?.('pie', session, {
      component: pie,
      projectedSession: projectedPieSession,
      session,
  ...
  function isPieSessionActivationTarget(session = null){
    return Shared.componentLifecycle?.isOwnerActivationTarget?.('pie', session, { component: pie }) === true;
  }

  function markPiePayloadDirtyForSession(session = null, reason = 'pie-payload-dirty'){
    const ownerSession = ensurePieSessionOwnershipShape(session);
    const tabId = String(ownerSession?.tabId || '').trim();
    if(!tabId){
      return false;
    }
    const mainSession = global.Main?.session || null;
    if(typeof mainSession?.markTabUserModified !== 'function'){
      return false;
    }
    return !!mainSession.markTabUserModified(tabId, reason, {
      origin: 'user',
      type: 'pie',
      source: 'pie-data-view',
      affectsPayload: true
    });
  }
  ...
  function schedulePieDrawForSession(session = null, options = {}){
    ...
    const scheduleOptions = sanitizePieDrawOptions(sourceOptions, {
      tabId: shaped.tabId || null,
      reason: sourceOptions.reason || 'pie-session-draw'
    });
    if(shaped.timers){
      shaped.timers.pendingDrawOptions = scheduleOptions;
    }
    if(!isPieSessionActive(shaped)){
  ...
  function bumpPieDrawGeneration(session = null){
    const owner = ensurePieSessionOwnershipShape(session || getActivePieSessionForState());
    if(!owner?.timers){ return 0; }
    owner.timers.drawGeneration = (Number(owner.timers.drawGeneration) || 0) + 1;
    owner.updatedAt = Date.now();
    return owner.timers.drawGeneration;
  }

  function isPieDrawGenerationCurrent(session = null, generation = null){
    const owner = ensurePieSessionOwnershipShape(session || getActivePieSessionForState());
    return !!owner && Number(generation) === Number(owner.timers?.drawGeneration || 0);
  }
  ...
  function patchPieLabelPosition(session = null, key, value, meta = {}){
    const owner = ensurePieSessionOwnershipShape(session || getActivePieSessionForState());
    const currentPositions = normalizePieLabelPositions(owner?.state?.labelPositions || state.labelPositions);
    const nextPositions = normalizePieLabelPositions({ ...currentPositions, [key]: value || null });
    return patchPieVisualState(owner, { labelPositions: nextPositions }, meta);
  }
  ...
  function syncPieSessionRefsFromActive(session = null){
    ...
    if(shaped.tabId && !isPieSessionActive(shaped)){
      return shaped;
    }
    ...
    shaped.refs = Object.assign(createDefaultPieRefs(shaped.root || null), shaped.refs || {}, {
      ...
      showStatsSummary: queryPieRoot('#pieShowStatsSummary'),
  ...
  function bindPieLayoutManagerForSession(session){
    const shaped = ensurePieSessionOwnershipShape(session);
    if(!shaped){ return null; }
    const ownedLayout = Shared.componentLayout?.getOwnedLayoutFor?.('pie', { tabId: shaped.tabId }) || null;
  ...
  function syncPieSessionManagersFromActive(session = null){
    const shaped = ensurePieSessionOwnershipShape(session || projectedPieSession || getActivePieSessionForState());
    if(!shaped){ return null; }
    const sessionIsActive = !shaped.tabId || isPieSessionActive(shaped);
    ...
    if(sessionIsActive){
      bindPieLayoutManagerForSession(shaped);
  ...
  function capturePieSessionStateFromActive(session = null, meta = {}){
    ...
    if(meta.captureStats !== false && typeof exportPieStatsConfig === 'function'){
      state.statsConfig = exportPieStatsConfig(shaped);
    }
    shaped.state = createDefaultPieDurableState({
      ...
      stats: normalizePieStatsSettings(state.statsConfig),
    ...
    shaped.advisor = normalizePieAdvisorState(state.statsConfig?.advisor || shaped.advisor);
  ...
  function applyPieSessionStateToActive(session = null, options = {}){
    ...
    state.labelPositions = normalizePieLabelPositions(durable.labelPositions);
    state.columnSignature = durable.columnSignature || null;
    state.statsDataModel = cloneSimple(shaped.results?.statsDataModel) || null;
    ...
    applyPieStatsConfig(savedStatsConfig, shaped);
    try{
      ...
      renderPieStatsControls(dataModel, { force: true, reason: 'session-state-apply', session: shaped });
    }catch(err){
      pieDebug('Debug: pie stats controls restore after session apply failed', {
    ...
    if(Number.isFinite(Number(shaped.results?.statsSummaryTabIdCounter))){
      pieStatsSummaryTabIdCounter = Math.max(0, Number(shaped.results.statsSummaryTabIdCounter) || 0);
    }
    syncPieRuntimeControlsFromState(durable.controls || {});
    syncPieSessionRefsFromActive(shaped);
    syncPieSessionManagersFromActive(shaped);
    shaped.updatedAt = Date.now();
    return true;
  ...
  function bindPieSessionForTab(tabLike = null, meta = {}, options = {}){
    ...
    bindPieLayoutManagerForSession(session);
  ...
  function setPieSessionStateFromRuntimeRecord(record, meta = {}){
    ...
    session.advisor = normalizePieAdvisorState(record.advisor || {});
    session.results = createDefaultPieResultsState({
      ...(record.results || {}),
      statsDataModel: record.results?.statsDataModel,
      statsPanelModel: record.results?.statsPanelModel || {},
      statsSummaryTabIdCounter: record.results?.statsSummaryTabIdCounter
    });
  ...
  function normalizePieRuntimeControls(source = {}){
    ...
    return {
      ...
      showStatsSummary: Object.prototype.hasOwnProperty.call(src, 'showStatsSummary') ? !!src.showStatsSummary : defaults.showStatsSummary,
  ...
  function syncPieRuntimeControlsFromState(controlSnapshot = {}){
    ...
    const showStatsSummaryInput = getPieNodeById('pieShowStatsSummary');
    if(showStatsSummaryInput && hasControl('showStatsSummary')){
      showStatsSummaryInput.checked = !!controls.showStatsSummary;
    }
  ...
  function syncPieRuntimeControlsFromDom(session = null){
    state.controls = normalizePieRuntimeControls({
      ...
      showStatsSummary: getPieNodeById('pieShowStatsSummary') ? !!getPieNodeById('pieShowStatsSummary').checked : state.controls?.showStatsSummary,
  ...
  function schedulePieViewRefresh(reason, extraOptions){
    ...
    if(normalizedReason !== 'resize' && (!ownerTabId || ownerTabId === activeTabId)){
    ...
    const scheduleOptions = sanitizePieDrawOptions(Object.assign({}, options, {
  ...
  function projectPieTraceBorderStyle(session = null){
    const owner = ensurePieSessionOwnershipShape(session || getActivePieSessionForState());
    const tabId = owner?.tabId || getPieProjectionTabId() || null;
    const svg = getPieNodeById('pieSvg', tabId) || getPieNodeById('pieSvg');
    const controls = normalizePieRuntimeControls(owner?.state?.controls || state.controls || {});
    return !!Shared.visualProjection?.apply?.(svg, {
      component: 'pie',
      channel: 'trace-border',
      tabId,
      attributes: {
        stroke: controls.borderColor || '#ffffff',
        strokeWidth: Math.max(0, Number.parseFloat(controls.borderWidth) || 0)
      }
    });
  }
  ...
  function ensurePieFontEventListener(){
    ...
    global.document.addEventListener('fontControls:styleChanged', event => {
      ...
      schedulePieViewRefresh('font-style-change', { tabId: detail.tabId || null, renderImpact: 'layout' });
    });
  ...
  function stampPieRadialResizePreview(svg, drawSession, drawableFrame){
    if(!svg?.dataset || !drawSession?.tabId || !drawableFrame){
      return;
    }
    const width = Number(drawableFrame.width);
    const height = Number(drawableFrame.height);
    if(!Number.isFinite(width) || !Number.isFinite(height)){
      return;
    }
    svg.dataset.pieResizePreview = 'radial';
    svg.dataset.pieResizePreviewOwnerTabId = String(drawSession.tabId);
    svg.dataset.pieResizePreviewWidth = String(width);
    svg.dataset.pieResizePreviewHeight = String(height);
  }

  function finalizePieRadialResizePreview(){
    const owner = ensurePieSessionOwnershipShape(getActivePieSessionForState());
    const tabId = owner?.tabId || getPieProjectionTabId() || null;
    const svg = getPieNodeById('pieSvg', tabId) || null;
    const plot = getPieNodeById('piePlot', tabId) || null;
    if(!owner || !isPieSessionActive(owner) || !svg || !plot || svg.dataset?.pieResizePreview !== 'radial'
      || String(svg.dataset.pieResizePreviewOwnerTabId || '') !== String(owner.tabId || '')){
      return false;
    }
    const previewWidth = Number(svg.dataset.pieResizePreviewWidth);
    const previewHeight = Number(svg.dataset.pieResizePreviewHeight);
    const currentFrame = resolvePieDrawableFrame(plot);
    if(!Number.isFinite(previewWidth) || !Number.isFinite(previewHeight)
      || Math.abs(previewWidth - Number(currentFrame?.width)) > 0.5
      || Math.abs(previewHeight - Number(currentFrame?.height)) > 0.5){
      return false;
    }
    applyPieSvgDefaults(svg);
    if(fontControls && typeof fontControls.enableForSvg === 'function'){
      fontControls.enableForSvg(svg, { scopeId: 'pie', tabId: owner.tabId });
    }
    if(pie.rehydrateGraphInteractions?.({ tabId: owner.tabId, root: state.root || resolvePieRoot(owner.tabId) }) !== true){
      return false;
    }
    delete svg.dataset.pieResizePreview;
    delete svg.dataset.pieResizePreviewOwnerTabId;
    delete svg.dataset.pieResizePreviewWidth;
    delete svg.dataset.pieResizePreviewHeight;
    primePieStatsComputation({ matrix: getPieStatsDataMatrix(), reason: 'resize-radial-finalize' });
    pieDebug('Debug: pie radial resize preview finalized without replacement', { tabId: owner.tabId });
    return true;
  }
  ...
  function bindPieRadialTitleDragInteraction(title, svg, ownerSession = null){
    const owner = ensurePieSessionOwnershipShape(ownerSession || getActivePieSessionForState());
    if(!title || !svg || !owner || typeof Shared.enableLabelDrag !== 'function'){
      return false;
    }
    if(title.__graphitixPieRadialTitleDragBinding?.svg === svg){
      return true;
    }
    const svgWidth = Number(svg.getAttribute('width')) || Number(svg.viewBox?.baseVal?.width) || 1;
    const svgHeight = Number(svg.getAttribute('height')) || Number(svg.viewBox?.baseVal?.height) || 1;
    Shared.enableLabelDrag(title, svg, {
      onDragEnd: pos => {
        patchPieLabelPosition(owner, 'title', {
          x: pos.x,
          y: pos.y,
          relX: pos.x / svgWidth,
          relY: pos.y / svgHeight
        }, { reason: 'pie-title-position' });
        pieDebug('Debug: pie title position saved', {
          absolute: pos,
          relative: { relX: pos.x / svgWidth, relY: pos.y / svgHeight }
        });
      }
    });
    title.__graphitixPieRadialTitleDragBinding = { svg };
    return true;
  }

  function rehydratePieRadialTitleInteractions(svg, ownerSession = null){
    const title = svg?.querySelector?.('[data-font-role="graphTitle"]') || null;
    if(!title){
      return true;
    }
    return bindPieTitleInlineInteraction(title, ownerSession)
      && bindPieRadialTitleDragInteraction(title, svg, ownerSession);
  }

  function stampPieLegendDragMetrics(legend, metrics = {}){
    if(!legend?.dataset){
      return;
    }
    const values = {
      legendDragOriginX: metrics.originX ?? metrics.reserveOriginX,
      legendDragOriginY: metrics.originY ?? metrics.reserveOriginY,
      legendDragScaleX: metrics.scaleX ?? metrics.reserveScaleX ?? metrics.svgWidth ?? metrics.width,
      legendDragScaleY: metrics.scaleY ?? metrics.reserveScaleY ?? metrics.svgHeight ?? metrics.height
    };
  ...
  function applyPieColorValue(label, value){
    ...
    schedulePieViewRefresh('color-change', { renderImpact: 'paint' });
    return true;
  }
  ...
  function setPieStartAngleFromToolbar(value){
    ...
    schedulePieViewRefresh('trace-start-angle-change', { tabId: session?.tabId || undefined, renderImpact: 'layout' });
  }
  ...
  function showPieTraceFormatControls(target){
    ...
    const toolbarOwnerSession = getPieProjectionSession({ reason: 'pie-trace-toolbar-open' });
    const toolbarOwnerTabId = toolbarOwnerSession?.tabId || getPieProjectionTabId() || null;
    const canEditToolbarOwner = () => !toolbarOwnerSession || isPieSessionActive(toolbarOwnerSession);
    ...
    const commitTraceFill = reason => {
      capturePieSessionStateFromActive(toolbarOwnerSession, {
        reason,
        captureStatsPanel: false
      });
    };
    const commitTraceBorder = reason => {
      if(!canEditToolbarOwner()){
        return false;
      }
      syncPieRuntimeControlsFromDom(toolbarOwnerSession);
      capturePieSessionStateFromActive(toolbarOwnerSession, {
        reason,
        captureStatsPanel: false
      });
      if(projectPieTraceBorderStyle(toolbarOwnerSession)){
        return true;
      }
      schedulePieViewRefresh(reason, { tabId: toolbarOwnerTabId || undefined, renderImpact: 'layout' });
      return false;
    };
    const toolbar = Shared.symbolToolbar.show({
      ...
      fillShape: {
        ...
        onColorInput(value, context){
          const scopedTrace = resolvePieScopedTraceLabel(context, targetTraceLabel);
          if(applyTraceFill(scopedTrace, value)){
            commitTraceFill('pie-trace-fill-input');
          }
        },
        onColorChange(value, context){
          ...
          commitTraceFill('pie-trace-fill-change');
          if(!resolveTraceNodes(scopedTrace).length){
            schedulePieViewRefresh('trace-fill-change', { tabId: toolbarOwnerTabId || undefined, renderImpact: 'paint' });
          }
        }
      },
      border: {
        ...
        onColorInput(value){
          ...
          commitTraceBorder('pie-trace-border-color-input');
        },
        onColorChange(value){
          if(!canEditToolbarOwner()){
            return;
          ...
          commitTraceBorder('pie-trace-border-color-change');
        },
        getWidth(){
        ...
        onWidthChange(value){
          ...
          commitTraceBorder('pie-trace-border-width-change');
        }
      },
      size: {
        enabled: false,
  ...
  function bindPieLegendInteractions(legend, svg, ownerSession = null, metrics = {}){
    const owner = ensurePieSessionOwnershipShape(ownerSession || getActivePieSessionForState());
    const metricDatasetKeys = {
      originX: 'legendDragOriginX',
      reserveOriginX: 'legendDragOriginX',
      originY: 'legendDragOriginY',
      reserveOriginY: 'legendDragOriginY',
      scaleX: 'legendDragScaleX',
      reserveScaleX: 'legendDragScaleX',
      svgWidth: 'legendDragScaleX',
      width: 'legendDragScaleX',
      scaleY: 'legendDragScaleY',
      reserveScaleY: 'legendDragScaleY',
      svgHeight: 'legendDragScaleY',
      height: 'legendDragScaleY'
    };
    const metric = (...keys) => {
      ...
      for(const key of keys){
        const value = Number(legend?.dataset?.[metricDatasetKeys[key]]);
        if(Number.isFinite(value)){
          return value;
        }
      }
    ...
    return Shared.bindLegendDragInteraction?.(legend, svg, {
      owner,
      originX: metric('originX', 'reserveOriginX') ?? 0,
      originY: metric('originY', 'reserveOriginY') ?? 0,
      scaleX: metric('scaleX', 'reserveScaleX', 'svgWidth', 'width'),
      scaleY: metric('scaleY', 'reserveScaleY', 'svgHeight', 'height'),
      positionAnchor: chartStyle.LEGEND_POSITION_ANCHOR,
      deferInitialConstraint: metrics.deferInitialConstraint,
      undoLabel: 'pie-legend',
      onCommit: (position, dragOwner) => {
        patchPieLabelPosition(dragOwner, 'legend', position, { reason: 'pie-legend-position' });
        if(!dragOwner || isPieSessionActive(dragOwner)){
          capturePieSessionStateFromActive(dragOwner || getPieProjectionSession({ reason: 'pie-legend-position-capture' }), {
  ...
  function constrainPieLegendPreview(legend, svg){
    // Resize previews must remain live, but they are not a user legend move and
    // therefore must not persist a position or bind another drag controller.
    return Shared.constrainLegendPositionToViewport?.(legend, svg)?.changed === true;
  }

  function resolvePieRadialLegendDefaultY(contentTop, contentHeight, legendLayout){
    const top = Number.isFinite(Number(contentTop)) ? Number(contentTop) : 0;
    const height = Math.max(0, Number(contentHeight) || 0);
    const legendHeight = Math.max(0, Number(legendLayout?.renderer?.height) || 0);
    return top + Math.max(0, (height - legendHeight) / 2);
  }

  function drawPieLegend(svg, legendLayout, defaults = {}, svgDimensions = {}, ownerSession = null){
    ...
    const position = chartStyle.resolveLegendPosition(stored?.legend, {
      defaultX: Number.isFinite(defaults.x) ? defaults.x : 0,
      defaultY: Number.isFinite(defaults.y) ? defaults.y : 0,
    ...
    if(!state.resizeState?.active){
      bindPieLegendInteractions(legendGroup, svg, ownerSession, {
  ...
  function syncPieAspectControls(reason){
    ...
    try{
      ...
      const activeTabId = normalizePieSessionTabId(getPieProjectionTabId() || null, { reason: 'pie-aspect-owner' });
      const activeTab = activeTabId
        ? global.Main?.session?.workspaceState?.tabs?.find?.(item => String(item?.id || '') === String(activeTabId))
        : null;
      const payloadChartType = String(activeTab?.payload?.config?.chartType || '').trim().toLowerCase();
      const stateChartType = normalizePieRuntimeControls(state.controls || {}).chartType;
      if(activeTab?.loadedFromArchive === true
        && activeTab.userModified !== true
        && payloadChartType
        && payloadChartType !== stateChartType){
        return;
      }
      ...
      if(lockRatioCheckbox){
        ...
        if(shouldEnforceLockRatio){
          ...
          const targetValue = restoreValue !== null
            ? restoreValue
            : (reason === 'chart-type-change' ? false : savedAspectLock);
          if(targetValue !== null){
  ...
  const bindPieControlHandler = Shared.componentLifecycle.createOwnerControlBinder({
    componentKey: 'pie',
    resolveOwner: (event, meta) => getPieSessionForEvent(event, { reason: meta?.reason }, { create: true }),
    isOwnerActive: session => !session?.tabId || isPieSessionActive(session)
  });
  ...
  function getPieAdvisorState(session = null){
    const owner = ensurePieSessionOwnershipShape(session || getActivePieSessionForState());
    if(owner){
      owner.advisor = normalizePieAdvisorState(owner.advisor || {});
      if(isPieSessionActive(owner)){
        const stats = getPieStatsConfig();
        stats.advisor = owner.advisor;
      }
      return owner.advisor;
    }
    const stats = getPieStatsConfig();
    stats.advisor = normalizePieAdvisorState(stats.advisor || {});
    return stats.advisor;
  }

  function persistPieStatsTabState(reason, session = null){
    const owner = ensurePieSessionOwnershipShape(session || getActivePieSessionForState());
    const tabId = String(owner?.tabId || getPieProjectionTabId() || '').trim();
    if(!tabId){
      return false;
    }
    return Shared.componentLifecycle?.persistOwnedUserState?.(
      'pie',
      { tabId, session: owner },
      { reason: reason || 'pie-stats-state-change' }
    ) === true;
  }
  ...
  function rememberPieStatsState(reason, options = {}){
    ...
    const session = ensurePieSessionOwnershipShape(options.session || getActivePieSessionForState());
    if(!session){
      return null;
    }
    const captured = capturePieSessionStateFromActive(session, {
      reason: reason || 'pie-stats-state',
      captureStats: true,
      syncControls: options.syncControls !== false
    });
    if(options.persist !== false){
      persistPieStatsTabState(reason || 'pie-stats-state', session);
    }
    return captured;
  }
  ...
  function getPieCorrectionOptions(){
    const resolver = Shared.stats && typeof Shared.stats.getCorrectionMeta === 'function'
      ...
      : null;
    return PIE_STATS_CORRECTION_KEYS.map(key => {
      if(resolver){
  ...
  function sanitizePieStatsCorrection(value){
    return PIE_STATS_CORRECTION_KEY_SET.has(value) ? value : PIE_STATS_DEFAULT_CORRECTION;
  }
  ...
  function formatPiePExpression(value){
    ...
    const display = String(formatPiePValue(value));
  ...
  function pieStatsPanelHasRenderedResults(session = null){
    const context = resolvePieStatsOwnerContext(session);
  ...
  function ensurePieStatsSelections(dataModel, statsConfig = getPieStatsConfig()){
    const stats = statsConfig && typeof statsConfig === 'object' ? statsConfig : getPieStatsConfig();
    ...
    if(available.length === 1){
      stats.expectedColumn = null;
    }else if(!availableSet.has(stats.expectedColumn)){
  ...
  function requestPieStatsContextRefresh(reason, options = {}){
    ...
    rememberPieStatsState(reason || 'pie-stats-context-refresh', { syncControls: false, session: options.session || null });
    if(pieDebugEnabled()){
  ...
  function primePieStatsComputation(options = {}){
    const statsContext = resolvePieStatsOwnerContext(getActivePieSessionForState());
  ...
  function ensurePieAdvisorDefaults(context, session = null){
    const advisor = getPieAdvisorState(session);
  ...
  function renderPieStatsAdvisor(dataModel, controls, session = null){
    if(!controls){
      return;
    }
    const advisorSession = ensurePieSessionOwnershipShape(session || getActivePieSessionForState());
    if(advisorSession?.tabId && !isPieSessionActive(advisorSession)){
      return;
    }
    const isAdvisorOwnerCurrent = () => !advisorSession || isPieSessionActive(advisorSession);
    const advisorState = getPieAdvisorState(advisorSession);
    const context = buildPieAdvisorContext(dataModel);
    const answers = ensurePieAdvisorDefaults(context, advisorSession);
    ...
    toggle.addEventListener('click', () => {
      ...
      if(advisorState.open && !advisorState.activated){ advisorState.activated = true; }
      if(pieDebugEnabled()){
      ...
      renderPieStatsControls(dataModel, { force: true, reason: 'advisor-toggle', session: advisorSession });
    });
    ...
    if(advisorState.open){
      ...
      questions.forEach(question => {
        ...
        (question.options || []).forEach(opt => {
          ...
          input.addEventListener('change', () => {
            ...
            rememberPieStatsState('pie-advisor-answer-change', { syncControls: false, session: advisorSession });
            renderPieStatsControls(dataModel, { force: true, reason: 'advisor-answer-change', session: advisorSession });
          });
      ...
      applyBtn.addEventListener('click', () => {
        if(!isAdvisorOwnerCurrent() || !recommendation.ready || !recommendation.apply){ return; }
        const stats = getPieStatsConfig();
        ...
        if(stats.scope === 'gof'){
          ...
          if(Number.isInteger(observed)){ stats.valueColumn = observed; }
          if(Number.isInteger(expected)){ stats.expectedColumn = expected; }
        }else if(stats.scope === 'reference'){
          const selected = Array.from(stats.selectedCols || []).sort((a, b) => a - b);
          if(!selected.includes(stats.referenceColumn)){ stats.referenceColumn = selected[0] ?? null; }
        }
        if(pieDebugEnabled()){
        ...
        rememberPieStatsState('pie-advisor-apply', { syncControls: false, session: advisorSession });
        renderPieStatsControls(dataModel, { force: true, reason: 'advisor-apply', session: advisorSession });
      ...
      resetBtn.addEventListener('click', () => {
        ...
        if(advisorSession){ advisorSession.advisor = advisorState; }
        if(pieDebugEnabled()){
          pieDebug('Debug: pie statsAdvisor reset');
        }
        rememberPieStatsState('pie-advisor-reset', { syncControls: false, session: advisorSession });
        renderPieStatsControls(dataModel, { force: true, reason: 'advisor-reset', session: advisorSession });
      });
  ...
  function buildPieGofDataset(dataModel, observedIndex, expectedIndex){
    ...
    if(equalProportions && observed.length){
      const expectedPerCategory = observed.reduce((sum, value) => sum + value, 0) / observed.length;
      observed.forEach(() => expected.push(expectedPerCategory));
    }
    return {
      observed,
      expected,
      labels,
      skipped,
      expectedSource: equalProportions ? 'equal-proportions' : 'column'
    };
  }
  ...
  function buildPiePlotStatsLines(dataModel, statsConfig = getPieStatsConfig()){
    const stats = statsConfig && typeof statsConfig === 'object' ? statsConfig : getPieStatsConfig();
    const scope = sanitizePieStatsScope(stats.scope);
    if(scope === 'gof'){
      const observedIndex = Number(stats.valueColumn);
      if(!Number.isInteger(observedIndex)){
        return [];
      }
      const dataset = buildPieGofDataset(dataModel, observedIndex, stats.expectedColumn);
      const result = computePieGofStats(dataset.observed, dataset.expected, { method: stats.test });
      if(!result.ok){ return []; }
      const test = result.method === 'g-test' ? 'G' : 'χ²';
      return [`${test}(${result.df}) = ${formatPieStatNumber(result.statistic, 2)}; ${formatPiePExpression(result.pValue)}; Cohen's w = ${formatPieStatNumber(result.cohensW, 2)}`];
    }
    const selected = Array.from(stats.selectedCols || []).map(Number).filter(Number.isInteger).sort((a, b) => a - b);
    if(selected.length < 2){
      return [];
    }
    const dataset = buildPieContingencyDataset(dataModel, selected);
    ...
    if(!result.ok){ return []; }
    const test = result.method === 'g-test' ? 'G' : 'χ²';
    return [`${test}(${result.df}) = ${formatPieStatNumber(result.statistic, 2)}; ${formatPiePExpression(result.pValue)}; Cramer's V = ${formatPieStatNumber(result.cramersV, 2)}`];
  }

  function renderPiePlotStatsAnnotation(svg, data, controls, geometry = {}, drawSession = null){
    if(!controls?.showStatsSummary || !svg){
      return null;
    }
    const dataModel = buildPieStatsDataModel(data);
    const ownerStatsConfig = clonePieStatsConfigForSession(
      drawSession
        ? composePieStatsConfig(drawSession.state?.stats || {}, drawSession.advisor || {}, drawSession.results?.statsPanelModel || {})
        : getPieStatsConfig()
    );
    ensurePieStatsSelections(dataModel, ownerStatsConfig);
    const lines = buildPiePlotStatsLines(dataModel, ownerStatsConfig);
    if(!lines.length){
      return null;
    }
    const statsFontStyles = exportFontStyles('pie', { tabId: drawSession?.tabId || getPieProjectionTabId() || null });
    const fontSize = chartStyle.resolveStatsAnnotationFontMetrics(Number(geometry.fontSize) || 12, { styles: statsFontStyles }).fontSizePx;
    const left = Number(geometry.left) || 0;
    const top = Number(geometry.top) || 0;
    const width = Math.max(20, Number(geometry.width) || Number(svg.getAttribute('width')) || 100);
    const height = Math.max(20, Number(geometry.height) || Number(svg.getAttribute('height')) || 100);
    const statsFrame = { originX: left, originY: top, width, height };
    const position = chartStyle.resolveStatsAnnotationPosition(
      normalizePieLabelPositions(drawSession?.state?.labelPositions || state.labelPositions).stats,
      { x: left + width - 4, y: top + fontSize },
      statsFrame
    );
    return chartStyle.renderStatsAnnotation(svg, {
      lines,
      x: position.x,
      y: position.y,
      fontSize,
      textAnchor: 'end',
      dataAttributes: { 'pie-plot-stats': '1' },
      fontScopeId: 'pie',
      tabId: drawSession?.tabId || null,
      onDragEnd: pos => {
        patchPieLabelPosition(
          drawSession,
          'stats',
          chartStyle.captureStatsAnnotationPosition(pos, statsFrame),
          { reason: 'pie-stats-position' }
        );
      }
    }) || null;
  }

  function buildPieFigureSummary(model, statsConfig = {}, options = {}){
    ...
    const analysisRows = [{
      label: 'Analysis',
      value: scope === 'gof'
        ? `${summary.testLabel} · ${summary.df} df · ${String(model.expectedSource || 'specified expectations') === 'equal-proportions' ? 'equal expected proportions' : 'specified expected counts'}`
        : `${summary.testLabel} · ${summary.df} df · ${Array.isArray(model.pairs) ? model.pairs.length : 0} configured pairwise comparison${Array.isArray(model.pairs) && model.pairs.length === 1 ? '' : 's'}`
    }];
    if(Number.isFinite(alpha)){
      analysisRows.push({ label:'Decision threshold', value:`α = ${formatPieStatNumber(alpha, 3)}` });
    }

    const resultRows = [{
      label: scope === 'gof' ? 'Overall fit' : 'Overall association',
      valueParts: [
        ...
        summary.effectValue && summary.effectValue !== 'N/A'
          ? `; ${scope === 'gof' ? "Cohen's w" : "Cramér's V"} = ${summary.effectValue}`
          : ''
      ]
    }];

    const pairs = Array.isArray(model.pairs) ? model.pairs : [];
    if(scope !== 'gof' && pairs.length){
      const inference = model.pairInferenceSpec || {};
      const method = String(inference.method || 'none').toLowerCase();
      const correctionMeta = Shared.stats?.getCorrectionMeta?.(method) || null;
      const correctionName = correctionMeta?.label || correctionMeta?.shortLabel || method;
      if(pairs.length === 1){
        analysisRows.push({ label:'Multiplicity', value:'One pairwise comparison; no multiplicity adjustment required.' });
      }else if(method === 'none'){
        analysisRows.push({ label:'Multiplicity', value:`${pairs.length} pairwise comparisons; pairwise p-values are unadjusted.` });
      }else if(inference.criterion === 'fdr'){
        analysisRows.push({
          label:'Multiplicity',
          value:`${correctionName} across ${pairs.length} pairwise comparisons; target FDR = ${Number.isFinite(targetFdr) ? formatPieStatNumber(targetFdr, 3) : 'configured value'}.`
        });
      }else{
        analysisRows.push({
          label:'Multiplicity',
          value:`${correctionName} across ${pairs.length} pairwise comparisons${Number.isFinite(alpha) ? `; family-wise α = ${formatPieStatNumber(alpha, 3)}` : ''}.`
        });
      }
      ...
      pairs.forEach((row, index) => {
        ...
        const parts = [
          `${statisticSymbol}(${row.df}) = ${row.statistic}`,
          '; raw p = ',
        ...
        if(Number.isFinite(Number(row.total))) parts.push(`; N = ${row.total}`);
        resultRows.push({ label, valueParts:parts });
      });
    }

    const diagnosticRows = [];
    const footnotes = Array.isArray(summary.footnotes) ? summary.footnotes : [];
    const sparseNote = footnotes.find(note => /expected count/i.test(String(note || '')));
    if(sparseNote){ diagnosticRows.push({ label:'Expected counts', value:String(sparseNote) }); }
    const overallYates = footnotes.find(note => /Yates/i.test(String(note || '')));
    const pairYates = Array.isArray(model.pairFootnotes)
      ? model.pairFootnotes.find(note => /Yates/i.test(String(note || '')))
      : null;
    if(overallYates || pairYates){
      diagnosticRows.push({ label:'Continuity correction', value:String(overallYates || pairYates) });
    }
  ...
  function renderPieStatsModel(model){
    ...
    const summaryRows = [
      ...
      ...(Number.isFinite(Number(model.summary.total)) ? [{ metric: 'N', value: model.summary.total }] : []),
      {
        metric: 'p-value',
        value: {
          type: 'pValue',
  ...
  function handlePieStatsComputeClick(event){
    const statsSession = bindPieStatsEventTarget(event?.currentTarget || event?.target || null, 'pie-stats-compute-event')
      || getActivePieSessionForState();
    const stats = getPieStatsConfig();
    const dataModel = state.statsDataModel || buildPieStatsDataModel(getPieStatsDataMatrix());
    state.statsDataModel = dataModel;
    ...
    try{
      if(stats.scope === 'gof'){
        ...
        const expectedMeta = findPieColumn(dataModel, stats.expectedColumn);
        const usesEqualProportions = Array.isArray(dataModel.columns) && dataModel.columns.length === 1;
        if(!observedMeta || (!usesEqualProportions && !expectedMeta)){
          clearPieStatsOutputs('Select observed and expected columns.');
          setPieStatsStatus('Statistics ready to calculate.');
          updatePieStatsButtonState({ disabled: false, label: 'Calculate statistics' });
          return;
        }
        const dataset = buildPieGofDataset(dataModel, observedColumn, stats.expectedColumn);
        ...
        renderedModel = {
          summary: {
            ...
            df: String(gof.df),
            total: formatPieStatNumber(gof.total, 0),
        ...
        renderedModel = {
          summary: {
            ...
            total: formatPieStatNumber(overall.total, 0),
      ...
      if(Shared.statsReporting && typeof Shared.statsReporting.appendReportPanel === 'function'){
        ...
        const reportMethods = [
          ...
          stats.scope === 'gof'
            ? (renderedModel.expectedSource === 'equal-proportions'
              ? 'Observed counts were compared with equal expected proportions across retained categories.'
        ...
        Shared.statsReporting.appendReportPanel(getPieNodeById('pieStatsResults'), {
          ...
          figureSummary: buildPieFigureSummary(renderedModel, stats, { alpha, targetFdr }),
          resultsParts: [
      ...
      updatePieStatsButtonState({ disabled: false, label: 'Calculate statistics' });
      rememberPieStatsState('pie-stats-compute-failed', { syncControls: false, session: statsSession });
    }
  }

  function renderPieStatsControls(dataModel, options = {}){
    const ownerSession = ensurePieSessionOwnershipShape(options.session || getActivePieSessionForState());
    if(ownerSession?.tabId && !isPieSessionActive(ownerSession)){
      return;
    }
    const controls = getPieNodeById('pieStatsControls', ownerSession?.tabId || null);
    if(!controls){
      return;
    }
    const stats = getPieStatsConfig();
    ...
    renderPieStatsAdvisor(dataModel, controls, ownerSession);
    ...
    (dataModel?.columns || []).forEach(column => {
      ...
      input.addEventListener('change', event => {
        ...
        ensurePieStatsSelections(dataModel);
        renderPieStatsControls(dataModel, { force: true, reason: 'selection-change', session: ownerSession });
        requestPieStatsContextRefresh('selection-change', { session: ownerSession });
      });
    ...
    scopeSelect.addEventListener('change', event => {
      ...
      renderPieStatsControls(dataModel, { force: true, reason: 'scope-change', session: ownerSession });
      requestPieStatsContextRefresh('scope-change', { session: ownerSession });
    });
    ...
    testSelect.addEventListener('change', event => {
      ...
      requestPieStatsContextRefresh('test-change', { session: ownerSession });
    });
    ...
    if(stats.scope === 'gof'){
      ...
      observedSelect.addEventListener('change', event => {
        ...
        ensurePieStatsSelections(dataModel);
        renderPieStatsControls(dataModel, { force: true, reason: 'gof-observed-change', session: ownerSession });
        requestPieStatsContextRefresh('gof-observed-change', { session: ownerSession });
      });
      ...
      const availableColumns = dataModel?.columns || [];
      if(availableColumns.length === 1){
        const option = document.createElement('option');
        option.value = 'equal-proportions';
        option.textContent = 'Equal proportions';
        expectedSelect.appendChild(option);
        expectedSelect.disabled = true;
        appendRow(leftColumn, 'Expected distribution:', expectedSelect);
      }else{
        availableColumns
          .filter(column => column.index !== stats.valueColumn)
          .forEach(column => {
            const option = document.createElement('option');
            option.value = String(column.index);
            option.textContent = column.label;
        ...
        expectedSelect.addEventListener('change', event => {
          ...
          ensurePieStatsSelections(dataModel);
          renderPieStatsControls(dataModel, { force: true, reason: 'gof-expected-change', session: ownerSession });
          requestPieStatsContextRefresh('gof-expected-change', { session: ownerSession });
        });
      ...
      referenceSelect.addEventListener('change', event => {
        ...
        requestPieStatsContextRefresh('reference-change', { session: ownerSession });
      });
      ...
      for(let i = 0; i < selected.length; i += 1){
        for(let j = i + 1; j < selected.length; j += 1){
          ...
          input.addEventListener('change', event => {
            ...
            renderPieStatsControls(dataModel, { force: true, reason: 'custom-pair-toggle', session: ownerSession });
            requestPieStatsContextRefresh('custom-pair-toggle', { session: ownerSession });
          });
          const label = document.createElement('label');
          label.setAttribute('for', input.id);
    ...
    correctionSelect.addEventListener('change', event => {
      ...
      requestPieStatsContextRefresh('correction-change', { session: ownerSession });
    });
    ...
    sparseInput.addEventListener('change', event => {
      ...
      requestPieStatsContextRefresh('sparse-threshold-change', { session: ownerSession });
    });
    ...
    yatesInput.addEventListener('change', event => {
      ...
      requestPieStatsContextRefresh('yates-change', { session: ownerSession });
    });
  ...
  function resolvePieStatsOwnerContext(session = null){
    const owner = ensurePieSessionOwnershipShape(session || getActivePieSessionForState());
    const canUseLiveProjection = !owner || isPieSessionActive(owner);
    ...
    if(canUseLiveProjection){
      ...
      if(belongsToOwner(ownedRef)){
        ...
        const resolved = getPieNodeById('pieStatsResults', owner?.tabId || null);
  ...
  function exportPieStatsConfig(session = null){
    const context = resolvePieStatsOwnerContext(session);
    const ownerSettings = normalizePieStatsSettings(context.owner?.state?.stats || {});
    const stats = context.canUseLiveProjection ? getPieStatsConfig() : ownerSettings;
    const previousPanel = normalizePieStatsPanelModel(context.owner?.results?.statsPanelModel || {});
    let panelModel = previousPanel;
    if(context.target && Shared.statsReporting && typeof Shared.statsReporting.capturePanelModel === 'function'){
      const captured = normalizePieStatsPanelModel(Shared.statsReporting.capturePanelModel(context.target) || {});
      if(pieStatsPanelModelHasContent(captured)){
        panelModel = captured;
      }
    }
    const settings = normalizePieStatsSettings(stats);
    const advisor = context.canUseLiveProjection
      ? normalizePieAdvisorState(stats.advisor || context.owner?.advisor || {})
      : normalizePieAdvisorState(context.owner?.advisor || {});
    const exported = composePieStatsConfig(settings, advisor, panelModel);
    if(context.owner){
      context.owner.state.stats = settings;
      context.owner.advisor = advisor;
      context.owner.results = createDefaultPieResultsState({
        ...context.owner.results,
        statsDataModel: context.owner.results?.statsDataModel || null,
        statsPanelModel: panelModel,
        statsSummaryTabIdCounter: context.owner.results?.statsSummaryTabIdCounter || 0
      });
      context.owner.updatedAt = Date.now();
      if(context.canUseLiveProjection){
        const liveStats = getPieStatsConfig();
        liveStats.advisor = context.owner.advisor;
      }
    }
    return exported;
  }

  function applyPieStatsConfig(config, session = null){
    const context = resolvePieStatsOwnerContext(session);
    const input = config && typeof config === 'object' ? config : {};
    const settings = normalizePieStatsSettings(input);
    const advisor = normalizePieAdvisorState(input.advisor || {});
    const panelModel = normalizePieStatsPanelModel(input);
    if(context.owner){
      context.owner.state.stats = settings;
      context.owner.advisor = advisor;
      context.owner.results = createDefaultPieResultsState({
    ...
    const stats = getPieStatsConfig();
    stats.scope = settings.scope;
    stats.test = settings.test;
    stats.correction = settings.correction;
    stats.sparseThreshold = settings.sparseThreshold;
    stats.yatesCorrection = settings.yatesCorrection;
    stats.referenceColumn = settings.referenceColumn;
    stats.valueColumn = settings.valueColumn;
    stats.expectedColumn = settings.expectedColumn;
    stats.advancedOpen = settings.advancedOpen;
    stats.resultsTab = settings.resultsTab;
    stats.selectedCols = new Set(settings.selectedCols);
    stats.customPairs = new Set(settings.customPairs);
    stats.advisor = context.owner ? context.owner.advisor : advisor;
    stats.contextSignature = settings.contextSignature;
    stats.lastRunSignature = settings.lastRunSignature;
    stats.controlsSignature = null;
    let restoredResults = false;
    const out = context.target;
    if(out){
      if(pieStatsPanelModelHasContent(panelModel)){
        if(Shared.statsReporting && typeof Shared.statsReporting.restorePanelModel === 'function'){
          Shared.statsReporting.restorePanelModel(out, panelModel, {
            ensureReportHost: () => ensurePieStatsReportHost(out)
          });
        }else{
          out.textContent = '';
        }
        restoredResults = pieStatsPanelHasRenderedResults(context.owner);
      }else{
        clearPieStatsReportHost(out);
        out.innerHTML = '';
      }
    }
    stats.restorePending = (restoredResults || pieStatsPanelModelHasContent(panelModel)) && !!settings.lastRunSignature
      ? {
          contextSignature: settings.contextSignature,
          lastRunSignature: settings.lastRunSignature,
          hasResults: true,
  ...
  function initHot(){
    ...
    const schedulePieDrawProxy = () => {
      ...
      scheduleActivePieDraw({ reason: 'pie-table-edit', renderImpact: 'analysis' });
  ...
  function ensurePieDataViewsForHot(hotInstance, options = {}){
    ...
    const manager = Shared.componentLifecycle?.ensureOwnedDataViewsManager?.({
      ...
      createOptions: {
        ...
        onViewsChanged(meta){
          ...
          if(state.applyingPayload || !reason || reason === 'initialize' || reason === 'deserialize'){
            return;
          }
          const session = getPieSessionForHot(hotInstance, {
            reason: 'pie-data-view-persistence'
          }, {
            create: false,
            fallbackActive: false
          });
          if(session){
            markPiePayloadDirtyForSession(session, `pie-data-view-${reason}`);
          }
        },
        ...
        onInteraction(){
          if(isPieSessionActive(getPieSessionForHot(hotInstance, { reason: 'pie-dataview-interaction' }, { create: false }))){
    ...
    if(isPieSessionActive(currentOwnerSession)){
  ...
  function syncPieActiveDataViewFromHot(hotInstance, reason){
    ...
    if(ownerSession && !isPieSessionActive(ownerSession)){
  ...
  function initControls(){
    ...
    const pieShowStatsSummary = getPieNodeById('pieShowStatsSummary');
    ...
    [pieShowPercents, pieStartAngle, pieFontSize, pieChartType].filter(Boolean).forEach(el => {
      el.addEventListener('input', event => {
        ...
        runPieControlOwner(event, reason, session => {
          ...
          if(el === pieChartType){
            const previousChartType = normalizePieRuntimeControls({
              chartType: state.controls?.chartType || 'pie'
            }).chartType;
            const nextChartType = normalizePieRuntimeControls({
              chartType: pieChartType.value
            }).chartType;
            if(previousChartType === 'stacked' && nextChartType !== 'stacked'){
              const lockRatioCheckbox = getPieLockRatioCheckbox();
              if(lockRatioCheckbox && !lockRatioCheckbox.disabled){
          ...
          const drawOptions = el === pieChartType
            ...
            : { tabId: session?.tabId || undefined, renderImpact: 'layout' };
    ...
    if(pieShowLegendInput){
      ...
      pieShowLegendInput.addEventListener('change', event => {
        runPieControlOwner(event, 'legend-toggle', session => {
          ...
          ensurePieLegendControlPlacement();
          syncPieRuntimeControlsFromDom(session);
          capturePieSessionStateFromActive(session, { reason: 'pie-legend-toggle', captureStats: false });
          Shared.componentLifecycle?.persistOwnedUserState?.('pie', session, { reason: 'pie-legend-toggle' });
          schedulePieViewRefresh('legend-toggle', { tabId: session?.tabId || undefined, userInitiated: true, renderImpact: 'layout' });
        });
      });
    }

    if(pieShowStatsSummary){
      pieShowStatsSummary.addEventListener('change', event => {
        runPieControlOwner(event, 'stats-summary-toggle', session => {
          syncPieRuntimeControlsFromDom(session);
          schedulePieViewRefresh('stats-summary-toggle', { tabId: session?.tabId || undefined, renderImpact: 'layout' });
        });
      });
    }
    ...
    if(pieShowFrame){
      pieShowFrame.addEventListener('change', event => {
        runPieControlOwner(event, 'frame-toggle', session => {
          ...
          syncPieRuntimeControlsFromDom(session);
          schedulePieViewRefresh('frame-toggle', { tabId: session?.tabId || undefined, renderImpact: 'paint' });
        });
      });
    }
    ...
    if(pieBorderColor){
      pieBorderColor.addEventListener('input', event => {
        runPieControlOwner(event, 'border-color-change', session => {
          ...
          syncPieRuntimeControlsFromDom(session);
          capturePieSessionStateFromActive(session, { reason: 'border-color-change', captureStatsPanel: false });
          if(!projectPieTraceBorderStyle(session)){
            schedulePieViewRefresh('border-color-change', { tabId: session?.tabId || undefined, renderImpact: 'paint' });
          }
        });
      });
    }
    ...
    if(pieBorderWidth){
      pieBorderWidth.addEventListener('input', event => {
        runPieControlOwner(event, 'border-width-change', session => {
          ...
          syncPieRuntimeControlsFromDom(session);
          capturePieSessionStateFromActive(session, { reason: 'border-width-change', captureStatsPanel: false });
          if(!projectPieTraceBorderStyle(session)){
            schedulePieViewRefresh('border-width-change', { tabId: session?.tabId || undefined, renderImpact: 'layout' });
          }
        });
      });
    }
    ...
    const loadExampleData = event => {
      runPieControlOwner(event, 'pie-example-load', session => {
        ...
        state.statsConfig = createDefaultPieStatsConfig();
        state.statsConfig.scope = sanitizePieStatsScope(exampleRecord?.meta?.preferredStatsScope);
        ...
        pieDebug('biomedical pie comparison example loaded', { categories: example.length - 1 });
        capturePieSessionStateFromActive(session, { reason: 'pie-example-load', captureStats: false });
        schedulePieDrawForSession(session, { reason: 'pie-example-load', renderImpact: 'structural', tabId: session?.tabId || undefined });
      });
    };
    getPieNodeById('pieLoadExample').addEventListener('click', loadExampleData);
    const pieImportBtn=getPieNodeById('pieImport');
    const pieFileInput=getPieNodeById('pieFile');
    const openImportPicker = () => {
      if(!pieFileInput || typeof pieFileInput.click !== 'function'){
        return false;
      }
      pieFileInput.value = '';
      pieFileInput.click();
    ...
    bindPieControlHandler(pieImportBtn, 'click', 'import-table', openImportPicker);
    bindPieControlHandler(pieFileInput, 'change', 'import-file', async (_event, ownerSession)=>{
      ...
      try{
        const applyPiePrismStyle = style => {
          ...
          schedulePieDrawForSession(ownerSession || getActivePieSessionForState(), { force: true, reason: 'import-prism-style', renderImpact: 'layout', tabId: ownerSession?.tabId || undefined });
        };
        const result = await tableImport.openFile(pieFileInput,{
          ...
          scheduleDraw: options => schedulePieDrawForSession(ownerSession || getActivePieSessionForState(), { ...(options || {}), reason: options?.reason || 'pie-import-load', renderImpact: 'structural', tabId: ownerSession?.tabId || undefined }),
          debugLabel: 'pie',
    ...
    if (Shared.exporter && typeof Shared.exporter.mountSvgControls === 'function') {
      Shared.exporter.mountSvgControls({
        container: getPieNodeById('pieExportControls'),
        getSvg: () => getPieNodeById('pieSvg'),
        fileName: 'pie',
    ...
    function getPayload(){
      ...
      const payloadSourceData = Shared.dataViews?.resolveRawDataForPersistence?.(dataViewsPayload, liveTableData)
        || liveTableData;
      const payloadSession = getPieProjectionSession({ reason: 'pie-payload-owner' }) || getActivePieSessionForState();
      const payload = {
        type:'pie',
        ...
        config: collectConfig(payloadSession)
      };
        ...
        if(targetSession){
          ...
          const snapshot = {
            state: {
              ...
              stats: normalizePieStatsSettings(durable.stats),
              colorSignature: durable.colorSignature || null,
              xTickRotateVertical: durable.xTickRotateVertical === true,
              lockRatioEnforcePrevious: durable.lockRatioEnforcePrevious,
              resizeState: cloneSimple(durable.resizeState) || null,
              controls: cloneSimple(durable.controls) || createDefaultPieRuntimeControls()
            },
            advisor: normalizePieAdvisorState(targetSession.advisor),
            results: {
              statsDataModel: cloneSimple(results.statsDataModel) || null,
              statsPanelModel: normalizePieStatsPanelModel(results.statsPanelModel),
              statsSummaryTabIdCounter: Number(results.statsSummaryTabIdCounter) || 0
            },
            notes: createDefaultPieNotesState(targetSession.notes || {}),
            reason: meta?.reason || 'pie-runtime-capture-inactive'
          };
          rememberPieOwnedRuntimeRecord(targetTabId, snapshot, { ...(meta || {}), tabId: targetTabId, reason: snapshot.reason });
          return Shared.componentLifecycle?.rememberComponentRuntimeSnapshot?.(pie, snapshot, {
            ...(meta || {}), tabId: targetTabId, reason: snapshot.reason
          }) || snapshot;
        }
      }
      const captureSession = targetTabId
        ? getPieSession(targetTabId, { ...(meta || {}), tabId: targetTabId, reason: 'pie-runtime-capture-stats' }, { create: false })
        : getActivePieSessionForState();
      syncPieRuntimeControlsFromDom(captureSession);
      const notesSnapshot = capturePieNotesMirror();
      if(captureSession){ exportPieStatsConfig(captureSession); }
      capturePieSessionStateFromActive(captureSession, { reason: meta?.reason || 'pie-runtime-capture', captureStats: false, syncControls: false });
      const owner = ensurePieSessionOwnershipShape(captureSession || getActivePieSessionForState());
      const durable = createDefaultPieDurableState(owner?.state || {});
      const results = createDefaultPieResultsState(owner?.results || {});
      const snapshot = {
        state: {
          titleText: durable.titleText,
          legendWidth: durable.legendWidth,
        ...
        advisor: normalizePieAdvisorState(owner?.advisor),
        results: {
          statsDataModel: cloneSimple(results.statsDataModel) || null,
          statsPanelModel: normalizePieStatsPanelModel(results.statsPanelModel),
          statsSummaryTabIdCounter: Number(results.statsSummaryTabIdCounter) || 0
        },
        notes: createDefaultPieNotesState(notesSnapshot),
        reason: meta?.reason || 'pie-runtime-capture'
      };
      setPieSessionStateFromRuntimeRecord(snapshot, { ...(meta || {}), reason: snapshot.reason });
      syncPieSessionRefsFromActive(owner);
      syncPieSessionManagersFromActive(owner);
      rememberPieOwnedRuntimeRecord(meta?.tab || meta?.tabId || null, snapshot, { ...(meta || {}), reason: snapshot.reason });
      return Shared.componentLifecycle?.rememberComponentRuntimeSnapshot?.(pie, snapshot, { ...(meta || {}), reason: snapshot.reason }) || snapshot;
    };;

    pie.applyRuntimeState = function applyPieRuntimeState(snapshot, meta = {}){
      const requestedTabId = normalizePieSessionTabId(meta?.tab || meta?.tabId || getPieProjectionTabId() || null, meta);
      const requestedSession = requestedTabId
        ? getPieSession(requestedTabId, { ...(meta || {}), tabId: requestedTabId, reason: meta?.reason || 'pie-runtime-apply-owner' }, { create: true })
        : getActivePieSessionForState();
      ...
      // Runtime state belongs to the requested owner even when that tab is detached. Store
      // it first, then project only for the active owner or the tab currently being activated.
      const applySession = setPieSessionStateFromRuntimeRecord(snapshot, {
        ...(meta || {}),
        tabId: requestedSession?.tabId || requestedTabId || meta?.tabId || null,
        reason: meta?.reason || 'pie-runtime-apply-session'
      }) || requestedSession;
      const canProjectOwner = !applySession
        || isPieSessionActive(applySession)
        || isPieSessionActivationTarget(applySession);
      if(applySession && !canProjectOwner){
        rememberPieOwnedRuntimeRecord(applySession.tabId, snapshot, {
          ...(meta || {}),
          tabId: applySession.tabId,
          reason: meta?.reason || 'pie-runtime-apply-inactive'
        });
        ...
        pieDebug('Debug: pie inactive runtime snapshot stored without active projection', {
          tabId: applySession.tabId || null,
          activeTabId: getPieProjectionTabId() || null,
      ...
      if(applySession?.tabId){
        bindPieSessionForTab(applySession.tabId, {
          ...(meta || {}),
          tabId: applySession.tabId,
          reason: meta?.reason || 'pie-runtime-apply-bind'
        }, { apply: false });
      }
      if(snapshot.state && typeof snapshot.state === 'object'){
        ...
        state.labelPositions = normalizePieLabelPositions(nextState.labelPositions);
        ...
        if(Object.prototype.hasOwnProperty.call(snapshot.results || {}, 'statsDataModel')){
          state.statsDataModel = cloneSimple(snapshot.results.statsDataModel);
        }
        if(Object.prototype.hasOwnProperty.call(nextState, 'stats')){
          const combined = composePieStatsConfig(nextState.stats || {}, snapshot.advisor || {}, snapshot.results?.statsPanelModel || {});
          applyPieStatsConfig(combined, applySession);
          try{
            const dataModel = state.statsDataModel || buildPieStatsDataModel(getPieStatsDataMatrix());
            state.statsDataModel = dataModel;
            ensurePieStatsSelections(dataModel);
            renderPieStatsControls(dataModel, { force: true, reason: 'runtime-state-apply', session: applySession });
          }catch(err){
            pieDebug('Debug: pie stats controls restore after runtime apply failed', { message: err?.message || String(err) });
          }
        }
        ...
        if(Object.prototype.hasOwnProperty.call(nextState, 'lockRatioEnforcePrevious')){
          state.lockRatioEnforcePrevious = (nextState.lockRatioEnforcePrevious === true || nextState.lockRatioEnforcePrevious === false)
            ? !!nextState.lockRatioEnforcePrevious
            : null;
          pieLockRatioEnforcePrevious = state.lockRatioEnforcePrevious;
        }
      ...
      pieStatsSummaryTabIdCounter = Number(snapshot.results?.statsSummaryTabIdCounter) || pieStatsSummaryTabIdCounter || 0;
      syncPieSessionRefsFromActive(applySession);
      syncPieSessionManagersFromActive(applySession);
      rememberPieOwnedRuntimeRecord(applySession?.tabId || requestedTabId || null, snapshot, {
        ...(meta || {}),
        tabId: applySession?.tabId || requestedTabId || meta?.tabId || null,
        reason: meta?.reason || 'pie-runtime-apply'
      });
      Shared.componentLifecycle?.rememberComponentRuntimeSnapshot?.(pie, snapshot, {
        ...(meta || {}),
      ...
      pieDebug('Debug: pie runtime snapshot applied', { tabId: applySession?.tabId || requestedTabId || null, title: state.titleText, reason: meta?.reason || 'pie-runtime-apply' });
      return true;
    };;
  ...
  pie.createEmptyPayload = function createEmptyPiePayload(){
      pieDebug('Debug: pie.createEmptyPayload pure factory invoked', {
        ready: !!pie.ready,
    ...
    function applyPiePayload(payload, meta){
      ...
      const canMuteActiveScheduler = hasExplicitScheduleTarget
        ? !!(scheduleTargetSession && isPieSessionActivationTarget(scheduleTargetSession))
        : (!scheduleTargetSession || isPieSessionActivationTarget(scheduleTargetSession));
      ...
      nextControls.showStatsSummary = Object.prototype.hasOwnProperty.call(config, 'showStatsSummary')
        ? !!config.showStatsSummary
        : false;
      state.controls = normalizePieRuntimeControls(nextControls);
      state.lockRatioEnforcePrevious = (config.stackedAspectLocked === true || config.stackedAspectLocked === false)
        ? !!config.stackedAspectLocked
        : null;
      pieLockRatioEnforcePrevious = state.lockRatioEnforcePrevious;
      if(payloadSession?.state){
        payloadSession.state.lockRatioEnforcePrevious = state.lockRatioEnforcePrevious;
      }
      ...
      state.labelPositions = normalizePieLabelPositions(config.labelPositions);
      if(!skipDraw){
        scheduleActivePieDraw({ reason: `pie-payload-${source}`, renderImpact: 'structural' });
      }
    ...
    function collectConfig(ownerSession = null){
      const axisSettings = ensureAxisSettings();
      const controls = normalizePieRuntimeControls(ownerSession?.state?.controls || state.controls || {});
      const stackedAspectLocked = controls.chartType === 'stacked'
        ? null
        : getPieLockRatioEnforcePrevious();
      ...
      return {
        ...
        showStatsSummary: !!controls.showStatsSummary,
        ...
        fontStyles: (exportFontStyles('pie', { tabId: ownerSession?.tabId || getPieProjectionTabId() || null }) || undefined),
    ...
    pie.save = async function(){
      ...
      const result = await fileIO.saveGraphFile({
        context: 'pie',
        owner: { component: 'pie', tabId: operationSession?.tabId || getPieProjectionTabId() || null },
    ...
    pie.open = async function(){
      ...
      const operationTabId = operationSession?.tabId || getPieProjectionTabId() || null;
      const result = await fileIO.openGraphFile({
        context: 'pie',
        owner: { component: 'pie', tabId: operationTabId },
        setFileHandle: handle => { setPieFileHandleForSession(handle, operationSession); },
        setFileName: name => { setPieFileNameForSession(name, operationSession); capturePieSessionStateFromActive(operationSession, { reason: 'save-file-name', captureStats: false }); },
        loadFromFile: (file, operation) => pie.loadFromFile(file, { operation, tabId: operationTabId }),
        triggerInput: () => {
    ...
    pie.loadFromFile = function(file, options = {}){
      const ownerTabId = String(options?.tabId || options?.operation?.tabId || getPieProjectionTabId() || '').trim() || null;
      const operation = fileIO?.createGraphOpenOperation?.({
        context: 'pie',
        operation: options?.operation,
        owner: { component: 'pie', tabId: ownerTabId }
      ...
      const apply = payload => {
        const routed = fileIO?.routeGraphOpenPayload?.({
          ...
          operation,
          payload,
          reason: 'pie-graph-file-open',
          apply: (nextPayload, owner) => applyPiePayload(nextPayload, {
            source: 'file',
            tabId: owner?.tabId || ownerTabId || undefined
          })
        });
        const fallbackOwnerIsCurrent = !ownerTabId || String(getPieProjectionTabId() || '') === ownerTabId;
        return routed ? routed.value !== false : (fallbackOwnerIsCurrent && applyPiePayload(payload, {
          source: 'file',
    ...
    getPieNodeById('pieGraphFile').addEventListener('change', e => {
      const file = e.target.files?.[0] || null;
      if(!file){
        return;
      }
      const session = getPieSessionForEvent(e, { reason: 'pie-graph-file-input' }, { create: false })
        || getPieProjectionSession({ reason: 'pie-projection-mutation' });
      const tabId = session?.tabId || getPieProjectionTabId() || null;
      setPieFileNameForSession(file.name, session);
      setPieFileHandleForSession(null, session);
      capturePieSessionStateFromActive(session, { reason: 'file-input-change', captureStats: false });
      pie.loadFromFile(file, { tabId });
    });
  }
  ...
  function updatePieStats(labels, observed, expected){
    try{
      ...
      const finiteExpected = expected.map(Number).filter(Number.isFinite);
      const minExpected = finiteExpected.length ? Math.min(...finiteExpected) : NaN;
      const lowExpectedCount = finiteExpected.filter(value => value < 5).length;
          ...
          figureSummary: {
            ...
            sections:[
              { key:'analysis', label:'', rows:[
                { label:'Analysis', value:`Pearson chi-square goodness-of-fit · ${df} df · ${observed.length} categories · total observed n = ${observed.reduce((sum, value) => sum + (Number(value) || 0), 0)}` },
                { label:'Expected counts', value:'User-supplied expected counts' }
              ] },
  ...
  async function draw(drawOptions = {}){
    ...
    drawOptions = sanitizePieDrawOptions(drawOptions, {
      tabId: drawSession?.tabId || drawOptions?.tabId || getPieProjectionTabId() || null,
      reason: drawOptions?.reason || 'pie-draw-session'
    });
    if(drawSession && !isPieSessionActive(drawSession)){
    ...
    const drawGeneration = bumpPieDrawGeneration(drawSession);
    ...
    // Pie and Donut are explicit exclusions from the Cartesian transaction.
    // Clear only this owner's derived stacked publication before rendering an
    // excluded mode so stale Lock metadata cannot survive a mode transition.
    if(type!=='stacked'){
      Shared.cartesianLayout?.clearPublishedLayout?.(state.svgBox, {
    ...
    const header=data[0]||[];
      ...
      for(let r=1;r<data.length;r++){
        ...
        if(seg){
          ...
          for(let c=1;c<=barHeaders.length;c+=1){
            const v=Shared.dataTransforms.toFiniteNumber(row[c]);
      ...
      // Stacked mode is Cartesian. The canonical SVG/user frame is never
      // widened for a legend; legend visibility is a derived envelope concern.
      ...
      framePublication = Shared.framePublication.stage({
        ...
        canCommit: () => execution?.isCurrent?.() !== false
          && (!drawSession || isPieSessionActive(drawSession))
          && isPieDrawGenerationCurrent(drawSession, drawGeneration)
      });
      ...
      if(!isResizePreview && fontControls && typeof fontControls.enableForSvg === 'function'){
        fontControls.enableForSvg(svg,{ scopeId: 'pie', tabId: drawTabId });
      ...
      const stackedBaseBottom = Math.max(0, Number(yMarginRequirements.baselineMargins.bottom || 0) - stackedAxisTitleReserve);
      let margin = {
        ...yMarginRequirements.baselineMargins,
        bottom: stackedBaseBottom
      };
      let chartWidth=Math.max(20,baseSvgWidth-margin.left-margin.right);
      let chartHeight=Math.max(20,svgHeight-margin.top-margin.bottom);
      const categoryEndpointInset = barHeaders.length ? chartWidth / (barHeaders.length * 2) : 0;
      const categoryMarginRequirements = chartStyle.computeCartesianMarginRequirements({
        fontSize:fs,
        legendWidth:0,
        maxYLabelWidth,
        hasYTitle,
        axisMetrics,
        xTickLabels:barHeaders,
      ...
      const categoryEndpointLeftReserve = Math.max(
        0,
        Number(categoryMarginRequirements.requiredMargins.left || 0) - Number(yMarginRequirements.requiredMargins.left || 0)
      );
      const categoryEndpointRightReserve = Math.max(
        0,
      ...
      const pieCartesianTransaction = pieAspectData.resizerAspectLocked === 'true'
        ...
        : null;
      const lockedPieGeometry = pieAspectData.resizerAspectLocked === 'true'
        ? Shared.cartesianLayout?.resolveLockedRenderGeometry?.({
            userFrame: { width: baseSvgWidth, height: svgHeight },
            transaction: pieCartesianTransaction
          })
        : null;
      if(lockedPieGeometry?.valid === true){
        margin = { ...lockedPieGeometry.margins };
      ...
      const stackedAuxiliaryReserves = [
        {
          ...
          side: 'left',
          amount: categoryEndpointLeftReserve,
          behavior: 'max',
      ...
      let pieCartesianPlan = Shared.cartesianLayout?.planCartesianLayout?.({
        owner: pieLayoutOwner,
        userFrame: { width: baseSvgWidth, height: svgHeight },
        ...
        orientation: 'normal',
        lock: {
          enabled: pieAspectData.resizerAspectLocked === 'true',
          targetRatio: Number(pieAspectData.resizerCartesianPlotRatio) || null,
          drive: pieAspectData.resizerLastAxis === 'x' ? 'width' : (pieAspectData.resizerLastAxis === 'y' ? 'height' : 'both')
        },
      ...
      if(pieCartesianPlan){
        margin = {
          left: pieCartesianPlan.plotRect.x,
          top: pieCartesianPlan.plotRect.y,
          right: baseSvgWidth - pieCartesianPlan.plotRect.x - pieCartesianPlan.plotRect.width,
          bottom: svgHeight - pieCartesianPlan.plotRect.y - pieCartesianPlan.plotRect.height
        };
      ...
      const categoricalChartWidth = chartWidth;
      const yAxisX = categoricalPlotStart;
      const plotRightX = margin.left + chartWidth;
      const stackedContentViewport = chartStyle.stageGraphContentViewport({
        svgBox: state.svgBox,
        plot: plotEl,
        svg,
        baseWidth: baseSvgWidth,
        baseHeight: svgHeight,
        rightWidth: pieCartesianPlan?.contentEnvelope?.extensionRight || stackedLegendWidthForMargin,
        leftWidth: pieCartesianPlan?.contentEnvelope?.extensionLeft || 0,
        topHeight: pieCartesianPlan?.contentEnvelope?.extensionTop || 0,
        bottomHeight: pieCartesianPlan?.contentEnvelope?.extensionBottom || requiredBottomViewportExtension,
        legendWidth: stackedLegendWidthForMargin
      });
      pieDebug('Debug: pie stacked Cartesian layout', {
        owner: pieLayoutOwner,
        margin,
        requiredMargins,
        chartWidth,
        ...
        categoryEndpointLeftReserve,
        categoryEndpointRightReserve,
        requiredBottomViewportExtension,
        envelope: pieCartesianPlan?.contentEnvelope || null
      });
      ...
      const minorTicksY = getAxisMinorTicksEnabled('y')
        ...
        : [];
      const axisControlConfig = axisName => buildPieAxisControlConfig(axisName, drawSession, {
        effectiveTickInterval: axisName === 'y' ? percentScale.step : null
      });
      ...
      if(minorTicksY.length){
        minorTicksY.forEach(value => {
          ...
          tick.setAttribute('data-pie-axis-minor-target','1');
      ...
      for(let j = 0; j < barHeaders.length; j += 1){
        ...
        xTick.setAttribute('data-pie-axis-style-target','1');
      ...
      let stackedLegendGroup = null;
      let stackedLegendMetrics = null;
      if(stackedLegendVisible){
        const legendRenderer = stackedLegendLayout.renderer;
        const defaultLegendX = baseSvgWidth + stackedLegendLayout.legendGapPx;
        const defaultLegendY = margin.top + (legendRenderer.baselineOffset || 0);
        stackedLegendMetrics = {
          width: svgWidth,
          height: svgHeight,
          reserveOriginX: baseSvgWidth,
          reserveOriginY: margin.top,
          reserveScaleX: stackedLegendLayout.legendGapPx,
          reserveScaleY: chartHeight,
          legacyOriginX: plotRightX,
          legacyOriginY: margin.top
        };
        stackedLegendGroup = drawPieLegend(svg, stackedLegendLayout, { x: defaultLegendX, y: defaultLegendY }, stackedLegendMetrics, drawSession);
        if(!stackedLegendGroup){
      ...
      const pieAxisOwnerTabId = drawSession?.tabId || drawTabId || getPieProjectionTabId() || null;
      Shared.visualProjection?.bind?.(
        axis.querySelectorAll('[data-axis-control="1"], [data-frame-edge], [data-pie-axis-style-target="1"]'),
        {
          component: 'pie',
          channel: 'axis',
          tabId: pieAxisOwnerTabId,
          strokeWidthBase: axisStrokeWidthBase,
          renderedStrokeWidth: axisStrokeWidth
        }
      );
      Shared.visualProjection?.bind?.(axis.querySelectorAll('[data-pie-axis-minor-target="1"]'), {
        component: 'pie',
        channel: 'axis',
        tabId: pieAxisOwnerTabId,
        strokeWidthBase: axisStrokeWidthBase,
      ...
      if(!isResizePreview){ bindPieTitleInlineInteraction(title, drawSession); }
      ...
      renderPiePlotStatsAnnotation(svg, data, controls, {
        left: yAxisX,
        top: margin.top,
        width: categoricalChartWidth,
        height: chartHeight,
        fontSize: fs
      }, drawSession);
      Shared.visualProjection?.bind?.(svg.querySelectorAll('[data-pie-trace="1"]'), {
        component: 'pie',
        channel: 'trace-border',
        tabId: execution?.tabId || drawSession?.tabId || drawTabId || drawOptions?.tabId || null,
        strokeWidthBase: borderWidthBase,
        renderedStrokeWidth: borderWidth,
        properties: ['stroke', 'strokeWidth']
      });
      const stackedRenderWidth = Math.max(baseSvgWidth, pieCartesianPlan?.contentEnvelope?.maxX || baseSvgWidth);
      const stackedRenderHeight = Math.max(svgHeight, pieCartesianPlan?.contentEnvelope?.maxY || svgHeight);
      ensureGraphViewport(svg, {
        padding: Math.max(fs, 14),
        debugLabel: 'pie-stacked-graph',
        baseViewport: { width: stackedRenderWidth, height: stackedRenderHeight },
        fitContent: false,
        remeasure: false
      });
      if(!(await checkpoint())){
      ...
      const measuredStackedViewport = stackedContentViewport.measure?.() || stackedContentViewport.getViewport?.() || null;
      if(pieCartesianPlan && measuredStackedViewport){
        pieCartesianPlan = Shared.cartesianLayout.planCartesianLayout({
          owner: pieLayoutOwner,
          ...
          contentBounds: {
            ...
            maxY: measuredStackedViewport.maxY
          },
          rounding: { mode: 'none', precision: 6 }
        });
      }
      const pieLayoutPublished = pieCartesianPlan
        ? Shared.cartesianLayout?.publishCartesianLayout?.(state.svgBox, pieCartesianPlan, {
            tabId: pieLayoutOwner.tabId,
            component: 'pie',
            generation: pieLayoutOwner.generation,
      ...
      if(!pieCartesianPlan){
        ...
        stackedContentViewport.commit();
      }
      if(stackedLegendGroup){
        if(isResizePreview){
          constrainPieLegendPreview(stackedLegendGroup, svg);
        }else{
          bindPieLegendInteractions(stackedLegendGroup, svg, drawSession, {
            ...stackedLegendMetrics,
    ...
    for(let r=1;r<data.length;r+=1){
      ...
      seriesColumnsRaw.forEach(series=>{
        ...
        const numVal=Shared.dataTransforms.toFiniteNumber(rawVal);
    ...
    const horizontalEdgePadding = chartStyle.resolveGraphHorizontalEdgePadding
      ...
      : 8;
    const legendReservedWidth = radialLegendVisible
      ? radialLegendLayout.legendWidthForMargin + horizontalEdgePadding
      : 0;
    ...
    if(centers.length){
      const leftLimit=contentLeft + horizontalEdgePadding;
      const rightLimit=contentRight - horizontalEdgePadding; // keep charts clear of the legend lane
    ...
    markFontEditable(title,'graphTitle','graphTitle');
    if(!isResizePreview){
      bindPieTitleInlineInteraction(title, drawSession);
      bindPieRadialTitleDragInteraction(title, svg, drawSession);
    }
    svg.appendChild(title);
    renderPiePlotStatsAnnotation(svg, data, controls, {
      left: contentLeft,
    ...
    let radialLegendGroup = null;
    let radialLegendMetrics = null;
    if(radialLegendVisible){
      ...
      const defaultLegendY = resolvePieRadialLegendDefaultY(contentTop, contentHeight, radialLegendLayout);
      radialLegendMetrics = {
        width: svgWidth,
        height: svgHeight,
        reserveOriginX: contentRight,
        reserveOriginY: contentTop,
        reserveScaleX: radialLegendLayout.legendGapPx,
        reserveScaleY: contentHeight,
        legacyOriginX: 0,
        legacyOriginY: 0
      };
      radialLegendGroup = drawPieLegend(svg, radialLegendLayout, { x: defaultLegendX, y: defaultLegendY }, radialLegendMetrics, drawSession);
      if(!radialLegendGroup){
    ...
    if(isResizePreview){
      stampPieRadialResizePreview(svg, drawSession, drawableFrame);
      stampPieLegendDragMetrics(radialLegendGroup, radialLegendMetrics || {});
    }
    if(radialLegendGroup){
      if(isResizePreview){
        constrainPieLegendPreview(radialLegendGroup, svg);
      }else{
        bindPieLegendInteractions(radialLegendGroup, svg, drawSession, {
          ...radialLegendMetrics,
          deferInitialConstraint: false
        });
      }
  ...
  pie.draw = async function drawPiePublic(options = {}){
    const drawSession = ensurePieSessionOwnershipShape(getPieSessionForDrawOptions(options, { reason: options?.reason || 'pie-draw' }));
    const normalizedOptions = sanitizePieDrawOptions(options, {
      tabId: drawSession?.tabId || options?.tabId || getPieProjectionTabId() || null,
      reason: options?.reason || 'pie-draw'
    });
    const nextReason = normalizedOptions.reason || 'pie-draw';
    if(drawSession && !isPieSessionActive(drawSession)){
      drawSession.state.drawPending = true;
      drawSession.updatedAt = Date.now();
      return;
    }
    const drawTabId = normalizedOptions.tabId || drawSession?.tabId || getPieProjectionTabId() || null;
    if(Shared.componentLifecycle?.shouldSuppressDraw?.('pie', { ...normalizedOptions, tabId: drawTabId, reason: nextReason })){
      pieDebug('Debug: pie draw suppressed by lifecycle', { reason: nextReason, tabId: drawTabId });
      Shared.componentLifecycle?.emitLifecycleEvent?.({ componentKey: 'pie', tabId: drawTabId, action: 'draw-suppressed', reason: nextReason, details: { source: 'pie.draw', renderImpact: resolvePieRenderImpact(normalizedOptions), viewOnly: isPiePresentationDraw(normalizedOptions) } });
      return;
    }
    Shared.componentLifecycle?.emitLifecycleEvent?.({ componentKey: 'pie', tabId: drawTabId, action: 'draw-executed', reason: nextReason, details: { source: 'pie.draw', renderImpact: resolvePieRenderImpact(normalizedOptions), viewOnly: isPiePresentationDraw(normalizedOptions) } });
    const overlayTabId = drawSession?.tabId || drawTabId || null;
    const overlayRequested = (normalizedOptions.force === true || normalizedOptions.forceOverlay === true) && normalizedOptions.silentOverlay !== true;
  ...
  pie.cancelCurrentDraw = function cancelCurrentDraw(meta = {}){
    ...
    const owner = getPieSession(tabId, { ...(meta || {}), tabId, reason: meta?.reason || 'pie-draw-cancel' }, { create: false })
      || getActivePieSessionForState();
    const drawGeneration = bumpPieDrawGeneration(owner);
    ...
    pieDebug('Debug: pie draw cancelled', { tabId, drawGeneration });
    return true;
  };
  ...
  function initNotes(){
    ...
    if(!stack && diagramArea){
      ...
      if(svgBox){
        ...
        stack.className = 'pie-plot-stack graph-plot-stack';
  ...
  pie.init = function init(options = {}){
    ...
    const schedulePieLayoutDraw = (meta) => {
      ...
      const layoutOptions = meta && typeof meta === 'object' ? meta : {};
      scheduleActivePieDraw({
        ...layoutOptions,
        renderImpact: layoutOptions.renderImpact || 'layout'
      });
    };
    const schedulePieResizeDraw = phase => {
      ...
      if(PIE_RESIZE_FINALIZE_PHASES.has(currentPhase) && finalizePieRadialResizePreview()){
        return;
      }
    ...
    state.layout = Shared.componentLayout?.createStandardPanels({
      componentName: 'pie',
        ...
        resizableBoxOptions: {
          // Only stacked Pie is Cartesian. Resolve capability from the explicit
          // owner session so a sibling Pie/Donut tab cannot affect this resizer.
          cartesianLayoutTransactionEnabled: context => {
            const ownerTabId = String(context?.tabId || '').trim();
            if(!ownerTabId){
              return false;
            }
            const ownerSession = getPieSession(ownerTabId, {
              tabId: ownerTabId,
              reason: 'pie-resizer-cartesian-capability'
            }, { create: false });
            return ownerSession?.state?.controls?.chartType === 'stacked';
          },
          // Pie publishes exact geometry for every live horizontal frame.
          liveViewportLockAxes: ['y'],
          onResize: phase => {
    ...
    const runPieDrawCycle = async (drawOptions = {}) => {
      const drawTabId = drawOptions?.tabId || getPieProjectionTabId() || null;
      const ownerSession = drawTabId
        ? getPieSession(drawTabId, { tabId: drawTabId, reason: 'pie-draw-owner' }, { create: false })
        : getActivePieSessionForState();
      const normalizedOptions = sanitizePieDrawOptions(drawOptions, {
        tabId: ownerSession?.tabId || drawTabId || null,
        reason: drawOptions?.reason || 'pie-draw'
      });
      try{
        return await draw(normalizedOptions);
      }finally{
        pieOverlayController?.resolve({ reason: 'complete', tabId: normalizedOptions?.tabId || drawTabId || null });
        Shared.componentLifecycle?.emitLifecycleEvent?.({
          componentKey: 'pie',
          tabId: normalizedOptions?.tabId || drawTabId || null,
          action: 'draw-settled',
          reason: normalizedOptions?.reason || 'pie-draw',
          phase: 'complete',
          details: {
            source: 'pie.draw',
    ...
    const schedulePieInstrumented = drawOptions => {
      const ownerTabId = drawOptions?.tabId || getPieProjectionTabId() || null;
      const ownerSession = ownerTabId
        ? getPieSession(ownerTabId, { tabId: ownerTabId, reason: 'pie-scheduled-draw-owner' }, { create: false })
        : getActivePieSessionForState();
      const nextOptions = sanitizePieDrawOptions(drawOptions || {}, {
        tabId: ownerSession?.tabId || ownerTabId || null,
        reason: drawOptions?.reason || 'pie-scheduled-draw'
      });
      if(
        nextOptions.silentOverlay !== true
        && (nextOptions.force === true || nextOptions.forceOverlay === true || nextOptions.importTransactionFinal === true)
      ){
    ...
    state.layout?.setScheduleDraw?.(schedulePieLayoutDraw);
      scheduleActivePieDraw({ reason: 'pie-init-complete', renderImpact: 'structural' });
  ...
  pie.hasRenderedGraph = function hasRenderedGraph(meta = {}){
    const root = meta.root
      || Shared.workspaceTabs?.getMountedRoot?.(meta.tab || meta.tabId || null, 'pie')
      || null;
    return Shared.componentLifecycle?.hasRenderableGraphContent?.(root, {
      selectors: ['#piePlot'],
      contentSelectors: ['#pieSvg [data-pie-trace="1"]'],
      allowText: false
    }) === true;
  };
  ...
  function getPieRenderCacheOwner(meta = {}, reason = 'pie-render-cache'){
    ...
    const session = ensurePieSessionOwnershipShape(source.session)
      ...
      || getActivePieSessionForState();
    if(session && !isPieSessionActive(session)){
      if(typeof Shared.isDebugEnabled === 'function' && Shared.isDebugEnabled()){
  ...
  pie.captureRenderCache = function captureRenderCache(meta = {}){
    ...
    // Render cache carries the graph only. Capturing it never changes the live frame.
    ...
    const cache = { plot: plotCache, __graphitixRenderCache: cacheMeta };
  ...
  function bindPieTraceFormatInteraction(node){
    if(!node || node.__graphitixPieTraceFormatBound === true){ return false; }
    node.style.cursor = 'pointer';
    node.addEventListener('click', event => {
      try{ event.stopPropagation(); }catch(_err){}
      showPieTraceFormatControls(event.currentTarget);
    });
    node.__graphitixPieTraceFormatBound = true;
    return true;
  }

  pie.rehydrateGraphInteractions = function rehydrateGraphInteractions(meta = {}){
    const owner = getPieRenderCacheOwner(meta, 'pie-render-cache-interaction-rehydrate');
    const root = meta.root || resolvePieRoot(owner?.tabId || meta.tab || meta.tabId || null);
    const svg = root?.querySelector?.('#pieSvg') || meta.svgs?.find?.(node => node?.id === 'pieSvg') || null;
    if(!owner || !svg){ return false; }
    const axesReady = axisControls?.rehydrateAxisElements?.(svg, (axis, _element, metadata) =>
      buildPieAxisControlConfig(axis, owner, metadata)
    ) !== false;
    const traceMode = svg.querySelector?.('[data-pie-trace="1"]')?.dataset?.pieTraceMode || null;
    const textReady = traceMode === 'stacked'
      ? rehydratePieInlineTextInteractions(svg, owner)
      : rehydratePieRadialTitleInteractions(svg, owner);
    svg.querySelectorAll?.('[data-pie-trace="1"]').forEach(bindPieTraceFormatInteraction);
    bindPieLegendInteractions(
  ...
  pie.restoreRenderCache = function restoreRenderCache(cache, meta = {}){
    ...
    const svg = plot?.querySelector?.('#pieSvg') || null;
    bindPieLegendInteractions(
      svg?.querySelector?.('[data-legend-viewport-content="true"]') || null,
      svg,
      owner
  ...
  pie.__testHooks = Object.assign({}, pie.__testHooks, {
    resolveRenderImpact: (options, fallback) => resolvePieRenderImpact(options, fallback),
    sanitizeDrawOptions: (options, owner) => sanitizePieDrawOptions(options, owner),
    ...
    buildPlotStatsLines: (dataModel, statsConfig) => buildPiePlotStatsLines(dataModel, statsConfig),
    buildFigureSummary: (model, statsConfig, options) => buildPieFigureSummary(model, statsConfig || {}, options || {}),
  ...
  pie.executeDesktopCommand = function executeDesktopCommand(command){
    const action = pie.__desktopCommandActions?.[command];
