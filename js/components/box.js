...
function renderStatsControls(traces){
  ...
  function draw(){
    ...
    const tickFont=chartStyle.makeFont(fs); const tickLabels=yScale.ticks.map(t=>formatTick(logScale?Math.pow(10,t):t)); const tickWidths=tickLabels.map(lbl=>chartStyle.measureText(lbl,tickFont)); const maxTickWidth=Math.max(...tickWidths,0); const yLabelGap=maxTickWidth+fs; const axisLabelFontSize=fs+4; const titleFontSize=fs+6; const axisLabelFont=chartStyle.makeFont(axisLabelFontSize); const yTitleWidth=chartStyle.measureText(state.yLabelText,axisLabelFont); const maxLevelEstimate=state.selectedCols.size>1?state.selectedCols.size:0; const topExtra=maxLevelEstimate?(ANN_BASE_OFFSET+maxLevelEstimate*ANN_LEVEL_GAP):0; let margin=chartStyle.computeBaseMargins({fontSize:fs,maxYLabelWidth:maxTickWidth,yTitleWidth}); margin.top+=topExtra; margin.left=Math.max(margin.left,yLabelGap+fs);
    let plotW=Math.max(20, W - margin.left - margin.right); let plotH=Math.max(20, H - margin.top - margin.bottom); const labelTexts=labelsUsed.map((lab,i)=>lab||`Col ${i+1}`); const bottomLayout=chartStyle.computeBottomLayout({labels:labelTexts,fontSize:fs,plotWidth:plotW,baseBottom:margin.bottom}); margin.bottom=bottomLayout.bottom; plotW=Math.max(20, W - margin.left - margin.right); plotH=Math.max(20, H - margin.top - margin.bottom); console.debug('Debug: box layout',{margin,plotW,plotH,rotate:bottomLayout.shouldRotate}); let bandW=plotW/labelsUsed.length;
    ...
    yScale.ticks.forEach(t => { const y=y2px(t); add('line',{ x1:margin.left - tickLen,y1:y,x2:margin.left,y2:y,stroke:'#000','stroke-width':1 }); const txt=add('text',{ x:margin.left - (tickLen+2), y, 'font-size':fs, 'text-anchor':'end', 'dominant-baseline':'middle', fill:chartStyle.TEXT_COLOR }); txt.textContent=formatTick(logScale?Math.pow(10,t):t); });
    const xAxisY = graphType === 'bar' ? y2px(0) : margin.top + plotH; add('line', { x1: margin.left - tickLen, y1: xAxisY, x2: margin.left + plotW + tickLen, y2: xAxisY, stroke: '#000', 'stroke-width': 1 }); const xLabelOffset=Math.max(14,fs)+tickLen; const xLabels=[]; labelsUsed.forEach((lab,i)=>{ const x=xCenter(i); add('line', { x1:x,y1:xAxisY,x2:x,y2:xAxisY+tickLen,stroke:'#000','stroke-width':1 }); const labelText=lab||`Col ${i+1}`; const t=add('text',{ x, y:xAxisY + xLabelOffset, 'font-size':fs, 'text-anchor':'middle','dominant-baseline':'hanging', fill:chartStyle.TEXT_COLOR }); t.textContent=labelText; t.style.cursor='ew-resize'; enableLabelDrag(t,i); xLabels.push(t); }); chartStyle.applyLabelOrientation(xLabels,{angle:-45,anchor:'end',dy:'0.35em',force:bottomLayout.shouldRotate});
    ...
    const yX = margin.left - (maxTickWidth+fs*1.6); const yText = add('text', { x: yX, y: margin.top + plotH / 2, transform: `rotate(-90 ${yX} ${margin.top + plotH / 2})`, 'text-anchor': 'middle', 'font-size': axisLabelFontSize, 'font-weight':'600', fill:chartStyle.TEXT_COLOR }); yText.textContent = state.yLabelText; makeEditable(yText,txt=>{state.yLabelText=txt;});
    ...
    const titleText = add('text', { x: margin.left + plotW / 2, y: margin.top / 2, 'text-anchor': 'middle', 'font-size': titleFontSize, 'font-weight':'600', fill:chartStyle.TEXT_COLOR }); titleText.textContent = state.titleText; makeEditable(titleText,txt=>{state.titleText=txt;});
