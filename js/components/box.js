...
function renderStatsControls(traces){
  ...
  function draw(){
    ...
    const xAxisY = graphType === 'bar' ? y2px(0) : margin.top + plotH; add('line', { x1: margin.left - tickLen, y1: xAxisY, x2: margin.left + plotW + tickLen, y2: xAxisY, stroke: '#000', 'stroke-width': 1 }); const xLabelOffset=Math.max(14,fs)+tickLen; const xLabels=[]; labelsUsed.forEach((lab,i)=>{ const x=xCenter(i); add('line', { x1:x,y1:xAxisY,x2:x,y2:xAxisY+tickLen,stroke:'#000','stroke-width':1 }); const t=add('text',{ x, y:xAxisY + xLabelOffset, 'font-size':fs, 'text-anchor':'middle', fill:'#000' }); t.textContent=lab||`Col ${i+1}`; t.style.cursor='ew-resize'; enableLabelDrag(t,i); xLabels.push(t); });
