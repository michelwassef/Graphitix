(function(global){
  ...
  function draw(){
    ...
    const defaultFill=els.boxFill.value; const defaultBorder=els.boxBorder.value; const bw=Number(els.boxBorderWidth.value); const fs=Number(els.boxFontSize.value);
    ...
    const svg=document.createElementNS(NS,'svg'); svg.setAttribute('id','boxSvg'); svg.setAttribute('width', String(W)); svg.setAttribute('height', String(H)); svg.setAttribute('viewBox',`0 0 ${W} ${H}`); svg.setAttribute('font-family','sans-serif'); els.plotDiv.appendChild(svg);
    ...
    if(graphType==='bar' && !logScale){
      ...
      console.debug('Debug: bar axis zero clamp',{prevMin,prevMax,adjustedMin:ymin,adjustedMax:ymax});
    }
    ...
    const zeroInRange=yScale.min<=0 && yScale.max>=0;
    const baselineValue=(graphType==='bar' && !logScale && zeroInRange)?0:yScale.min;
    if(graphType==='bar'){
      console.debug('Debug: bar axis baseline',{baselineValue,zeroInRange,yScaleMin:yScale.min,yScaleMax:yScale.max});
    }
    function formatTick(v){ return v.toLocaleString('en-US',{maximumFractionDigits:2,useGrouping:false}); }
    function measureTextWidth(text,font){ const canvas=measureTextWidth.canvas || (measureTextWidth.canvas=document.createElement('canvas')); const ctx=canvas.getContext('2d'); ctx.font=font; return ctx.measureText(text).width; }
    const tickFont=`${fs}px sans-serif`; const tickLabels=yScale.ticks.map(t=>formatTick(logScale?Math.pow(10,t):t)); const tickWidths=tickLabels.map(lbl=>measureTextWidth(lbl,tickFont)); const maxTickWidth=Math.max(...tickWidths,0); const yLabelGap=maxTickWidth+fs; const baseTop=Math.max(32,Math.round(fs*2.2)); const maxLevelEstimate=state.selectedCols.size>1?state.selectedCols.size:0; const topExtra=maxLevelEstimate?(ANN_BASE_OFFSET+maxLevelEstimate*ANN_LEVEL_GAP):0; let margin={ top:baseTop+topExtra, right:20, bottom:Math.max(32,Math.round(fs*2.2)), left:Math.max(48,Math.round(fs*3.0), yLabelGap + fs) };
    // Estimate if x labels will tilt; reserve extra bottom height if needed
    let plotW=Math.max(20, W - margin.left - margin.right); let plotH=Math.max(20, H - margin.top - margin.bottom); let bandW=plotW/labelsUsed.length;
    const measCanvas=measureTextWidth.canvas||(measureTextWidth.canvas=document.createElement('canvas')); const measCtx=measCanvas.getContext('2d'); measCtx.font=tickFont;
    const labelWidths=labelsUsed.map(l=>measCtx.measureText(l||'').width); const maxLabelWidth=Math.max(0,...labelWidths); const willTilt=labelWidths.some(w=>w>bandW*0.9);
    if(willTilt){ const tiltBottomExtra=Math.min(200, Math.max(fs*1.6, Math.ceil(Math.SQRT1_2 * maxLabelWidth) + fs)); margin.bottom += tiltBottomExtra; plotW=Math.max(20, W - margin.left - margin.right); plotH=Math.max(20, H - margin.top - margin.bottom); bandW=plotW/labelsUsed.length; console.debug('Debug: box tilt bottom extra',{tiltBottomExtra,maxLabelWidth,bandW}); }
    ...
    const baselinePx=y2px(baselineValue);
    ...
    const tickLen=6; if (showGrid) { yScale.ticks.forEach(t => { const y=y2px(t); add('line',{ x1:margin.left,y1:y,x2:margin.left+plotW,y2:y,stroke:'#ddd','stroke-width':1 }); }); }
    add('line',{ x1:margin.left,y1:margin.top - tickLen,x2:margin.left,y2:margin.top + plotH + tickLen,stroke:'#000','stroke-width':1 });
    yScale.ticks.forEach(t => { const y=y2px(t); add('line',{ x1:margin.left - tickLen,y1:y,x2:margin.left,y2:y,stroke:'#000','stroke-width':1 }); const txt=add('text',{ x:margin.left - (tickLen+2), y, 'font-size':fs, 'text-anchor':'end', 'dominant-baseline':'middle', fill:'#000' }); txt.textContent=formatTick(logScale?Math.pow(10,t):t); });
    const xAxisY = baselinePx; add('line', { x1: margin.left - tickLen, y1: xAxisY, x2: margin.left + plotW + tickLen, y2: xAxisY, stroke: '#000', 'stroke-width': 1 }); const xLabelOffset=Math.max(14,fs)+tickLen; const xLabels=[]; labelsUsed.forEach((lab,i)=>{ const x=xCenter(i); add('line', { x1:x,y1:xAxisY,x2:x,y2:xAxisY+tickLen,stroke:'#000','stroke-width':1 }); const t=add('text',{ x, y:xAxisY + xLabelOffset, 'font-size':fs, 'text-anchor':'middle', fill:'#000' }); t.textContent=lab||`Col ${i+1}`; t.style.cursor='ew-resize'; enableLabelDrag(t,i); xLabels.push(t); });
    let needsTilt=false; const boxes=xLabels.map(l=>l.getBBox()); for(let i=1;i<boxes.length;i++){ const prev=boxes[i-1]; const curr=boxes[i]; if(prev.x+prev.width>curr.x){needsTilt=true;break;} } if(needsTilt||willTilt){ xLabels.forEach(l=>{ const x=l.getAttribute('x'); const y=l.getAttribute('y'); l.setAttribute('transform',`rotate(-45 ${x} ${y})`); l.setAttribute('text-anchor','end'); l.setAttribute('dy','0.35em'); }); }
    ...
    const yX = margin.left - (maxTickWidth+fs); const yText = add('text', { x: yX, y: margin.top + plotH / 2, transform: `rotate(-90 ${yX} ${margin.top + plotH / 2})`, 'text-anchor': 'middle', 'font-size': fs + 4 }); yText.textContent = state.yLabelText; makeEditable(yText,txt=>{state.yLabelText=txt;});
    for(let i=0;i<traces.length;i++){
      ...
      if (graphType === 'bar'){
        const mean=t.y.reduce((a,b)=>a+b,0)/t.y.length; const sd=Math.sqrt(t.y.reduce((a,b)=>a+Math.pow(b-mean,2),0)/(t.y.length-1||1)); const yMean=y2px(mean); const rectY=Math.min(yMean,baselinePx); const rectH=Math.abs(baselinePx-yMean); add('rect',{ x:x0, y:rectY, width:boxW, height:Math.max(1,rectH), fill:fillColor, stroke:borderColor, 'stroke-width':bw }); const ySdTop=y2px(mean+sd); const cap=Math.max(6, boxW*0.4); if(errorMode==='both'){ const ySdBottom=y2px(mean-sd); add('line',{x1:cx,y1:ySdTop,x2:cx,y2:ySdBottom,stroke:borderColor,'stroke-width':bw}); add('line',{x1:cx-cap/2,y1:ySdBottom,x2:cx+cap/2,y2:ySdBottom,stroke:borderColor,'stroke-width':bw}); } else { add('line',{x1:cx,y1:ySdTop,x2:cx,y2:yMean,stroke:borderColor,'stroke-width':bw}); } add('line',{x1:cx-cap/2,y1:ySdTop,x2:cx+cap/2,y2:ySdTop,stroke:borderColor,'stroke-width':bw});
      }
      if(pointMode!=='none'){
    ...
    const titleText = add('text', { x: margin.left + plotW / 2, y: margin.top / 2, 'text-anchor': 'middle', 'font-size': fs + 4 }); titleText.textContent = state.titleText; makeEditable(titleText,txt=>{state.titleText=txt;});
