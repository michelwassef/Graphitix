...
- Fixed shared deferred canonical capture dropping `meta.graphSizing`. Exact owner layout capture and graph-sizing enrichment now occur before the payload is assigned, with a focused regression covering the boundary.
...
- Added compact, context-aware figure-summary precision: p-values use threshold-aware reporting, estimates and intervals use restrained precision, decorated and standalone metrics lose formatter-only trailing zeros, and extra digits are retained near α = 0.05 so rounding cannot change the significance interpretation. Detailed statistical reports remain unchanged.
