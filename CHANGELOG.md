## Unreleased
- Fixed crash recovery losing the newest edits. Recovery now keeps a write-through per-tab payload journal: every payload/layout commit writes a small serialized checkpoint immediately, coalesced to at most one pending write per tab and trimmed whenever a rich recovery snapshot is captured, so a crash that lands inside the debounced snapshot window still restores the exact last committed revision instead of the previous checkpoint. Restore folds the journal over the snapshot with revision ordering and clear-on-consume semantics, and the desktop shell exposes journal IPC alongside the existing snapshot channel.
