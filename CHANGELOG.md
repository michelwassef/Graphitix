...
- Restored Box's dynamic x-label frame reserve. Longer or rotated category labels now grow the physical SVG frame by the measured bottom reserve instead of compressing the plot, while shortening labels releases that space and automatic reserves leave user aspect-lock metadata unchanged.
- Fixed resize-handle double-click resets with automatic graph reserves and flipped Box layouts. The shared resizer now restores the current component orientation's default frame plus component-owned reserves; Box preserves separate tab-owned vertical and horizontal reset frames, so manual resizing cannot make reset fall back to non-flipped proportions.
...
- Fixed Box crash-recovery/reopen plot-height parity by persisting the automatic viewport-reserve decomposition separately from the user-owned graph frame. Restored cached Box graphs now rehydrate their x-label, significance, and side reserves before the first live redraw instead of temporarily treating the bottom label reserve as extra plot height.
